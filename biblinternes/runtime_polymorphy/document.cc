...
{
	out << dls::chaine(position, ' ') <<  "\033[0;32m" << "<document>\n";
...
{
	out << dls::chaine(position, ' ') << "\033[0m" << "my_class_t\n";
}
...
void test_history()
{
	...
	current(h).emplace_back(dls::chaine("Hello !"));
	...
	current(h)[1] = object_t(dls::chaine("World"));

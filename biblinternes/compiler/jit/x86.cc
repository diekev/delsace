...
void rex(code_vector &vec, int l, int w, int r, int x, int b)
{
	...
	if (l || v != 0x40) {
		vec.insere(vec.fin(), static_cast<unsigned char>(v));
	}
}
...
void rx(code_vector &vec, int rd, int md, int rb, int ri, int ms)
{
	if (ri == _NOREG) {
		if (rb == _NOREG) {
			vec.insere(vec.fin(), mrm(0x00, rd, 0x04));
			vec.insere(vec.fin(), sib(0x00, 0x04, 0x05));
			vec.push_value(vec.fin(), md); // push_value
		}
		else if (rb == _RSP_REGNO) {
			if (md == 0) {
				vec.insere(vec.fin(), mrm(0x00, rd, 0x04));
				vec.insere(vec.fin(), sib(ms, 0x04, 0x04));
			}
			else if (true /*md == md*/) {
				vec.insere(vec.fin(), mrm(0x01, rd, 0x04));
				vec.insere(vec.fin(), sib(ms, 0x04, 0x04));
				vec.insere(vec.fin(), static_cast<unsigned char>(md));
			}
			else {
				vec.insere(vec.fin(), mrm(0x02, rd, 0x04));
				vec.insere(vec.fin(), sib(ms, 0x04, 0x04));
				vec.push_value(vec.fin(), md); // push_value
			}
		}
			...
			if (md == 0 && rb != _RBP_REGNO)
				vec.insere(vec.fin(), mrm(0x00, rd, rb));
			else if (true /*md == md*/) {
				vec.insere(vec.fin(), mrm(0x01, rd, rb));
				vec.insere(vec.fin(), static_cast<unsigned char>(md));
			}
			else {
				vec.insere(vec.fin(), mrm(0x02, rd, rb));
				vec.push_value(vec.fin(), md); // push_value
			}
		}
	}
		...
		vec.insere(vec.fin(), mrm(0x00, rd, 0x04));
		vec.insere(vec.fin(), sib(ms, ri, 0x05));
		vec.push_value(vec.fin(), md); // push_value
	}
	else if (ri != _RSP_REGNO) {
		if (md == 0 && rb != _RBP_REGNO) {
			vec.insere(vec.fin(), mrm(0x00, rd, 0x04));
			vec.insere(vec.fin(), sib(ms, ri, rb));
		}
		else if (true /*md == md*/) {
			vec.insere(vec.fin(), mrm(0x01, rd, 0x04));
			vec.insere(vec.fin(), sib(ms, ri, rb));
...
void mov(code_vector &vec, int r0, int i0)
{
	...
	vec.insere(vec.fin(), static_cast<unsigned char>(0xb8 | register_index(r0)));
	vec.push_value(vec.fin(), i0);
}

void mov_reg(code_vector &vec, int r0, int r1)
{
	rex(vec, 0, 1, r1, static_cast<int>(reg::none), r0);
	vec.insere(vec.fin(), 0x89);
	vec.insere(vec.fin(), static_cast<unsigned char>(0xc0 | (register_index(r0) << 3) | register_index(r1)));
}
...
void push(code_vector &vec, int r0)
{
	...
	vec.insere(vec.fin(), static_cast<unsigned char>(0x50 | register_index(r0)));
}
...
{
	rex(vec, 0, WIDE, 0, 0, r0);
	vec.insere(vec.fin(), static_cast<unsigned char>(0x58 | register_index(r0)));
}

/* pop stack */
void ret(code_vector &vec)
{
	vec.insere(vec.fin(), 0xc3);
}
...
void unr(code_vector &vec, int code, int r0)
{
	...
	vec.insere(vec.fin(), 0xf7);
	vec.insere(vec.fin(), mrm(0x03, code, r0));
}
...
void alu_reg(code_vector &vec, int code, int r0, int r1)
{
	...
	vec.insere(vec.fin(), static_cast<unsigned char>(code | 0x01));
	vec.insere(vec.fin(), mrm(0x03, r1, r0));
}
...
{
	rex(vec, 0, WIDE, r0, _NOREG, r1);
	vec.insere(vec.fin(), 0x0f);
	vec.insere(vec.fin(), 0xaf);
	vec.insere(vec.fin(), mrm(0x03, r0, r1));
}

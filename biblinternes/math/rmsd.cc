...
void normalize(double a[3])
{
	...
	b = std::sqrt(a[0]*a[0] + a[1]*a[1] + a[2]*a[2]);
	a[0] /= b;
	a[1] /= b;
	a[2] /= b;
}
...
double* residual)
{
   ...
   */
	...
	*residual = E0 - std::sqrt(std::fabs(eigenval[0]))
			- std::sqrt(std::fabs(eigenval[1]))
			- sigma * std::sqrt(std::fabs(eigenval[2]));
...
double* rmsd)
{
	...
	*rmsd = std::sqrt(std::fabs(residual * 2.0/ static_cast<double>(n_list)) );
}
...
{ 
   ...
   */
	...
	residual = Eo - std::sqrt(r1) - std::sqrt(r2) - omega * std::sqrt(rlow);
	*rmsd = std::sqrt(residual * 2.0 / static_cast<double>(n_list) );
}

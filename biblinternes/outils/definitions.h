...
#define REMBOURRE(x)                                                           \
  _Pragma("clang diagnostic push")                                             \
      _Pragma("clang diagnostic ignored \"-Wunused-private-field\"") char      \
      VARIABLE_ANONYME(_pad)[x] _Pragma("clang diagnostic pop")
#else
#define REMBOURRE(x)
#endif
...
#define DEFINIS_OPERATEURS_DRAPEAU_IMPL(_type_drapeau_, _type_)                \
  ...
  inline bool drapeau_est_actif(_type_drapeau_ const v,                        \
                                _type_drapeau_ const d) {                      \
    return (d & v) != _type_drapeau_(0);                                       \
  }

#define DEFINIS_OPERATEURS_DRAPEAU(_type_drapeau_)                             \
  DEFINIS_OPERATEURS_DRAPEAU_IMPL(_type_drapeau_,                              \
                                  std::underlying_type_t<_type_drapeau_>)

#define taille_de(x) static_cast<int64_t>(sizeof(x))

#define POINTEUR_NUL(Type)                                                     \
  static inline Type *nul() { return static_cast<Type *>(nullptr); }           \

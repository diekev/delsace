...
auto SHA2_UNPACK32(const uint32 w, uint8 *s)
{
	*(s + 3) = static_cast<uint8>(w      );
	*(s + 2) = static_cast<uint8>(w >>  8);
	*(s + 1) = static_cast<uint8>(w >> 16);
	*(s + 0) = static_cast<uint8>(w >> 24);
}

auto SHA2_PACK32(const uint8 *s, uint32 *w)
{
	*w =   static_cast<uint32>(*(s + 3)      )
		 | static_cast<uint32>(*(s + 2) <<  8)
		 | static_cast<uint32>(*(s + 1) << 16)
		 | static_cast<uint32>(*(s + 0) << 24);
}
...
void SHA256::transform(const uint8 *message, uint32 block_nb)
{
	uint32 w[64];
	uint32 wv[8];
	uint32 t1, t2;
...
void SHA256::update(const uint8 *message, uint32 len)
{
	...
	const auto shifted_message = message + rem_len;
	transform(m_block, 1);
...
{
	const auto block_nb = (1u + ((SHA224_256_BLOCK_SIZE - 9u) < (m_len % SHA224_256_BLOCK_SIZE)));

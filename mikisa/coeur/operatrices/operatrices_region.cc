...
{
	usine->register_type(NOM_ANALYSE, create_desc<OperatriceAnalyse>(NOM_ANALYSE, AIDE_ANALYSE));
	usine->register_type(NOM_FILTRAGE, create_desc<OperatriceFiltrage>(NOM_FILTRAGE, AIDE_FILTRAGE));
	usine->register_type(NOM_FLOU, create_desc<OperatriceFloutage>(NOM_FLOU, AIDE_FLOU));
	usine->register_type(NOM_CHAMPS_DISTANCE, create_desc<OperatriceChampsDistance>(NOM_CHAMPS_DISTANCE, AIDE_CHAMPS_DISTANCE));
	usine->register_type(NOM_NORMALISE, create_desc<OperatriceNormalisation>(NOM_NORMALISE, AIDE_NORMALISE));
	usine->register_type(NOM_TOURNOIEMENT, create_desc<OperatriceTournoiement>(NOM_TOURNOIEMENT, AIDE_TOURNOIEMENT));
	usine->register_type(NOM_DEFORMATION, create_desc<OperatriceDeformation>(NOM_DEFORMATION, AIDE_DEFORMATION));
	usine->register_type(NOM_SIMULATION_GRAIN, create_desc<OperatriceSimulationGrain>(NOM_SIMULATION_GRAIN, AIDE_SIMULATION_GRAIN));
	usine->register_type(NOM_COORD_POLAIRE, create_desc<OperatriceCoordonneesPolaires>(NOM_COORD_POLAIRE, AIDE_COORD_POLAIRE));
	usine->register_type(NOM_ONDELETTE_HAAR, create_desc<OperatriceOndeletteHaar>(NOM_ONDELETTE_HAAR, AIDE_ONDELETTE_HAAR));
	usine->register_type(NOM_DILATION, create_desc<OperatriceDilation>(NOM_DILATION, AIDE_DILATION));
	usine->register_type(NOM_EROSION, create_desc<OperatriceErosion>(NOM_EROSION, AIDE_EROSION));
}

...
int trouve_entrevalle(int taille, const Predicate &predicat)
{
	int debut = 0, longueur = taille;

	while (longueur > 0) {
		int moitie = longueur >> 1, milieu = debut + moitie;
		...
		const float fin_lambda)
{
	...
	for (; i + 1 < n && fin_lambda >= lambdas[i]; ++i) {
		...
		somme += 0.5 * entrepole(debut_segment, i) + entrepole(debut_segment, i) * (fin_segment - debut_segment);
	}

	return somme / (fin_lambda - debut_lambda);
}

bool echantillons_spectre_trie(const float *lambdas, const float *, int n)
{
	for (int i = 0; i < n - 1; ++i) {
		...
		float l)
{
	...
	const auto offset = trouve_entrevalle(n, [&](int index) { return lambdas[index] <= l; });
...
void corps_noir(const float *lambda, int n, float T, float *Le)
{
	...
	const auto c = 299792458;
	const auto h = 6.62606957e-34;
	const auto kb = 1.3806488e-23;

	for (int i = 0; i < n; ++i) {
		/* Calcule la radiance émise pour le corps noir à la longeur d'onde
		 * lambda[i] */
		auto l = lambda[i] * 1e-9;
...
SpectreEchantillon SpectreEchantillon::depuis_echantillons(const float *lambda, const float *v, int n)
{
...
void SpectreEchantillon::vers_xyz(float xyz[]) const
{
	...
	float scale = float(FIN_LAMBDA_SPECTRE - DEBUT_LAMBDA_SPECTRE) /
				   float(CIE_Y_integral * NOMBRE_ECHANTILLONS);
	xyz[0] *= scale;
	xyz[1] *= scale;
	xyz[2] *= scale;
}
...
float SpectreEchantillon::y() const
{
	...
	return yy * float(FIN_LAMBDA_SPECTRE - DEBUT_LAMBDA_SPECTRE) /
			float(CIE_Y_integral * NOMBRE_ECHANTILLONS);
}
...
SpectreRGB SpectreRGB::depuis_echantillons(const float *lambda, const float *v, int n)
{
	if (!echantillons_spectre_trie(lambda, v, n)) {
		std::vector<float> slambda(&lambda[0], &lambda[n]);
		std::vector<float> sv(&v[0], &v[n]);

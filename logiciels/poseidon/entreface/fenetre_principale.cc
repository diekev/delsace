...
FenetrePrincipale::FenetrePrincipale(QWidget *parent)
	: QMainWindow(parent)
{
	m_poseidon.fenetre_principale = this;

	m_poseidon.enregistre_commandes();
...
void FenetrePrincipale::ajoute_editeur_proprietes()
{
	...
	auto editeur_arbre = new EditriceLigneTemps(&m_poseidon, dock_arbre);
...
void FenetrePrincipale::ajoute_visionneur_image()
{
	...
	if (m_viewer_dock == nullptr) {
		...
		auto view_2d = new EditeurCanevas(m_poseidon, m_viewer_dock);
...
{
	//	auto moteur_rendu = m_poseidon.moteur_rendu;
...
void FenetrePrincipale::repond_action()
{
	...
	m_poseidon.repondant_commande->repond_clique( action->data().toString().toStdString(), "");
}

...
base_analyseuse::base_analyseuse(lng::tampon_source const &tampon,
                                 dls::tableau<DonneesMorceaux> &identifiants)
    : lng::analyseuse<DonneesMorceaux>(identifiants), m_tampon(tampon)
{
}
...
void base_analyseuse::lance_erreur(const dls::chaine &quoi)
{
    ...
    throw ErreurSyntactique(
        ligne, numero_ligne + 1, position_ligne, quoi, contenu, donnees().genre);
}

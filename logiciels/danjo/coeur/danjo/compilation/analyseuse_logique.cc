...
template <typename T>
Loggeuse &operator<<(Loggeuse &l, const T &)
{
    return l;
}
...
AnalyseuseLogique::AnalyseuseLogique(Manipulable *manipulable,
                                     lng::tampon_source const &tampon,
                                     dls::tableau<DonneesMorceaux> &identifiants,
                                     bool initialise_manipulable)
    : base_analyseuse(tampon, identifiants), m_manipulable(manipulable),
...
void AnalyseuseLogique::analyse_expression(const dls::chaine &nom, const int type)
{
    ...
    const auto construit_graphe = !m_initialise_manipulable &&
                                  (type == EXPRESSION_RELATION || type == EXPRESSION_SORTIE);
    ...
    while (!est_identifiant(id_morceau::POINT_VIRGULE)) {
        ...
        if (est_identifiant(id_morceau::NOMBRE_ENTIER)) {
            ...
            while (!pile.est_vide() && est_operateur(pile.haut().genre) &&
        ...
        switch (resultat.genre) {
            ...
            case id_morceau::CHAINE_LITTERALE:
                m_manipulable->ajoute_propriete(
                    nom, TypePropriete::CHAINE_CARACTERE, resultat.valeur);
                break;

...
void ajoute_point_courbe(CourbeBezier &courbe, float x, float y)
{
    ...
    courbe.extension_max.co[POINT_CENTRE].y =
        courbe.points[courbe.points.taille() - 1].co[POINT_CENTRE].y;
...
void calcule_controles_courbe(CourbeBezier &courbe)
{
    ...
    if (nombre_points > 2) {
        // premier
        {
            ...
            auto hlen = longueur(p0.co[POINT_CENTRE],
                                 p0.co[POINT_CONTROLE2]); /* original handle length */
            /* clip handle point */
        ...
        // dernier
        {
            ...
            auto hlen = longueur(p0.co[POINT_CENTRE],
                                 p0.co[POINT_CONTROLE1]); /* original handle length */
            /* clip handle point */
    ...
    if (courbe.utilise_table) {
        ...
        for (long i = 0; i < courbe.points.taille() - 1; ++i) {
            ...
            if (p0.co[POINT_CENTRE].x <= valeur && valeur <= p1.co[POINT_CENTRE].x) {
                auto fac = (valeur - p0.co[POINT_CENTRE].x) /
                           (p1.co[POINT_CENTRE].x - p0.co[POINT_CENTRE].x);
                auto mfac = 1.0f - fac;
                                           ...
                                           const dls::phys::couleur32 &valeur)
{
    ...
    if (courbe.type == COURBE_COULEUR_RGB) {
        ...
        if (ordre == R_S_V_E_B) {
            ...
            const float v0 = evalue_courbe_bezier(courbe.courbes[COURBE_COULEUR_ROUGE + index[0]],
                                                  v0in);
            const float v2 = evalue_courbe_bezier(courbe.courbes[COURBE_COULEUR_ROUGE + index[2]],
                                                  v2in);

...
Ray::Ray(
		const dls::math::vec3f& origin,
		const dls::math::vec3f& direction,
		const float& frame,
...
Intersection::Intersection(
		...
		const dls::math::vec3f& normal,
		const dls::math::vec2f& uv,
...
Intersection Intersection::CompareClosestAgainst(
		...
		const dls::math::vec3f& point)
{
	...
	auto distanceA = longueur(m_point - point);
	auto distanceB = longueur(b.m_point - point);
...
{
	auto transformedPoint = dls::math::vec3f(m*dls::math::vec4f(m_point, 1.0f));
	auto transformedNormal = dls::math::vec3f(m*dls::math::vec4f(m_normal, 0.0f));

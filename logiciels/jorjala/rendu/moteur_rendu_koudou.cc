...
static int ajoute_vertex(dls::tableau<dls::math::vec3i> &vertex,
...
static void ajoute_volume(kdo::Scene &scene, kdo::maillage *maillage, Corps const &corps)
{
...
static void ajoute_sphere(kdo::Scene &scene_koudou, Corps const &corps)
{
    auto points = corps.points_pour_lecture();
...
MoteurRenduKoudou::MoteurRenduKoudou() : m_koudou(new kdo::Koudou())
{
}
    ...
    StatistiquesRendu &stats, float *tampon, int hauteur, int largeur, bool rendu_final)
{
...
#else
            memoire_normaux += mail->normaux.taille() *
                               static_cast<long>(sizeof(dls::math::vec3f));
#endif
        }
    }
    ...
    moteur_rendu->pointeur_pellicule()->redimensionne(dls::math::Hauteur(m_camera->hauteur()),
    ...
    auto const carreaux_x = static_cast<unsigned>(
        std::ceil(static_cast<float>(largeur_pellicule) / static_cast<float>(largeur_carreau)));
    auto const carreaux_y = static_cast<unsigned>(
        std::ceil(static_cast<float>(hauteur_pellicule) / static_cast<float>(hauteur_carreau)));
    ...
    for (; e < nombre_echantillons; ++e) {
        ...
        // m_notaire->signale_progres_avance(((static_cast<float>(e) + 1.0f) /
        // static_cast<float>(nombre_echantillons)) * 100.0f);
    }

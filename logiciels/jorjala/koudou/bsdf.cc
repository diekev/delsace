...
BSDF::BSDF(ContexteNuancage &ctx) : contexte(ctx)
{
}

/* ************************************************************************** */

BSDFTrivial::BSDFTrivial(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFTrivial::evalue_echantillon(GNA &gna,
...
void BSDFTrivial::genere_echantillon(GNA &gna,
                                     ParametresRendu const &parametres,
                                     dls::math::vec3d &dir,
                                     Spectre &L,
                                     double &pdf,
...
BSDFAngleVue::BSDFAngleVue(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFAngleVue::evalue_echantillon(GNA &gna,
                                      ParametresRendu const &parametres,
                                      dls::math::vec3d const &dir,
                                      Spectre &L,
                                      double &pdf)
...
void BSDFAngleVue::genere_echantillon(GNA &gna,
                                      ParametresRendu const &parametres,
                                      dls::math::vec3d &dir,
                                      Spectre &L,
                                      double &pdf,
                                      uint profondeur)
{
    ...
    L = Spectre(couleur) *
        spectre_lumiere(parametres, parametres.scene, gna, contexte.P, contexte.N);
}

/* ************************************************************************** */

BSDFDiffus::BSDFDiffus(ContexteNuancage &ctx, Spectre s) : BSDF(ctx), spectre(s)
{
}

void BSDFDiffus::evalue_echantillon(GNA &gna,
                                    ParametresRendu const &parametres,
                                    dls::math::vec3d const &dir,
                                    Spectre &L,
                                    double &pdf)
...
void BSDFDiffus::genere_echantillon(GNA &gna,
                                    ParametresRendu const &parametres,
                                    dls::math::vec3d &dir,
                                    Spectre &L,
                                    double &pdf,
...
BSDFReflectance::BSDFReflectance(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFReflectance::evalue_echantillon(GNA &gna,
                                         ParametresRendu const &parametres,
                                         dls::math::vec3d const &dir,
                                         Spectre &L,
                                         double &pdf)
...
void BSDFReflectance::genere_echantillon(GNA &gna,
                                         ParametresRendu const &parametres,
                                         dls::math::vec3d &dir,
                                         Spectre &L,
                                         double &pdf,
...
BSDFVerre::BSDFVerre(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFVerre::evalue_echantillon(GNA &gna,
                                   ParametresRendu const &parametres,
                                   dls::math::vec3d const &dir,
                                   Spectre &L,
                                   double &pdf)
...
void BSDFVerre::genere_echantillon(GNA &gna,
                                   ParametresRendu const &parametres,
                                   dls::math::vec3d &dir,
                                   Spectre &L,
                                   double &pdf,
                                   uint profondeur)
{
    ...
    /* Calcul la réfraction s'il n'y a pas un cas de réflection interne totale. */
    if (kr < 1) {
        auto refractionDirection = normalise(
            refracte(rayon.direction, contexte.N, index_refraction));
...
BSDFVolume::BSDFVolume(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFVolume::evalue_echantillon(GNA &gna,
                                    ParametresRendu const &parametres,
                                    dls::math::vec3d const &dir,
                                    Spectre &L,
                                    double &pdf)
...
void BSDFVolume::genere_echantillon(GNA &gna,
                                    ParametresRendu const &parametres,
                                    dls::math::vec3d &dir,
                                    Spectre &L,
                                    double &pdf,
                                    uint profondeur)
{
    ...
    while (n--) {
        accumulation[0] *= static_cast<float>(
            std::exp(-densite * static_cast<double>(sigma_a[0] + sigma_s[0]) * 0.001));
        accumulation[1] *= static_cast<float>(
            std::exp(-densite * static_cast<double>(sigma_a[1] + sigma_s[1]) * 0.001));
        accumulation[2] *= static_cast<float>(
            std::exp(-densite * static_cast<double>(sigma_a[2] + sigma_s[2]) * 0.001));
    }
...
BSDFPhaseIsotropique::BSDFPhaseIsotropique(ContexteNuancage &ctx) : BSDF(ctx)
{
}

void BSDFPhaseIsotropique::evalue_echantillon(GNA &gna,
                                              ParametresRendu const &parametres,
                                              dls::math::vec3d const &dir,
                                              Spectre &L,
                                              double &pdf)
...
void BSDFPhaseIsotropique::genere_echantillon(GNA &gna,
                                              ParametresRendu const &parametres,
                                              dls::math::vec3d &dir,
                                              Spectre &L,
                                              double &pdf,
...
BSDFPhaseAnisotropique::BSDFPhaseAnisotropique(ContexteNuancage &ctx, double g_) : BSDF(ctx), g(g_)
{
    if (g < -1.0) {
        g = -1.0;
    }
...
void BSDFPhaseAnisotropique::evalue_echantillon(GNA &gna,
                                                ParametresRendu const &parametres,
                                                dls::math::vec3d const &dir,
                                                Spectre &L,
                                                double &pdf)
...
void BSDFPhaseAnisotropique::genere_echantillon(GNA &gna,
                                                ParametresRendu const &parametres,
                                                dls::math::vec3d &dir,
                                                Spectre &L,
                                                double &pdf,
...
{
    return 0.25 * un_moins_g2 /
           (constantes<double>::PI * std::pow(un_plus_g2 - 2.0 * g * cos_theta, 1.5f));
}

...
Volume::Volume(ContexteNuancage &ctx) : contexte(ctx)
{
}

/* ************************************************************************** */

VolumeLoiBeers::VolumeLoiBeers(ContexteNuancage &ctx) : Volume(ctx)
{
}

bool VolumeLoiBeers::integre(GNA &gna,
...
Spectre VolumeLoiBeers::transmittance(GNA &gna,
                                      ParametresRendu const & /*parametres*/,
...
VolumeHeterogeneLoiBeers::VolumeHeterogeneLoiBeers(ContexteNuancage &ctx,
    ...
    : Volume(ctx), grille(grille_)
{
}

bool VolumeHeterogeneLoiBeers::integre(GNA &gna,
                                       ParametresRendu const &parametres,
                                       Spectre &L,
                                       Spectre &tr,
                                       Spectre &poids,
...
Spectre VolumeHeterogeneLoiBeers::transmittance(GNA &gna,
                                                ParametresRendu const & /*parametres*/,
                                                dls::math::point3d const &P0,
                                                dls::math::point3d const &P1)
{
...
VolumeDiffusionSimple::VolumeDiffusionSimple(ContexteNuancage &ctx) : Volume(ctx)
{
}

bool VolumeDiffusionSimple::integre(GNA &gna,
                                    ParametresRendu const &parametres,
                                    Spectre &L,
                                    Spectre &tr,
                                    Spectre &poids,
                                    dls::phys::rayond &wo)
{
    ...
    auto xi = gna.uniforme(0.0, 1.0);
    auto distance_diffusion = -std::log(1.0f - static_cast<float>(xi) * (1.0f - tr.y())) /
                              extinction.y();

    /* Initialise le nuançage au point de diffusion. */
    auto P_diffusion = contexte.P +
                       static_cast<double>(distance_diffusion) * contexte.rayon.direction;
    ...
    auto trd = Spectre::depuis_rgb(std::exp(extinction[0] * -distance_diffusion),
...
Spectre VolumeDiffusionSimple::transmittance(GNA &gna,
                                             ParametresRendu const &parametres,
                                             dls::math::point3d const &P0,
                                             dls::math::point3d const &P1)
{
    ...
    return Spectre::depuis_rgb(std::exp(extinction[0] * -distance),
...
VolumeHeterogeneDiffusionSimple::VolumeHeterogeneDiffusionSimple(
    ContexteNuancage &ctx, wlk::grille_eparse<float> const *grille_)
    : Volume(ctx), grille(grille_)
{
}

bool VolumeHeterogeneDiffusionSimple::integre(GNA &gna,
                                              ParametresRendu const &parametres,
                                              Spectre &L,
                                              Spectre &tr,
                                              Spectre &poids,
                                              dls::phys::rayond &wo)
{
    ...
    if (termine) {
        ...
        auto transmittance_rayon = 1.0;  //  À FAIRE
        auto poids_MIS = 1.0;            //  À FAIRE
        auto L_lumiere = spectre_lumiere(
            parametres, parametres.scene, gna, contexte.P, contexte.N);

        bsdf_phase.evalue_echantillon(gna, parametres, dir_echantillon, L_bsdf, pdf_bsdf);

        L += L_lumiere * L_bsdf *
             static_cast<float>(transmittance_rayon * poids_MIS / pdf_lumiere);

        bsdf_phase.genere_echantillon(gna, parametres, dir_echantillon, L_bsdf, pdf_bsdf, 1);
        // rs.EvaluateLightSample(m_ctx, sampleDirection, L_lumiere, pdf_lumiere,
        // beamTransmittance);

        L += L_lumiere * L_bsdf * beamTransmittance * albedo_diffusion * extinction *
             static_cast<float>(poids_MIS);
...
Spectre VolumeHeterogeneDiffusionSimple::transmittance(GNA &gna,
                                                       ParametresRendu const &parametres,
                                                       dls::math::point3d const &P0,
                                                       dls::math::point3d const &P1)
{
    ...
    return Spectre::depuis_rgb(std::exp(-distance), std::exp(-distance), std::exp(-distance));
}

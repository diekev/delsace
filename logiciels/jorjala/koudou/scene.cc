                        ...
                        dls::math::vec3d const &nor)
{
            ...
            case type_lumiere::DISTANTE:
            {
                ...
                if (ombre > 0.0) {
                    spectre += lumiere->spectre *
                               static_cast<float>(lumiere->intensite * angle * ombre);
                }
    ...
    calcul_direction_inverse(rayon);
    spectre += spectre_monde(scene.monde, direction) *
               static_cast<float>(ombre_scene(parametres, scene, rayon, 1000.0));
...
{
    auto r = dls::math::vec2d(gna.uniforme(0.0, 1.0) * constantes<double>::TAU,
                              gna.uniforme(0.0, 1.0) * constantes<double>::TAU);
    auto sin_r = sin(r[0]);
...
delegue_scene::delegue_scene(const Scene &scene) : ptr_scene(scene)
{
}

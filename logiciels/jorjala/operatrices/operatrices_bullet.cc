...
static btCollisionShape *cree_forme_pour_corps(Corps &corps)
{
    ...
    auto colShape = memoire::loge<btBoxShape>("btBoxShape",
                                              btVector3(static_cast<double>(taille.x * 0.5f),
    ...
    //	auto hull_shape = memoire::loge<btConvexHullShape>(&(hull_computer.vertices[0].getX()),
    //hull_computer.vertices.taille());
...
class MondePhysique {
  ...
  public:
    ...
    void ajoute_corps_rigide(btRigidBody *corps_rigide,
                             Objet *objet,
                             math::transformation const &transformation)
    {
        //	std::cerr << "---------------------------------------------------\n";
    ...
    void initialise_monde()
    {
        ...
        m_repartitrice = memoire::loge<btCollisionDispatcher>("btCollisionDispatcher",
        ...
        m_monde_dynamics = memoire::loge<btDiscreteDynamicsWorld>("btDiscreteDynamicsWorld",
                                                                  m_repartitrice,
                                                                  m_tampon_paires,
...
class AjoutCorpsRigide final : public OperatriceCorps {
  ...
  public:
    ...
    AjoutCorpsRigide(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(2);
        sorties(1);
    }
    ...
    {
        return CheminFichier{"entreface/operatrice_ajout_corps_rigide.jo"};
    }
    ...
    btRigidBody *cree_corps_rigide(btTransform const &transforme_initiale,
                                   btCollisionShape *forme_collision)
    {
         ...
         * 'actifs'. */
        auto etat_mouvement = memoire::loge<btDefaultMotionState>("btDefaultMotionState",
                                                                  transforme_initiale);

        auto infos = btRigidBody::btRigidBodyConstructionInfo(
            masse, etat_mouvement, forme_collision, intertie_locale);
        ...
        corps_rigide->setDamping(static_cast<double>(amort_lin), static_cast<double>(amort_ang));
        corps_rigide->setSleepingThresholds(static_cast<double>(seuil_lin),
        ...
        //	corps_rigide->setCollisionFlags(corps_rigide->getCollisionFlags() |

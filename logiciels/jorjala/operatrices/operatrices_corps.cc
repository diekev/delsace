...
class OperatriceCreationCorps : public OperatriceCorps {
  ...
  public:
    ...
    {
        return type == MANIPULATION_POSITION || type == MANIPULATION_ECHELLE ||
...
class OpImportObjet final : public OperatriceCorps {
  ...
  public:
    ...
    OpImportObjet(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(0);
        sorties(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
            ...
            if (chemin.trouve(".obj") != dls::chaine::npos) {
                m_poignee_fichier->lecture_chemin(
                    [](const char *chemin_, std::any const &donnees_) {
                        objets::charge_fichier_OBJ(
                            std::any_cast<AdaptriceCreationCorps *>(donnees_), chemin_);
                    },
                    donnees);
            }
        ...
        m_transformation = math::construit_transformation(
            dls::math::vec3f(position), rotation, taille * echelle_uniforme);
...
class OperatriceFusionnageCorps final : public OperatriceCorps {
  public:
    ...
    void fusionne_primitives(Corps const *corps1, Corps const *corps2)
    {
        ...
        pour_chaque_polygone(*corps1, [&](Corps const &, Polygone *poly) {
        ...
        pour_chaque_polygone(*corps2, [&](Corps const &, Polygone *poly) {
            auto polygone = m_corps.ajoute_polygone(poly->type, poly->nombre_sommets());
            ...
            if (paire_attr.first != nullptr && paire_attr.second != nullptr) {
                ...
                if (attr1->type() != attr2->type()) {
                    ajoute_avertissement(
                        "Les types des attributs '", attr1->nom(), "' sont différents !");
                    continue;
                }

                if (attr1->portee != attr2->portee) {
                    ajoute_avertissement(
                        "Les portées des attributs '", attr1->nom(), "' sont différentes !");
                    continue;
                }

                auto attr = m_corps.ajoute_attribut(
                    attr1->nom(), attr1->type(), attr1->dimensions, attr1->portee);

                std::memcpy(attr->donnees(),
                            attr1->donnees(),
                            static_cast<size_t>(attr1->taille_octets()));
                ...
                auto attr = m_corps.ajoute_attribut(
                    attr2->nom(), attr2->type(), attr2->dimensions, attr2->portee);
...
class OperatriceSeparationPrims final : public OperatriceCorps {
  public:
    ...
    static constexpr auto AIDE = "Sépare les primitives du corps d'entrée de sorte que chaque "
                                 "primitive du corps de sortie n'ait que des points uniques.";
...
static auto ajoute_deformeur(Deformer &deformeurs,
                             ...
                             Scalar time)
{
    ...
    pos_eigen << static_cast<double>(pos.x), static_cast<double>(pos.y),
...
static auto deforme_kelvinlet(const Vector3 &p,
                              const Scalar &t,
...
static void ajoute_deformeur(Deformer &deformeur,
                             dls::math::vec3f const &pos,
                             bool pousse,
                             ...
                             double temps)
{
    if (action == "grab") {
        ...
        if (pousse) {
            ...
            ajoute_deformeur<DynaType>(
                deformeur, pos, f, incompressibilite, vitesse, echelle, temps);
        }
        else {
...
class OpCreationKelvinlet final : public OperatriceCorps {
  public:
    ...
    static constexpr auto AIDE =
        "Ajout un ou plusieurs déformeurs pour calculer les dynamiques de Kelvinlets.";
        ...
        boucle_parallele(tbb::blocked_range<long>(0, points_entree.taille()),
                         [&](tbb::blocked_range<long> const &plage) {
                             for (auto i = plage.begin(); i < plage.end(); ++i) {
                                 ...
                                 point_eigen << static_cast<double>(p.x), static_cast<double>(p.y),
                                     static_cast<double>(p.z);

                                 auto dist = deforme_kelvinlet(
                                     point_eigen, temps, m_deformeurs, rk4);
...
static void ajoute_deformeur(DeformeursBrosse &deformer,
...
static Vector3 deforme_kelvinlet_brosse(const Vector3 &p, const DeformeursBrosse &deformer)
{
    Vector3 u = Vector3::Zero();
                         ...
                         [&](tbb::blocked_range<long> const &plage) {
                             for (auto i = plage.begin(); i < plage.end(); ++i) {
                                 ...
                                 for (auto j = 0; j < repetition; ++j) {
                                     point_eigen << static_cast<double>(p.x),
                             ...
                             auto delta = static_cast<float>(plage.end() - plage.begin()) /
                                          static_cast<float>(points_entree.taille());
...
class OpCreationPancarte final : public OperatriceCorps {
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto attr_UV = m_corps.ajoute_attribut("UV", type_attribut::R32, 2, portee_attr::VERTEX);
    ...
    void cree_pancarte(vision::Camera3D *camera,
                       dls::math::vec3f const &pos,
                       Attribut &attr_UV,

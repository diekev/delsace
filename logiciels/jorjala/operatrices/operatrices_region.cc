...
template <typename T, typename TypeOperation>
void applique_fonction_position(ChefExecution *chef,
                                wlk::grille_dense_2d<T> &image,
                                TypeOperation &&op)
{
    auto const res_x = image.desc().resolution.x;
    auto const res_y = image.desc().resolution.y;
...
class OperatriceAnalyse final : public OperatriceImage {
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        applique_fonction_position(
            chef, image_tampon, [&](dls::phys::couleur32 const & /*pixel*/, int l, int c) {
        ...
        auto calque = m_image.ajoute_calque(
            nom_calque, image_tampon.desc(), wlk::type_grille::COULEUR);
...
class OperatriceFiltrage final : public OperatriceImage {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto calque = m_image.ajoute_calque(
            nom_calque, tampon_entree->desc(), wlk::type_grille::COULEUR);
...
auto extrait_min_max(grille_couleur const &grille,
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (operation == "max") {
            ...
            applique_fonction(
                chef, *tampon, [&](dls::phys::couleur32 const &pixel) { return maximum * pixel; });
        }
        else {
            ...
            applique_fonction(chef, *tampon, [&](dls::phys::couleur32 const &pixel) {
...
class OpAffinageImage final : public OperatriceImage {
  public:
    ...
    OpAffinageImage(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
    }
...
class OpMedianeImage final : public OperatriceImage {
  public:
    ...
    OpMedianeImage(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
    }
            ...
            tbb::blocked_range<int>(0, res_y), [&](tbb::blocked_range<int> const &plage) {
                ...
                for (int y = plage.begin(); y < plage.end(); ++y) {
                    ...
                    for (int x = 0; x < res_x; ++x) {
                        ...
                        if (est_paire) {
                            res.r = (valeurs_R[moitie_taille] + valeurs_R[moitie_taille + 1]) *
                                    0.5f;
                            res.v = (valeurs_V[moitie_taille] + valeurs_R[moitie_taille + 1]) *
                                    0.5f;
                            res.b = (valeurs_B[moitie_taille] + valeurs_R[moitie_taille + 1]) *
                                    0.5f;
                            res.a = (valeurs_A[moitie_taille] + valeurs_R[moitie_taille + 1]) *
                                    0.5f;
                        }
                        else {
                    ...
                    for (int x = 0; x < res_x; ++x) {
                        ...
                        for (auto j = -taille; j <= taille; ++j) {
                            for (auto i = -taille; i <= taille; ++i) {
                                ...
                                auto gs = gaussien(std::sqrt(static_cast<float>(i * i + j * j)),
                                                   sigma_s);
...
class OpLueurImage final : public OperatriceImage {
  public:
    ...
    static constexpr auto AIDE =
        "Ajoute une lueur autour des parties les plus brillantes de l'image.";

    OpLueurImage(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
    }
...
class OperatriceTournoiement final : public OperatriceImage {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        applique_fonction_position(
            chef, *tampon, [&](dls::phys::couleur32 const & /*pixel*/, int l, int c) {
...
static void calcule_distance(wlk::grille_dense_2d<float> &phi, int x, int y, float h)
{
    auto a = std::min(phi.valeur(dls::math::vec2i(x - 1, y)),
                      phi.valeur(dls::math::vec2i(x + 1, y)));
    auto b = std::min(phi.valeur(dls::math::vec2i(x, y - 1)),
                      phi.valeur(dls::math::vec2i(x, y + 1)));
    auto xi = 0.0f;
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (methode == "balayage_rapide") {
            ...
            // dls::image::operation::navigation_estime::genere_champs_distance(image_grise,
            // image_tampon, valeur_iso);
        }
        else if (methode == "8SSEDT") {
            // dls::image::operation::ssedt::genere_champs_distance(image_grise, image_tampon,
            // valeur_iso);
        }
...
class OperatriceDeformation final : public OperatriceImage {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto calque = m_image.ajoute_calque(
            nom_calque_a, tampon_a->desc(), wlk::type_grille::COULEUR);
        auto tampon = extrait_grille_couleur(calque);
...
static void assemble_image(grille_couleur &image,
...
static type_image_grise simule_grain_image(ChefExecution *chef,
                                           ...
                                           const float sigma_filtre)
{
    ...
    for (auto i = 0; i <= MAX_NIVEAU_GRIS; ++i) {
        ...
        auto const lambda_tmp = -((ag * ag) / (constantes<float>::PI *
                                               (rayon_max * rayon_max + sigma * sigma))) *
                                std::log(1.0f - u);
            ...
            for (int i = 0; i < res_x; ++i) {
                ...
                for (auto k = 0; k < iter; ++k) {
                    ...
                    for (int jd = static_cast<int>(min_y); jd <= static_cast<int>(max_y); ++jd) {
                        for (int id = static_cast<int>(min_x); id <= static_cast<int>(max_x);
                             ++id) {
                            ...
                            // échantillone image
                            auto const u = std::max(0.0f,
                                                    std::min(1.0f,
                                                             image.valeur(dls::math::vec2i(
                            ...
                            for (unsigned l = 1; l <= Q; ++l) {
                                ...
                                if (sigma_rayon > 0.0f) {
                                    rayon_courant = std::min(
                                        std::exp(mu + sigma * gna_local.uniforme(0.0f, 1.0f)),
                                        rayon_max);
...
class OperatriceSimulationGrain final : public OperatriceImage {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto const &bruit_rouge = simule_grain_image(
            chef, canal_rouge, graine, rayon_grain_r, sigma_rayon_r, sigma_filtre_r);
        auto const &bruit_vert = simule_grain_image(
            chef, canal_vert, graine, rayon_grain_v, sigma_rayon_v, sigma_filtre_v);
        auto const &bruit_bleu = simule_grain_image(
            chef, canal_bleu, graine, rayon_grain_b, sigma_rayon_b, sigma_filtre_b);
...
class OperatriceCoordonneesPolaires final : public OperatriceImage {
  public:
    ...
    static constexpr auto AIDE =
        "Transforme une image entre les coordonnées cartésiennes et les coordonnées polaires.";
...
class OperatriceDilation final : public OperatriceImage {
  public:
    ...
    static constexpr auto AIDE = "Dilate les pixels de l'image. Les parties les plus sombres de "
                                 "l'image rapetissent tandis que les plus claires grossissent.";
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto performe_dilation = [&](dls::phys::couleur32 const & /*pixel*/, int y, int x) {
...
class OperatriceErosion final : public OperatriceImage {
  public:
    ...
    static constexpr auto AIDE = "Érode les pixels de l'image. Les parties les plus claires de "
                                 "l'image rapetissent tandis que les plus sombres grossissent.";
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto performe_erosion = [&](dls::phys::couleur32 const & /*pixel*/, int y, int x) {
...
static void initialise_causal_prefiltre(dls::phys::couleur32 *pixel,
...
static void initialise_anticausal_prefiltre(dls::phys::couleur32 *pixel)
{
    auto const Zp = std::sqrt(3.0f) - 2.0f;
...
class OpRayonsSoleil final : public OperatriceImage {
  public:
    ...
    OpRayonsSoleil(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        boucle_parallele(
            tbb::blocked_range<int>(0, hauteur), [&](tbb::blocked_range<int> const &plage) {
...
static void calcule_ondelette(grille_couleur const &source,
                              ...
                              grille_couleur &grille_haute)
{
    ...
    auto poids_passe_haute = dls::tableau<float>(
        {1.0f,  -6.0f, 10.0f,  -6.0f,  1.0f,   -6.0f, 36.0f, -60.0f, 36.0f,
         -6.0f, 10.0f, -60.0f, 100.0f, -60.0f, 10.0f, -6.0f, 36.0f,  -60.0f,
         36.0f, -6.0f, 1.0f,   -6.0f,  10.0f,  -6.0f, 1.0f});

    auto poids_passe_basse = dls::tableau<float>(
        {0.0f,  0.00f, 0.0f, 0.00f, 0.0f, 0.0f,  0.25f, 0.5f, 0.25f, 0.0f, 0.0f,  0.50f, 1.0f,
         0.50f, 0.0f,  0.0f, 0.25f, 0.5f, 0.25f, 0.0f,  0.0f, 0.00f, 0.0f, 0.00f, 0.0f});
            ...
            for (auto x = 0; x < res_x; ++x) {
                ...
                for (auto yy = y - 2; yy <= y + 2; ++yy) {
                    for (auto xx = x - 2; xx <= x + 2; ++xx, ++idx) {
                        passe_basse += source.valeur(dls::math::vec2i(xx, yy)) *
                                       poids_passe_basse[idx];
                        passe_haute += source.valeur(dls::math::vec2i(xx, yy)) *
                                       poids_passe_haute[idx];
                    }
                }
...
class OpMappageTonalOndelette final : public OperatriceImage {
  ...
  public:
    ...
    void calcule_mappage(grille_couleur const *tampon_entree, grille_couleur *tampon)
    {
...
static auto calcul_energie(dls::tableau<float> const &bruit, int taille, int dimensions)
{
    auto calcul_energie_ex = [&](tbb::blocked_range<int> const &plage, float init) {
        ...
        for (auto i = plage.begin(); i < plage.end(); ++i) {
            ...
            for (auto j = 0; j < taille_carree; j += 4) {
                ...
                imageDistX = ou_binaire(et_binaire(maskX, imageDistX),
                                        etnon_binaire(maskX, imageWrapX));
                imageDistY = ou_binaire(et_binaire(maskY, imageDistY),
                                        etnon_binaire(maskY, imageWrapY));
    ...
    auto energie = tbb::parallel_reduce(
        tbb::blocked_range<int>(0, taille * taille), 0.0f, calcul_energie_ex, std::plus<float>());
...
struct bruit_bleu_genetique {
    ...
    static type_chromosome croise(type_donnees const &,
                                  type_chromosome &chr1,
                                  type_chromosome &chr2,
                                  std::mt19937 &rng)
    {
    ...
    static void calcule_aptitude(type_donnees const &donnees, type_chromosome &chromosome)
    {
        chromosome.aptitude = calcul_energie(
            chromosome.donnees, donnees.taille, donnees.dimensions);
    }
...
class OpGenerationBruitBleu final : public OperatriceImage {
  public:
    ...
    static constexpr auto AIDE = "Génère un bruit bleu utile pour ajouter une dispersion à un "
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (genetique) {
            ...
            auto res = dls::chisei::lance_algorithme_genetique<bruit_bleu_genetique>(std::cerr,
                                                                                     donnees_alg);
            ...
            for (int i = 0; i < iterations; ++i) {
                ...
                auto u1 = static_cast<int>(
                    empreinte_n32_vers_r32(static_cast<unsigned>(i * 4 + 0)) *
                    static_cast<float>(taille));
                auto u2 = static_cast<int>(
                    empreinte_n32_vers_r32(static_cast<unsigned>(i * 4 + 1)) *
                    static_cast<float>(taille));
                auto u3 = static_cast<int>(
                    empreinte_n32_vers_r32(static_cast<unsigned>(i * 4 + 2)) *
                    static_cast<float>(taille));
                auto u4 = static_cast<int>(
                    empreinte_n32_vers_r32(static_cast<unsigned>(i * 4 + 3)) *
                    static_cast<float>(taille));

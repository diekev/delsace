...
class OpEntreeGaz final : public OperatriceCorps {
  ...
  public:
    ...
    OpEntreeGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
    ...
    void performe_versionnage() override
    {
        if (propriete("mode_fusion") == nullptr) {
            ajoute_propriete(
                "mode_fusion", danjo::TypePropriete::ENUM, dls::chaine("superposition"));
...
class OpObstacleGaz final : public OperatriceCorps {
  ...
  public:
    ...
    OpObstacleGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpSimulationGaz final : public OperatriceCorps {
  ...
  public:
    ...
    OpSimulationGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(2);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        while (true) {
            ...
            if ((m_poseidon.temps_par_frame + 1e-6f) > m_poseidon.duree_frame) {
                ...
                m_poseidon.temps_total = static_cast<float>(m_poseidon.image) *
                                         m_poseidon.duree_frame;
        ...
        dessine_boite(m_corps,
                      attr_C,
                      etendu.min,
                      etendu.min + dls::math::vec3f(taille_voxel),
...
class OpAdvectionGaz final : public OperatriceCorps {
  public:
    ...
    OpAdvectionGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (poseidon_gaz->solveur_flip) {
            ...
            tbb::parallel_for(0l, poseidon_gaz->parts.taille(), [&](long i) {
                auto &p = pos_parts[i];
            ...
            if (poseidon_gaz->temperature) {
                psn::advecte_semi_lagrange(
                    *drapeaux, *vieille_vel, *poseidon_gaz->temperature, poseidon_gaz->dt, ordre);
            }

            if (poseidon_gaz->fioul) {
                psn::advecte_semi_lagrange(
                    *drapeaux, *vieille_vel, *poseidon_gaz->fioul, poseidon_gaz->dt, ordre);
            }

            if (poseidon_gaz->oxygene) {
                psn::advecte_semi_lagrange(
                    *drapeaux, *vieille_vel, *poseidon_gaz->oxygene, poseidon_gaz->dt, ordre);
            }
        }
    ...
    void performe_versionnage() override
    {
        if (propriete("ordre") == nullptr) {
            ajoute_propriete(
                "ordre", danjo::TypePropriete::ENUM, dls::chaine("semi_lagrangienne"));
        }
    }
};
...
class OpFlottanceGaz final : public OperatriceCorps {
  public:
    ...
    OpFlottanceGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpIncompressibiliteGaz final : public OperatriceCorps {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto const precision = 1.0f /
                               std::pow(10.0f, static_cast<float>(evalue_entier("précision_")));
...
class OpVorticiteGaz final : public OperatriceCorps {
  public:
    ...
    OpVorticiteGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        psn::ajoute_vorticite(
            *poseidon_gaz, quantite_vorticite, vorticite_flamme, poseidon_gaz->dt);
...
class OpAffinageGaz final : public OperatriceCorps {
  public:
    ...
    OpAffinageGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpDiffusionGaz final : public OperatriceCorps {
  public:
    ...
    OpDiffusionGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (diff_fum != 0.0f) {
            if (poseidon_gaz->densite_prev == nullptr) {
                poseidon_gaz->densite_prev = memoire::loge<wlk::grille_dense_3d<float>>(
                    "grilles", poseidon_gaz->densite->desc());
            }
        ...
        if (diff_tmp != 0.0f) {
            if (poseidon_gaz->temperature_prev == nullptr) {
                poseidon_gaz->temperature_prev = memoire::loge<wlk::grille_dense_3d<float>>(
                    "grilles", poseidon_gaz->temperature->desc());
            }
            ...
            psn::diffuse(
                *temperature, *temperature_prev, *drapeaux, iterations, precision, diff_tmp, dt);
        }
        ...
        if (diff_oxy != 0.0f) {
            if (poseidon_gaz->oxygene_prev == nullptr) {
                poseidon_gaz->oxygene_prev = memoire::loge<wlk::grille_dense_3d<float>>(
                    "grilles", poseidon_gaz->oxygene->desc());
            }
...
class OpDissipationGaz final : public OperatriceCorps {
  public:
    ...
    OpDissipationGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpErosionGaz final : public OperatriceCorps {
  public:
    ...
    OpErosionGaz(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpGrapheGaz final : public OperatriceCorps {
  ...
  public:
    ...
    OpGrapheGaz(Graphe &graphe_parent, Noeud &noeud_)
        : OperatriceCorps(graphe_parent, noeud_), m_compileuse(noeud.graphe)
    {
        m_execute_toujours = true;
        entrees(1);
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        boucle_parallele(
            tbb::blocked_range<int>(0, res_z), [&](tbb::blocked_range<int> const &plage) {
                ...
                for (auto z = plage.begin(); z < plage.end(); ++z) {
                    ...
                    for (auto y = 0; y < res_y; ++y) {
                        ...
                        for (auto x = 0; x < res_x; ++x) {
                            ...
                            m_compileuse.remplis_donnees(
                                donnees,
                                "temps",
                                static_cast<float>(contexte.temps_courant) /
                                    static_cast<float>(contexte.cadence));
                            m_compileuse.remplis_donnees(donnees, "fumée", densite->valeur(index));
                            m_compileuse.remplis_donnees(
                                donnees, "vélocité", velocite->valeur(index));

                            if (divergence != nullptr) {
                                m_compileuse.remplis_donnees(
                                    donnees, "divergence", divergence->valeur(index));
                            }

                            if (oxygene != nullptr) {
                                m_compileuse.remplis_donnees(
                                    donnees, "oxygène", oxygene->valeur(index));
                            }

                            if (temperature != nullptr) {
                                m_compileuse.remplis_donnees(
                                    donnees, "température", temperature->valeur(index));
                            }

                            if (fioul != nullptr) {
                                m_compileuse.remplis_donnees(
                                    donnees, "fioul", fioul->valeur(index));
                            }

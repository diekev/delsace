...
static res_exec cree_volume(OperatriceCorps &op,
        ...
        for (auto k = 0; k < wlk::TAILLE_TUILE; ++k) {
            for (auto j = 0; j < wlk::TAILLE_TUILE; ++j) {
                for (auto i = 0; i < wlk::TAILLE_TUILE; ++i, ++index_tuile) {
                    ...
                    tuile->donnees[index_tuile] = bruit::ondelette::evalue(params_bruit,
                                                                           pos_monde);
                }
            }
        }
...
static res_exec maillage_vers_volume(OperatriceCorps &op,
                                     ContexteEvaluation const &contexte,
                                     DonneesAval *donnees_aval,
        ...
        for (auto k = 0; k < wlk::TAILLE_TUILE; ++k) {
            for (auto j = 0; j < wlk::TAILLE_TUILE; ++j) {
                for (auto i = 0; i < wlk::TAILLE_TUILE; ++i, ++index_tuile) {
                    ...
                    dist_max *= dist_max;
#    if 1
                    auto dpp = cherche_point_plus_proche(
                        arbre_hbe, delegue_prims, rayon.origine, dist_max);
...
static void rasterise_polygone(Corps const &corps,
                               Polygone &poly,
                               ...
                               bruit::param_turbulence *params_turb)
{
    ...
    for (auto i = 0; i < nombre_echantillons; ++i) {
        ...
        if (params_bruit != nullptr) {
            ...
            auto disp = rayon *
                        bruit::evalue_turb(*params_bruit, *params_turb, nsP) /* * amplitude*/;
            wsP += N * disp;
        }
...
static void rasterise_ligne(Corps const &corps,
                            Polygone &poly,
                            wlk::Rasteriseur<float> &rast,
                            float rayon,
                            float densite,
...
static res_exec ratisse_primitives(OperatriceCorps &op,
                                   ContexteEvaluation const &contexte,
                                   DonneesAval *donnees_aval,
                                   Corps const &corps_entree)
{
    ...
    for (auto i = 0; i < prims_entree->taille(); ++i) {
        ...
        if (prim->type_prim() == type_primitive::POLYGONE) {
            ...
            if (poly->type == type_polygone::FERME) {
                rasterise_polygone(corps_entree,
                                   *poly,
                                   rast,
                                   rayon,
                                   densite,
        ...
        chef->indique_progression(static_cast<float>(i + 1) /
                                  static_cast<float>(prims_entree->taille()) * 100.0f);
    }
...
static res_exec reechantillonne_volume(OperatriceCorps &op,
                                       ContexteEvaluation const &contexte,
                                       DonneesAval *donnees_aval,
                                       Corps const &corps_entree)
{
    ...
    auto resultat = wlk::reechantillonne(*grille_scalaire,
                                         grille_scalaire->desc().taille_voxel * 2.0);
...
static void ajoute_volume_temps(wlk::grille_auxilliaire &grille_aux,
                                ...
                                bool debut)
{
     ...
     */
    wlk::pour_chaque_tuile_parallele(grille_aux, [&](wlk::grille_auxilliaire::type_tuile *tuile) {
        if (tuile->visite) {
...
static void simplifie_courbes(wlk::grille_auxilliaire &grille, float seuil_saillance)
{
    ...
    wlk::pour_chaque_tuile_parallele(grille, [&](wlk::grille_auxilliaire::type_tuile *tuile) {
        for (auto i = 0; i < wlk::VOXELS_TUILE; ++i) {
            ...
            for (auto j = 1; j < donnees.taille() - 1; ++j) {
                ...
                if (dls::math::sont_environ_egaux(v0, v1) &&
                    dls::math::sont_environ_egaux(v1, v2)) {
                    continue;
                }
    ...
    std::cerr << "Compression              : "
              << (1.0 - (static_cast<double>(nouveau_nombre_points) /
                         static_cast<double>(ancien_nombre_points)))
              << '\n';
#endif
}

static auto compresse_grille_aux(wlk::grille_auxilliaire const &grille)
{
    auto grille_temp = memoire::loge<wlk::grille_temporelle>("grille", grille.desc());

    wlk::pour_chaque_tuile(grille, [&](wlk::grille_auxilliaire::type_tuile const *tuile_aux) {
...
static auto cree_volume_temporelle(float temps_courant, float seuil_saillance)
{
    ...
    for (auto i = 0; i < 3; ++i) {
        ...
        if (i == 0) {
            grille_aux = memoire::loge<wlk::grille_auxilliaire>("wlk::grille_auxilliaire",
                                                                grille->desc());
        }
...
static auto echantillonne_grille_temp(wlk::grille_temporelle const &grille_temp, float temps)
{
    ...
    wlk::pour_chaque_tuile_parallele(*grille, [&](wlk::tuile_scalaire<float> *tuile) {
        ...
        for (auto k = 0; k < wlk::TAILLE_TUILE; ++k) {
            for (auto j = 0; j < wlk::TAILLE_TUILE; ++j) {
                for (auto i = 0; i < wlk::TAILLE_TUILE; ++i, ++index_tuile) {
                    tuile->donnees[index_tuile] = wlk::tuile_temporelle::echantillonne(
                        tuile_temp, index_tuile, temps);
                }
            }
        }
    });
...
class OpFiltrageVolume final : public OperatriceCorps {
  public:
    ...
    OpFiltrageVolume(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(1);
    }
...
class OpAffinageVolume final : public OperatriceCorps {
  public:
    ...
    OpAffinageVolume(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(1);
    }
...
class LightingGridHierarchy {
  ...
  public:
    ...
    {
        return m_nombre_niveaux;
    }  //!< Returns the number of levels in the hierarchy.
    float GetCellSize() const
    {
        return m_taille_cellule;
    }  //!< Returns the size of a cell in the lowest (finest) level of the hierarchy.
    const dls::math::vec3f &GetLightPos(int level, int i) const
    {
        return m_niveaux[level].pc.pos_point(i);
    }  //!< Returns the i^th light position at the given level. Note that this is not the position
       //!< of the light with index i.
    int GetLightIndex(int level, int i) const
    {
        return m_niveaux[level].pc.index_point(i);
    }  //!< Returns the i^th light index at the given level.
    const dls::math::vec3f &GetLightIntens(int level, int ix) const
    {
        return m_niveaux[level].colors[ix];
    }  //!< Returns the intensity of the light with index ix at the given level.
    const dls::math::vec3f &GetLightPosDev(int level, int ix) const
    {
        return level > 0 ? m_niveaux[level].pDev[ix] : m_dev_pos_n0;
    ...
    //! Builds the Lighting Grid Hierarchy for the given point light positions and intensities
    //! using the given parameters. This method builds the hierarchy using the given cellSize as
    //! the size of the lowest (finest) level grid cells.
    bool Build(const dls::math::vec3f *lightPos,          //!< Light positions.
               const dls::math::vec3f *lightIntensities,  //!< Light intensities.
               int numLights,                             //!< Number of lights.
               int minLevelLights,  //!< The minimum number of lights permitted for the highest
                                    //!< (coarsest) level of the hierarchy. The build stops if a
                                    //!< higher (coarser) level would have fewer lights.
               float cellSize,   //!< The size of a grid cell in the lowest (finest) level of the
                                 //!< hierarchy.
               int highestLevel  //!< The highest level permitted, where level 0 contains the
                                 //!< original lights.
    )
    {
        return DoBuild(
            lightPos, lightIntensities, numLights, 0, minLevelLights, cellSize, highestLevel);
    }

    //! Builds the Lighting Grid Hierarchy for the given point light positions and intensities
    //! using the given parameters. This method automatically determines the grid cell size based
    //! on the bounding box of the light positions.
    bool Build(const dls::math::vec3f *lightPos,          //!< Light positions.
               const dls::math::vec3f *lightIntensities,  //!< Light intensities.
               int numLights,                             //!< Number of lights.
               int minLevelLights,  //!< The minimum number of lights permitted for the highest
                                    ...
                                    //!< higher (coarser) level would have fewer lights.
               float autoFitScale =
                   1.01f  //!< Extends the bounding box of the light positions using the given
                          //!< scale. This value must be 1 or greater. A value slightly greater
    ...
    void Light(
        const dls::math::vec3f &pos,  //!< The position where the lighting will be evaluated.
        float alpha,  //!< The accuracy parameter. It should be 1 or greater. Larger values produce
                      //!< more accurate results with substantially more computation.
        LightingFunction
            lightingFunction  //!< This function is called for each light used for lighting
                              //!< computation. It should be in the form void LightingFunction(int
                              //!< level, int light_id, const dls::math::vec3f &light_position,
                              //!< const dls::math::vec3f &light_intensity).
    )
    {
    ...
    //! Computes the illumination at the given position using the given accuracy parameter alpha.
    //! This method provides stochastic sampling by randomly changing the given light position when
    //! calling the lighting function.
    template <typename LightingFunction>
    void Light(
        const dls::math::vec3f &pos,  //!< The position where the lighting will be evaluated.
        float alpha,  //!< The accuracy parameter. It should be 1 or greater. Larger values produce
                      //!< more accurate results with substantially more computation.
        int stochasticShadowSamples,  //!< When this parameter is zero, the given lightingFunction
                                      //!< is called once per light, using the position of the
                                      //!< light. Otherwise, it is called as many times as this
                                      //!< parameter specifies, using random positions around each
                                      //!< light position.
        LightingFunction
            lightingFunction  //!< This function is called for each light used for lighting
                              //!< computation. It should be in the form void LightingFunction(int
    ...
    )
    {
        if (m_nombre_niveaux > 1) {
            ...
            m_niveaux[0].pc.cherche_points(
                pos, r * 2, [&](int i, const dls::math::vec3f &p, float dist2, float &radius2) {
                    INUTILISE(radius2);
            ...
            auto callLightingFunc =
                [&](int level, int i, const dls::math::vec3f &p, const dls::math::vec3f &c) {
            ...
            for (int level = 1; level < m_nombre_niveaux - 1; level++) {
                ...
                m_niveaux[level].pc.cherche_points(
                    pos,
                    r * 2,
                    [&](int i, const dls::math::vec3f &p, float dist2, float &radius2) {
                        INUTILISE(radius2);
  ...
  private:
    float RandomX()
    {
        ...
        if (y > (cosf(x * constantes<float>::PI) + 1) * 0.5f)
            x -= 1;
        return x;
    }
    ...
    bool DoBuild(const dls::math::vec3f *lightPos,
                 const dls::math::vec3f *lightColor,
                 int numLights,
                 float autoFitScale,
                 int minLevelLights,
                 float cellSize = 0,
                 int highestLevel = 10)
    {
        ...
        if (autoFitScale > 0) {
            ...
            highestGridRes = dls::math::converti_type<int>(boundDif / highestCellSize) +
                             dls::math::vec3i(2);
        }
        ...
        auto addLightToNodes = [](dls::tableau<Node> &nds,
                                  const int nodeIDs[8],
                                  const dls::math::vec3f &interp,
                                  const dls::math::vec3f &light_pos,
                                  const dls::math::vec3f &light_color) {
            for (int j = 0; j < 8; j++) {
                float w = ((j & 1) ? interp.x : (1 - interp.x)) *
                          ((j & 2) ? interp.y : (1 - interp.y)) *
                          ((j & 4) ? interp.z : (1 - interp.z));
                nds[nodeIDs[j]].AddLight(w, light_pos, light_color);
            }
        };

        // Generate the grid for the highest level
        auto highestGridSize = dls::math::converti_type<float>(highestGridRes -
                                                               dls::math::vec3i(1)) *
                               highestCellSize;
        ...
        for (int z = 0, j = 0; z < highestGridRes.z; z++) {
            for (int y = 0; y < highestGridRes.y; y++) {
                for (int x = 0; x < highestGridRes.x; x++, j++) {
                    nodes[highestLevel][j].origPos = corner +
                                                     dls::math::vec3f(x, y, z) * highestCellSize;
                }
            }
        }
#endif  // CY_LIGHTING_GRID_ORIG_POS
        for (int i = 0; i < numLights; i++) {
            ...
            auto interp = gridIndex(
                index, dls::math::vec3f(lightPos[i]) - corner, highestCellSize);
            int is = index.z * highestGridRes.y * highestGridRes.x + index.y * highestGridRes.x +
                     index.x;
        ...
        for (auto level = highestLevel - 1; level > 0; level--) {
            ...
            for (int i = 0; i < (int)nodes[level + 1].size(); i++) {
                ...
                for (int z = 0, j = 0; z < 2; z++) {
                    for (int y = 0; y < 2; y++) {
                        for (int x = 0; x < 2; x++, j++) {
                            nodes[level][fc + j].origPos = nodes[level + 1][i].origPos +
            ...
            for (int i = 0; i < numLights; i++) {
                dls::math::vec3i index;
                auto interp = gridIndex(
                    index, dls::math::vec3f(lightPos[i]) - corner, nodeCellSize);
                // find the node IDs
                int nodeIDs[8];
                index <<= level + 2;
                ...
                for (auto z = 0, j = 0; z < 2; z++) {
                    ...
                    for (auto y = 0; y < 2; y++) {
                        ...
                        for (auto x = 0; x < 2; x++, j++) {
                            ...
                            auto nid = hiz * highestGridRes.y * highestGridRes.x +
                                       hiy * highestGridRes.x + hix;

                            for (auto l = highestLevel - 1; l >= level; l--) {
                                auto ii = ((index.z >> l) & 4) | ((index.y >> (l + 1)) & 2) |
                                          ((index.x >> (l + 2)) & 1);
...
class OpGrilleEclairage final : public OperatriceCorps {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        hierarchie.Build(positions.donnees(),
                         intensites.donnees(),
...
class OpDeepScattering final : public OperatriceCorps {
  public:
    ...
    OpDeepScattering(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto k = 1; k <= 10; ++k) {
            ...
            descripteurs[k - 1].taille_voxel = desc.taille_voxel *
        ...
        auto mat = dls::math::mat3x3f(
            axe_x.x, axe_x.y, axe_x.z, axe_y.x, axe_y.y, axe_y.z, axe_z.x, axe_z.y, axe_z.z);
        ...
        for (auto k = 0; k < 10; ++k) {
            auto min = descripteurs[k].etendue.min *
                       static_cast<float>(descripteurs[k].taille_voxel);
            auto max = descripteurs[k].etendue.max *
                       static_cast<float>(descripteurs[k].taille_voxel);
...
{
    usine.enregistre_type(cree_desc(
        "Créer volume", "", "entreface/operatrice_creation_volume.jo", cree_volume, false));
    usine.enregistre_type(cree_desc("Maillage vers Volume",
                                    "",
                                    "entreface/operatrice_maillage_vers_volume.jo",
                                    maillage_vers_volume,
                                    false));
    usine.enregistre_type(cree_desc("Rastérisation Prim",
                                    "",
                                    "entreface/operatrice_rasterisation_prim.jo",
                                    ratisse_primitives,
                                    false));
    usine.enregistre_type(
        cree_desc("Rééchantillonne Volume", "", "", reechantillonne_volume, false));

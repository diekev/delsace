...
static fftw_plan cree_plan(int M,
                           int N,
...
auto spectre_phillips(Ocean &o, T kx, T kz)
{
    ...
    return static_cast<T>(o.amplitude) * std::exp(-1.0 / (k2 * static_cast<T>(o.L * o.L))) *
           std::exp(-k2 * static_cast<T>(o.l * o.l)) * std::pow(std::abs(tmp), o.alignement_vent) /
           (k2 * k2);
}
...
/* note that this doesn't wrap properly for i, j < 0, but its not really meant for that being just
 * a way to get the raw data out to save in some image format.
 */
static void evalue_ocean_ij(Ocean &oc, OceanResult *ocr, int i, int j)
{
    ...
    ocr->disp[1] = oc.calcul_deplacement_y ? static_cast<float>(oc.disp_y[i * oc.res_y + j]) :
                                             0.0f;
    ...
    if (oc.calcul_ecume) {
        compute_eigenstuff(ocr,
                           ...
                           static_cast<float>(oc.Jzz[i * oc.res_y + j]),
                           static_cast<float>(oc.Jxz[i * oc.res_y + j]));
    }
}

static void simule_ocean(
    Ocean &o, double t, double scale, double chop_x, double chop_z, double gravite)
{
    ...
    boucle_parallele(
        tbb::blocked_range<int>(0, o.res_x), [&](tbb::blocked_range<int> const &plage) {
            auto gna = GNA(static_cast<unsigned long>(o.graine + plage.begin()));

            for (int i = plage.begin(); i < plage.end(); ++i) {
                /* note the <= _N/2 here, see the fftw doco about the mechanics of the
                 * complex->real fft storage */
                for (int j = 0; j <= o.res_y / 2; ++j) {
                    ...
                    auto h0_minus = r1r2 *
                                    std::sqrt(spectre_phillips(o, -o.kx[i], -o.kz[j]) / 2.0);

                    auto exp_param1 = dls::math::complexe(0.0,
                                                          omega(k, o.profondeur, gravite) * t);
                    auto exp_param2 = dls::math::complexe(0.0,
                                                          -omega(k, o.profondeur, gravite) * t);
    ...
    tbb::parallel_for(0l, plans.taille(), [&](long i) { fftw_execute(plans[i]); });
...
static void initialise_donnees_ocean(Ocean &o, double gravite)
{
...
class OperatriceSimulationOcean final : public OperatriceCorps {
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto temps = evalue_decimal("temps", contexte.temps_courant) /
                     static_cast<float>(contexte.cadence);
        ...
        auto grille_depl = dynamic_cast<wlk::grille_dense_2d<dls::math::vec3f> *>(
            calque_depl.tampon());
        auto grille_norm = dynamic_cast<wlk::grille_dense_2d<dls::math::vec3f> *>(
            calque_norm.tampon());
        auto grille_ecume = dynamic_cast<wlk::grille_dense_2d<float> *>(
            m_ecume_precedente.tampon());
...
auto LinearInterpolation = [](auto fun) {
    ...
    return [=](auto x) mutable -> ValueType {
        ...
        return (wx != 0 ? wx * fun(ix + 1) : zero) + (wx != 1 ? (1 - wx) * fun(ix) : zero);
    };
};
...
auto InterpolationDimWise_impl(Fun fun, Interpolation... interpolation)
{
    ...
    if constexpr (I == sizeof...(interpolation)) {
        ...
        return InterpolationDimWise_impl<I + 1>(InterpolateNthArgument<I>(fun, interpol),
                                                interpolation...);
    }
}
...
auto InterpolationDimWise_impl2(Fun fun, Interpolation... interpolation)
{
    ...
    if constexpr (I == sizeof...(J)) {
        ...
        return InterpolationDimWise_impl2<I + 1, J...>(InterpolateNthArgument<K>(fun, interpol),
                                                       interpolation...);
    }
}
...
{
    return [=](auto fun) { return InterpolationDimWise_impl<0>(fun, interpolation...); };
}
...
{
    assert(i0 >= 0 && i0 < dimension(0) && i1 >= 0 && i1 < dimension(1) && i2 >= 0 &&
           i2 < dimension(2) && i3 >= 0 && i3 < dimension(3));
...
auto igrid = InterpolationDimWise(LinearInterpolation, LinearInterpolation)(data_grid);

auto igrid_dx = InterpolationDimWise(LinearInterpolation, LinearInterpolation)(dx_data_grid);

auto igrid_dy = InterpolationDimWise(LinearInterpolation, LinearInterpolation)(dy_data_grid);
...
bool Environment::inDomain(dls::math::vec2f pos) const
{
    return levelset(pos) >= 0;
}

float Environment::levelset(dls::math::vec2f pos) const
{
    return grid(pos, _dx);
}
...
class ProfileBuffer {
  public:
    ...
    void precompute(Spectrum &spectrum,
                    float time,
                    float zeta_min,
                    float zeta_max,
                    int resolution = 4096,
...
dls::math::vec4f ProfileBuffer::operator()(float p) const
{
    ...
    auto extended_buffer = [=](int i) -> dls::math::vec4f { return m_data[pos_modulo(i, N)]; };
...
dls::math::vec4f ProfileBuffer::gerstner_wave(float phase /*=knum*x*/, float knum) const
{
    float s = sin(phase);
    float c = cos(phase);
...
class WaveGrid {
   ...
   */
    struct Settings {
        ...
        enum SpectrumType { LinearBasis, PiersonMoskowitz } spectrumType = PiersonMoskowitz;
    };
...
std::pair<dls::math::vec3f, dls::math::vec3f> WaveGrid::waterSurface(dls::math::vec2f pos) const
{
    ...
    for (int izeta = 0; izeta < gridDim(Zeta); izeta++) {
        ...
        for (float a = 0; a < 1; a += da) {
            ...
            dls::math::vec4f wave_data = dx *
                                         amplitude(dls::math::vec4f{pos[X], pos[Y], angle, zeta}) *
                                         profile(kdir_x);

            surface += dls::math::vec3f{
                kdir[0] * wave_data[0], kdir[1] * wave_data[0], wave_data[1]};
...
auto WaveGrid::interpolatedAmplitude() const
{
    ...
    auto interpolated_grid = DomainInterpolation(interpolation, domain)(extended_grid);
        ...
        for (int iy = 0; iy < gridDim(Y); iy++) {
            ...
            for (int itheta = 0; itheta < gridDim(Theta); itheta++) {
                for (int izeta = 0; izeta < gridDim(Zeta); izeta++) {
                    ...
                    if (ls >= 4 * dx(X)) {
                        m_newAmplitude(
                            ix, iy, itheta, izeta) = (1 - gamma) * grid(ix, iy, itheta, izeta) +
                                                     gamma * 0.5 *
...
void WaveGrid::precomputeGroupSpeeds()
{
    ...
    for (int izeta = 0; izeta < gridDim(Zeta); izeta++) {
        ...
        m_groupSpeeds[izeta] = 3 /*the 3 should not be here !!!*/ * result[0] / result[1];
    }
}
...
{
    return dls::math::vec4f{idxToPos(idx[X], X),
                            idxToPos(idx[Y], Y),
                            idxToPos(idx[Theta], Theta),
                            idxToPos(idx[Zeta], Zeta)};
}
...
{
    return dls::math::vec4f{posToGrid(pos4[X], X),
                            posToGrid(pos4[Y], Y),
                            posToGrid(pos4[Theta], Theta),
                            posToGrid(pos4[Zeta], Zeta)};
}
...
{
    return Idx{posToIdx(pos4[X], X),
               posToIdx(pos4[Y], Y),
               posToIdx(pos4[Theta], Theta),
               posToIdx(pos4[Zeta], Zeta)};
}
...
float WaveGrid::waveNumber(int izeta) const
{
...
float WaveGrid::groupSpeed(int izeta) const
{
...
float WaveGrid::dx(int dim) const
{
    return m_dx[dim];
}
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto i = 0; i < points->taille(); ++i) {
            ...
            for (auto i = 0; i < 8; ++i) {
                ...
                auto pos4 = dls::math::vec4f(p.x / 50.0f,
                                             p.z / 50.0f,
                                             theta,
                                             m_grille->idxToPos(0, WaterWavelets::WaveGrid::Zeta));

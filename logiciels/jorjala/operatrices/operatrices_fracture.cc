...
class OpBooleensMaillage final : public OperatriceCorps {
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        std::cerr << "Il y a " << donnees_booleen.triangles_esect.taille()
                  << " triangles_esect.\n";
    ...
    void intersecte_tri_tri(DonneesBooleen &donnees_booleens, unsigned &A, unsigned &B)
    {
        ...
        // if an edge of each triangle is on the plane of the other
            ...
            float posA2_A, posB_A, posB2_A, posB_B2, posA_B, posB2_B, posB_A2, posB2_A2, posA2_B,
                posA2_B2;
            ...
            if (nAcnB2 == dls::math::vec3f(0.0f) && nA2cnB == dls::math::vec3f(0.0f) &&
                nAnB2 * nA2nB > 0)
                stop = true;
        ...
        switch (posBbin) {
            ...
            case 41:
                ...
                InterTriangleSegment(donnees_booleens, &inter[1]);
                break;
                // particular cases : one point on the plane, one point one one side and one point
                // on the other side
            case 6:
            case 9:
                inter[0].he = heB[2];
        ...
        if (IsSegment(inter)) {
            ...
            if (produit_scalaire(produit_croix(nA, nB),
                                 (InterPts[ptInter[1]] - InterPts[ptInter[0]])) *
                ...
                switch (m_BOOP) {
                    case UNION:
                        ...
                        if (edgeB != 3)
                            donnees_booleens.triangles_esect[fB2->label].CutList.ajoute(
                                ptInterInv);
                        break;
                    case INTER:
                        donnees_booleens.triangles_esect[fA->label].CutList.ajoute(ptInterInv);
                        if (edgeA != 3)
                            donnees_booleens.triangles_esect[fA2->label].CutList.ajoute(
                                ptInterInv);
                        donnees_booleens.triangles_esect[fB->label].CutList.ajoute(ptInter);
                        if (edgeB != 3)
                            donnees_booleens.triangles_esect[fB2->label].CutList.ajoute(ptInter);
                        break;
                    case MINUS:
                        donnees_booleens.triangles_esect[fA->label].CutList.ajoute(ptInter);
                        if (edgeA != 3)
    ...
    bool IsSegment(Info_Inter *inter)
    {
        ...
        if (inter[1].Id != 0xFFFFFFFF) {
            ...
            // (the intersection is a segment) (it is not possible that the two first points are the
            //same)
            if (point || id)
                return true;
            id = true;
    ...
    void Get_Segment(DonneesBooleen &donnees_booleens,
                     Info_Inter *inter,
                     dls::tableau<unsigned> &I)
    {
        for (unsigned int i = 0; i != 4; ++i) {
            // if the point have an Id
            if (inter[i].Id != 0xFFFFFFFF) {
                // the Id is stored if it is not already done
                if (I.taille() == 0 || I[0] != inter[i].Id)
                    I.ajoute(inter[i].Id);
            }
        ...
        switch (inter->res) {
            case 0:  // intersection on the facet
            {
                if (!inter->IsOnVertex) {
                    ...
                    do {
                        ...
                        donnees_booleens.triangles_esect[f->label].RefInter[H_circ->paire->label] =
                            I;
                        H_circ++;
                    } while (H_circ != H_end);
                }
            ...
            case 1:  // Intersection on the first halfedge of the facet
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    donnees_booleens.triangles_esect[f->label].RefInter[he->label] = I;
                    donnees_booleens.triangles_esect[f->label].RefInter[he->paire->label] = I;
                    donnees_booleens.triangles_esect[f->arete->paire->face->label]
                        .RefInter[he->label] = I;
                    donnees_booleens.triangles_esect[f->arete->paire->face->label]
                        .RefInter[he->paire->label] = I;
                    donnees_booleens.triangles_esect[he->face->label].RefInter[f->arete->label] =
                        I;
                    donnees_booleens.triangles_esect[he->face->label]
                        .RefInter[f->arete->paire->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->paire->label] = I;
                }
                else {
                    ...
                    do {
                        ...
                        donnees_booleens.triangles_esect[f->arete->paire->face->label]
                            .RefInter[H_circ->paire->label] = I;
                        donnees_booleens.triangles_esect[H_circ->face->label]
                            .RefInter[f->arete->label] = I;
                        donnees_booleens.triangles_esect[H_circ->face->label]
                            .RefInter[f->arete->paire->label] = I;
                        H_circ++;
            ...
            {
                donnees_booleens.triangles_esect[f->label].PtList.insere(I);
                donnees_booleens.triangles_esect[f->arete->suivante->paire->face->label]
                    .PtList.insere(I);
                if (!inter->IsOnVertex) {
                    donnees_booleens.triangles_esect[he->face->label].PtList.insere(I);
                    donnees_booleens.triangles_esect[he->paire->face->label].PtList.insere(I);
                    ...
                    donnees_booleens.triangles_esect[f->arete->suivante->paire->face->label]
                        .RefInter[he->paire->label] = I;
                    donnees_booleens.triangles_esect[he->face->label]
                        .RefInter[f->arete->suivante->label] = I;
                    donnees_booleens.triangles_esect[he->face->label]
                        .RefInter[f->arete->suivante->paire->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->suivante->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->suivante->paire->label] = I;
                     ...
                     // by the first halfedge)
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    do {
                        donnees_booleens.triangles_esect[H_circ->face->label].RefInter[he->label] =
                            I;
                        donnees_booleens.triangles_esect[H_circ->face->label]
                            .RefInter[he->paire->label] = I;
                        donnees_booleens.triangles_esect[he->face->label].RefInter[H_circ->label] =
                            I;
                        donnees_booleens.triangles_esect[he->face->label]
                            .RefInter[H_circ->paire->label] = I;
                        donnees_booleens.triangles_esect[he->paire->face->label]
                    ...
                    do {
                        ...
                        do {
                            donnees_booleens.triangles_esect[F_circ->face->label]
                                .RefInter[H_circ->label] = I;
                            donnees_booleens.triangles_esect[F_circ->face->label]
                                .RefInter[H_circ->paire->label] = I;
                            donnees_booleens.triangles_esect[H_circ->face->label]
                                .RefInter[F_circ->label] = I;
                            donnees_booleens.triangles_esect[H_circ->face->label]
                                .RefInter[F_circ->paire->label] = I;
                            F_circ++;
                        } while (F_circ != F_end);
                        H_circ++;
                    } while (H_circ != H_end);
            ...
            {
                donnees_booleens.triangles_esect[f->label].PtList.insere(I);
                donnees_booleens.triangles_esect[f->arete->suivante->suivante->paire->face->label]
                    .PtList.insere(I);
                if (!inter->IsOnVertex) {
                    donnees_booleens.triangles_esect[he->face->label].PtList.insere(I);
                    donnees_booleens.triangles_esect[he->paire->face->label].PtList.insere(I);
                    ...
                    donnees_booleens.triangles_esect[f->label].RefInter[he->paire->label] = I;
                    donnees_booleens
                        .triangles_esect[f->arete->suivante->suivante->paire->face->label]
                        .RefInter[he->label] = I;
                    donnees_booleens
                        .triangles_esect[f->arete->suivante->suivante->paire->face->label]
                        .RefInter[he->paire->label] = I;
                    donnees_booleens.triangles_esect[he->face->label]
                        .RefInter[f->arete->suivante->suivante->label] = I;
                    donnees_booleens.triangles_esect[he->face->label]
                        .RefInter[f->arete->suivante->suivante->paire->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->suivante->suivante->label] = I;
                    donnees_booleens.triangles_esect[he->paire->face->label]
                        .RefInter[f->arete->suivante->suivante->paire->label] = I;
                     ...
                     // by the third halfedge)
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    Halfedge_around_vertex_circulator
                        H_circ = f->arete->suivante->suivante->vertex_begin(),
                    ...
                    do {
                        Halfedge_around_vertex_circulator
                            F_circ = f->arete->suivante->suivante->vertex_begin(),
                     ...
                     // by the second halfedge)
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    do {
                        Halfedge_around_vertex_circulator
                            F_circ = f->arete->suivante->vertex_begin(),
    ...
    void add_facet_to_solution(DonneesBooleen &donnees_booleens,
                               mi_triangle *&pFacet,
                               bool facet_from_A)
    {
        ...
        if (pFacet->label < donnees_booleens.triangles_esect.taille()) {
            ...
            for (std::set<unsigned>::iterator i = TriCut.PtList.begin(); i != TriCut.PtList.end();
                 ++i) {
                T.add_new_pt(InterPts[*i], (unsigned long &)*i);  // MT: ajout cast
            }
            // get all the triangles of the triangulation
            dls::tableau<dls::tableau<unsigned long>> Tri_set = T.get_all_triangles(

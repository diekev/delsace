...
static Image const *cherche_image_profonde(OperatriceImage &op,
                                           int index,
...
static auto fusionne_echant_surposes(float a1, float c1, float a2, float c2, float &am, float &cm)
{
...
class OpAplanisProfonde final : public OperatriceImage {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        boucle_parallele(
            tbb::blocked_range<long>(0, largeur), [&](tbb::blocked_range<long> const &plage) {
                if (chef->interrompu()) {
                    return;
                }
                ...
                for (auto j = plage.begin(); j < plage.end(); ++j) {
                    for (auto i = 0; i < hauteur; ++i) {
                        ...
                        //					for (auto i = 1u; i < n; ++i) {
                        //						if (sZ[i] == sZ[i - 1]) {
                        //							sR[i] = fusionne_echant_surposes(sR[i], sA[i], sR[i - 1],
                        //sA[i - 1], sR[i], sA[i])
                        //						}
                        //					}
                        ...
                        for (auto e = 1u; e < n; ++e) {
                            ...
                            pixel.r = (pixel.r * pixel.a + sR[e] * sA[e] * (1.0f - pixel.a)) *
                                      denum;
                            pixel.v = (pixel.v * pixel.a + sG[e] * sA[e] * (1.0f - pixel.a)) *
                                      denum;
                            pixel.b = (pixel.b * pixel.a + sB[e] * sA[e] * (1.0f - pixel.a)) *
                                      denum;
...
class OpFusionProfonde final : public OperatriceImage {
  public:
    ...
    OpFusionProfonde(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(1);
    }
  ...
  public:
    ...
    void performe_versionnage() override
    {
        if (propriete("nom_caméra") == nullptr) {
            ajoute_propriete(
                "nom_caméra", danjo::TypePropriete::CHAINE_CARACTERE, dls::chaine(""));
        }

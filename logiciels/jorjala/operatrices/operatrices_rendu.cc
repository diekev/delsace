...
/**
 * Instançage (possibilité d'avoir une seule représentation d'un objet en mémoire et de le rendre
 * plusieurs fois) (Arnold) pouvoir redéfinir les attributs non-géométrique pour chaque instance
 * (UV, couleurs, nuanceurs, etc.) (Arnold) pouvoir avoir une instance d'un objet, au lieu de
 * multiples (il suffit d'un seul point) (Houdini) l'instance se fait dans les objets avec un objet
 * instance spéciale (Blender) l'instance se fait soit via particules, soit via un parentage sur
 * les points, arêtes, ou primitives (Général) avoir un index par instance
 */
class OpRenduInstance : public OperatriceImage {
  public:
    ...
    OpRenduInstance(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(1);
        m_execute_toujours = true;
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        points_instance->objet->donnees.accede_lecture([&](DonneesObjet const *donnees) {
            ...
            for (int i = 0; i < nombre_points; ++i) {
                ...
                auto transforme = math::construit_transformation(pnt, rot_locale, ech_locale);
...
class OpMoteurRendu : public OperatriceImage {
  ...
  public:
    ...
    OpMoteurRendu(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
        sorties(0);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        m_moteur_rendu->calcule_rendu(
            *stats, tampon, camera->hauteur(), camera->largeur(), contexte.rendu_final);

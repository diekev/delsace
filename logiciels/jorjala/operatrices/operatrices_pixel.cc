...
static Image const *cherche_image(OperatriceImage &op,
                                  int index,
                                  ...
                                  int index_lien = 0)
{
    ...
    if (image == nullptr) {
        op.ajoute_avertissement(
            "Aucune image trouvée dans l'entrée à l'index ", index, ", lien ", index_lien, " !");
...
class OperatriceBruitage final : public OperatricePixel {
  ...
  public:
    ...
    dls::phys::couleur32 evalue_pixel(dls::phys::couleur32 const &pixel,
                                      const float x,
...
class OperatriceEtalonnage final : public OperatricePixel {
  ...
  public:
    ...
    static constexpr auto AIDE =
        "Étalonne l'image au moyen d'une rampe linéaire et d'une fonction gamma.";
...
static auto mappage_ton_lottes(float x)
{
    ...
    // Can be precomputed
    const float b = (-std::pow(midIn, a) + std::pow(hdrMax, a) * midOut) /
                    ((std::pow(hdrMax, a * d) - std::pow(midIn, a * d)) * midOut);
    const float c = (std::pow(hdrMax, a * d) * std::pow(midIn, a) -
...
class OperatricePremultiplication final : public OperatricePixel {
  ...
  public:
    ...
    static constexpr auto AIDE =
        "Prémultiplie les couleurs des pixels par leurs valeurs alpha respectives.";
...
class OperatriceMinMax final : public OperatricePixel {
  ...
  public:
    ...
    OperatriceMinMax(Graphe &graphe_parent, Noeud &noeud_) : OperatricePixel(graphe_parent, noeud_)
    {
        entrees(1);
    }

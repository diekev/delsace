...
static auto calcul_gradient(dls::tableau<float> const &gradientblock,
    ...
    for (auto j = 0; j < taille_gradient_block; ++j) {
        for (auto i = 0; i < taille_gradient_block; ++i) {
            ...
            if (j == 0) {
                ...
                gy[index] = gradientblock[index + taille_gradient_block] -
                            gradientblock[index - taille_gradient_block];
            }
        }
    }
}

static auto calcule_cle_empreinte(dls::tableau<float> const &gradientblock,
                                  int taille_gradient_block,
                                  float Qangle,
...
class OpTestSRIRP final : public OperatriceImage {
  public:
    ...
    OpTestSRIRP(Graphe &graphe_parent, Noeud &noeud_) : OperatriceImage(graphe_parent, noeud_)
    {
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto gradientmargin = static_cast<int>(
            std::floor(static_cast<float>(gradientsize) * 0.5f));

        auto idxQ = otl::cree_indexeuse_statique(
            Qangle, Qstrength, Qcoherence, R * R, patchsize * patchsize, patchsize * patchsize);
        auto idxV = otl::cree_indexeuse_statique(
            Qangle, Qstrength, Qcoherence, R * R, patchsize * patchsize);
            ...
            for (auto x = margin; x < desc.resolution.x - margin; ++x) {
                ...
                for (auto yp = y - patchmargin; yp < y + patchmargin; ++yp) {
                    for (auto xp = x - patchmargin; xp < x + patchmargin; ++xp) {
                        patch[index] = tampon_nb.valeur(
                            dls::math::vec2i(xp, yp));  // tampon upsaled
                    }
                }
                ...
                for (auto yp = y - patchmargin; yp < y + patchmargin; ++yp) {
                    for (auto xp = x - patchmargin; xp < x + patchmargin; ++xp) {
                        gradientblock[index] = tampon_nb.valeur(
                            dls::math::vec2i(xp, yp));  // tampon upsaled
                    }
                }
                ...
                float strength;
                float coherence;
                calcule_cle_empreinte(
                    gradientblock, gradientmargin, Qangle, poids, angle, strength, coherence);

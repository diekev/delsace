...
struct MTreeNode {
    ...
    int creator = 0;
    /* when growing nodes it is useful to know how much they should grow. this parameter gives the
     * length a node has grown since grow was called */
    float growth = 0.0f;
    ...
    MTreeNode(dls::math::vec3f const &p, dls::math::vec3f const &d, float r, int c = 0)
        : position(p), direction(d), radius(r), creator(c)
    {
    }
    ...
    float set_positions_in_branches(float current_distance = 0, float distance_from_parent = 0)
    {
        ...
        current_distance +=
            distance_from_parent;  //# increase current distance by the distance from node parent

        if (this->children.est_vide()) {  // if node is the end of the branch, return the length of
                                          // the branch and set the position_in_branch to 1
            this->position_in_branch = 1.0f;
            return current_distance;
        }
        for (auto i = 1; i < this->children.taille();
             ++i) {  // recursivelly call the function on all side children
            this->children[i]->set_positions_in_branches(
                0, 0);  // the current_distance of a side child is 0 since it is the begining of a
                        // new branch
        }

        auto distance_to_child = longueur(this->position - this->children[0]->position);
        auto branch_length = this->children[0]->set_positions_in_branches(current_distance,
                                                                          distance_to_child);

        this->position_in_branch = (branch_length == 0.0f) ? 0.0f :
                                                             current_distance / branch_length;
        return branch_length;
    }

    void get_split_candidates(dls::tableau<MTreeNode *> &candidates,
                              int creator_,
                              float start,
                              float end)
    {
        if ((this->children.taille() == 1) && (!this->is_branch_origin) &&
            (this->creator == creator_) && (end >= this->position_in_branch) &&
            (this->position_in_branch >= start)) {
            if (end <= start) {
                this->position_in_branch = 0.0f;
            }
            else {
                this->position_in_branch =
                    (this->position_in_branch - start) /
                    (end -
                     start);  // transform the position in branch so that a position at offset is 0
            }
    ...
    {
        /* recursively populates a list with position, direction radius of all modules susceptible
         * to create a leaf*/
        if ((this->radius <= max_radius) && (this->can_spawn_leaf)) {
            auto extremity = this->children.est_vide();
            auto direction_ = extremity ? this->direction :
                                          (this->children[0]->position - this->position);
            ...
            // XXX
            // candidates.ajoute(memoire::loge<MTreeNode>("MTreeNode", this->position, direction,
            // length, this->radius, extremity))
        }
    ...
    void get_branches(dls::tableau<dls::tableau<dls::math::vec4f>> &positions,
                      ...
                      dls::math::vec3f *parent_pos = nullptr)
    {
        ...
        positions.back().ajoute(
            dls::math::vec4f(pos.x, pos.y, pos.z, 0.0f));  // # add position to last branch
    ...
    void get_armature_data(float min_radius,
                           int *bone_index,
                           dls::tableau<dls::tableau<MTreeNode *>> &armature_data,
                           int parent_index)
    {
        /* armature data is list of list of (position_head, position_tail, radius_head,
         * radius_tail, parent bone index) of each node. bone_index is a list of one int*/
        auto index = 0;

        if (this->radius > min_radius &&
            !this->children
                 .est_vide()) {  // if radius is greater than max radius, add data to armature data
                                 // auto child = this->children[0];
            // armature_data.back().ajoute(memoire::loge<MTreeNode>("MTreeNode", this->position,
            // child.position, this->radius, child->radius, parent_index));
    ...
    {
        /* used when creating tree from grease pencil, rescales the radius of each branch according
         * to its parent radius */
...
{
    return normalise(dls::math::vec3f(gna.uniforme(0.0f, 1.0f) - 0.5f,
...
auto add_candidates(dls::tableau<CandidatFeuille> &leaf_candidates, int dupli_number)
{
    ...
    for (auto candidat : leaf_candidates) {
        ...
        for (auto i = 0; i < dupli_number; ++i) {
            auto pos = candidat.position + candidat.direction * candidat.length *
                                               (static_cast<float>(i) + 1.0f) /
                                               (static_cast<float>(dupli_number) + 2.0f);
            new_candidates.ajoute(
                {pos, candidat.direction, candidat.length, candidat.radius, candidat.is_end});
        }
    }
...
struct MTree {
    ...
    void add_trunk(float length,
                   float radius,
                   float end_radius,
                   float shape,
                   float resolution,
                   float randomness,
                   float axis_attraction,
                   int creator)
    {
        ...
        while (remaining_length > 0) {
            if (remaining_length < 1.0f / resolution) {
                resolution = 1.0f / remaining_length;  // last last branch is shorter so that the
                                                       // trunk is exactly of required length
            }

            auto tangent = random_tangent(extremity->direction);
            auto direction = extremity->direction +
                             tangent * randomness / resolution;  // direction of new TreeNode

            auto course_correction = dls::math::vec3f(
                -extremity->position.x, -extremity->position.y, 1.0f / direction.z);
            direction += course_correction * axis_attraction;
            direction = normalise(direction);
            auto position = extremity->position +
                            extremity->direction / resolution;  // position of new TreeNode
            auto rad = radius * std::pow(remaining_length / length, shape) +
                       (1.0f - remaining_length / length) * end_radius;  // radius of new TreeNode
            auto new_node = memoire::loge<MTreeNode>(
                "MTreeNode", position, direction, rad, creator);  // new TreeNode
              ...
              int selection)
    {
        ...
        this->stem->get_grow_candidates(grow_candidates,
                                        selection);  // get all leafs of valid creator

        auto branch_length =
            1.0f / resolution;  // branch length is use multiple times so best to calculate it once

        auto shape_length = [&](float x) {
            /* returns y=f(x) so that f(0)=shape_start, f(1)=shape_end and f(0.5) =
             * shape_convexity+1/2(shape_start+shape_end) */
            return -4 * shape_convexity * x * (x - 1) + x * shape_end + (1 - x) * shape_start;
        };

        for (auto node : grow_candidates) {
            node->growth = 0;
            node->growth_goal = std::max(
                0.001f,
                length *
                    shape_length(node->position_in_branch));  // add length to node growth goal
            node->growth_radius = node->radius;
        }

        // grow_candidates = deque(grow_candidates); // convert grow_candidates to deque for
        // performance (lots of adding/removing last element)

        while (grow_candidates.taille() > 0) {  // grow all candidates until there are none (all
                                                // have grown to their respective length)
            auto node = grow_candidates[0];

            auto children_number = ((gna.uniforme(0.0f, 1.0f) > split_proba) ||
                                    node->is_branch_origin) ?
                                       1 :
                                       2;  // if 1 the branch grows normally, if more than 1 the
                                           // branch forks into more branches
            ...
            for (auto i = 0; i < children_number; ++i) {
                auto deviation =
                    children_number == 1 ?
                        randomness :
                        split_angle;  // how much the new direction will be changed by tangent
                auto direction = lerp(node->direction,
                                      tangent * (static_cast<float>(i) - 0.5f) * 2.0f,
                                      deviation);  // direction of new node
                direction += dls::math::vec3f(0.0f, 0.0f, -1.0f) * gravity_strength / 10.0f /
                             resolution;  // apply gravity

                if (floor_avoidance != 0.0f) {
                    auto below_ground = floor_avoidance < 0 ?
                                            -1.0f :
                                            1.0f;  // if -1 then the branches must stay below
                                                   // ground, if 1 they must stay above ground
                    auto distance_from_floor = std::max(0.01f, std::abs(node->position.z));
                    auto direction_toward_ground = std::max(
                        0.0f,
                        -direction.z *
                            below_ground);  // get how much the branch is going towards the floor
                    auto floor_avoidance_strength = direction_toward_ground * 0.3f /
                                                    distance_from_floor * floor_avoidance;

                    if (floor_avoidance_strength >
                        0.1f * (1.0f + floor_avoidance)) {  // if the branch is too much towards
                                                            // the floor, break it
                        break;
                    }
                ...
                if (i == 0) {
                    position = node->position +
                               direction * branch_length;  // # position of new node
                }
                else {
                    auto t = normalise(tangent - projette(tangent, node->direction));
                    position = (node->position + node->children[0]->position) / 2.0f +
                               t * node->radius;
                }

                auto growth = std::min(node->growth_goal,
                                       node->growth + branch_length);  // growth of new node

                auto radius = node->growth_radius * ((1.0f - node->growth / node->growth_goal) +
                                                     end_radius * node->growth /
                                                         node->growth_goal);  // radius of new node
                if (i > 0) {
                    radius *= split_radius;  // # forked branches have smaller radii
                }

                auto child = memoire::loge<MTreeNode>(
                    "MTreeNode", position, direction, radius, creator);
                child->growth_goal = node->growth_goal;
                child->growth = growth;
                child->growth_radius = (i == 0) ? node->growth_radius :
                                                  node->growth_radius * split_radius;
                ...
                if (growth < node->growth_goal) {
                    grow_candidates.ajoute(
                        child);  // if child can still grow, add it to the grow candidates
                }
            }
        }
    }

    void split(int amount,
               float angle,
               ...
               int selection)
    {
        ...
        for (auto &node : split_candidates) {
            ...
            tangent = normalise(tangent);
            auto rot = dls::math::quaternion<float>(
                node->direction, constantes<float>::TAU / static_cast<float>(n_children));

            for (auto i = 0; i < n_children; ++i) {
                auto t = node->position_in_branch;
                auto direction = normalise(
                    lerp(node->direction, tangent, angle * (1.0f - t / 2.0f)));
                ...
                auto child = memoire::loge<MTreeNode>(
                    "MTreeNode", position, direction, rad, creator);
                      ...
                      float selection)
    {
        ...
        this->split(amount,
                    ...
                    1,
                    split_flatten,
                    static_cast<int>(split_creator),
                    static_cast<int>(split_selection));
        this->grow(length,
                   shape_start,
                   shape_end,
                   shape_convexity,
                   ...
                   floor_avoidance,
                   can_spawn_leaf,
                   static_cast<int>(grow_creator),
                   static_cast<int>(grow_selection));
    }

    void roots(float length, float resolution, float split_proba, float randomness, int creator)
    {
        if (this->stem->children
                .est_vide()) {  // roots can only be added on a trunk on non 0 length
            return;
        }

        auto roots_origin = memoire::loge<MTreeNode>(
            "MTreeNode", this->stem->position, -this->stem->direction, this->stem->radius, -1);
        roots_origin->is_branch_origin = true;
        this->stem->children.ajoute(
            roots_origin);  // stem is set as branch origin, so it cannot be splitted by split
                            // function. second children of stem will then always be root origin
    ...
    //				auto factor = number / len([i for i in leaf_candidates if not i[-1]]); // remove
    //extremities from factor because they won't participate in candidate addition
              ...
              float flatten)
    {
        ...
        this->grow(1,
                   ...
                   1,
                   0);
        this->add_branches(branch_number,
                           0.5f,
                           2,
                           0.7f,
                           0.1f,
...
class OpAjouteBranch final : public OperatriceCorps {
  public:
    ...
    OpAjouteBranch(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
    }
...
class OpGrow final : public OperatriceCorps {
  public:
    ...
    OpGrow(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
    }
...
res_exec OperatriceCreationArbre::execute(const ContexteEvaluation &contexte,
                                          DonneesAval *donnees_aval)
{
    ...
    m_arbre->add_trunk(
        length, radius, end_radius, shape, resolution, randomness, axis_attraction, 0);
...
void axis_angle_normalized_to_quat(dls::math::quaternion<float> &q,
                                   const dls::math::vec3f &axis,
                                   const float angle)
{
...
{
    if ((fac <= -1.0f))
        return -constantes<float>::PI / 2.0f;
    else if ((fac >= 1.0f))
        return constantes<float>::PI / 2.0f;
    else
        return asinf(fac);
...
void axis_angle_to_quat(dls::math::quaternion<float> &q,
                        const dls::math::vec3f &axis,
                        const float angle)
{
    dls::math::vec3f nor;
...
void rotation_between_vecs_to_quat(dls::math::quaternion<float> &q,
...
struct stemSpline {
    ...
    stemSpline(Spline *spline,
               ...
               int splineNum,
               float ofst,
               dls::math::quaternion<float> pquat)
    {
        this->spline = spline;
    ...
    dls::math::quaternion<float> quat()
    {
        if (this->spline->bezier_points.taille() == 1) {
            return to_track_quat(normalise(this->spline->bezier_points.back()->handle_right -
                                           this->spline->bezier_points.back()->co),
                                 2,
                                 1);
        }

        return to_track_quat(
            normalise(this->spline->bezier_points.back()->co -
                      this->spline->bezier_points[this->spline->bezier_points.taille() - 2]->co),
            2,
            1);
    }
...
void kickstart_trunk(dls::tableau<stemSpline> &addstem,
                     int levels,
                     int leaves,
                     ...
                     float rootFlare)
{
    ...
    auto startRad =
        scaleVal * ratio *
        scale0 /* * uniform(1 - scaleV0, 1 + scaleV0)*/;  // * (scale0 + uniform(-scaleV0,
                                                          // scaleV0));
    ...
    auto stem = stemSpline(newSpline,
                           curveVal,
                           curveV[0] / curveRes[0],
                           attractUp[0],
                           0,
...
struct childPoint {
    ...
    childPoint(dls::math::vec3f const &coords,
               ...
               double _lengthPar,
               int _parBone)
        : co(coords), quat(quaternion), radiusPar(_radiusPar), offset(_offset), stemOffset(_sOfst),
          lengthPar(_lengthPar), parBone(_parBone)
    {
    }
};

double shapeRatio(int shape,
                  double ratio,
                  double pruneWidthPeak = 0.0,
                  double prunePowerHigh = 0.0,
                  double prunePowerLow = 0.0,
                  double *custom = nullptr)
{
    if (shape == 0) {
...
void fabricate_stems(double addsplinetobone,
                     ...
                     double boneStep)
{
    ...
    if ((n == 1) && (rMode != "original")) {
        ...
        auto childP_T = dls::tableau<dls::tableau<childPoint>>{};  //[childP_T[k] for k in
                                                                   //sorted(childP_T.keys())];
                ...
                for (auto &a  { p)
                    {
                        ...
                        auto d = std::min(
                            std::fmod(a1 - a2 + constantes<double>::TAU, constantes<double>::TAU),
                            std::fmod(a2 - a1 + constantes<double>::TAU, constantes<double>::TAU));
                        a_diff.ajoute(d);
                    }
                    ...
                    auto bL =
                        br.lengthPar * length[1] *
                        shapeRatio(
                            shape, (1 - br.offset) / (1 - baseSize), 0.0, 0.0, 0.0, customShape);
                    ...
                    if (downAngleV[1] > 0) {
                        downA = downAngle[n] +
                                std::pow(-downAngleV[n] * (1 - (1 - br.offset) / (1 - baseSize)),
                                         2.0);
                    }
                    else {
                        downA = downAngle[n];
                    ...
                    if (p[0].offset > fillHeight) { // prob_b and (len(p) > 1) {  ##(p[0].offset >
                    fillHeight) and childP.append(p[randint(0, len(p)-1)]) rot_a.append(bRotate)//
                    + pi)
                    */
                    childP.ajoute(p[idx]);
                    rot_a.ajoute(a);
            ...
            for (auto const &p : childP) {
                ...
                if (useOldDownAngle) {
                    ...
                    if (downAngleV[n] < 0.0) {
                        ...
                        downV = -downAngleV[n] * std::pow((1 - (1 - p.offset) / (1 - baseSize)),
                                                          2);  //  // (110, 80) = (60, -50);
                    }
                }
                ...
                if (n == 1) {
                    lShape = shapeRatio(
                        shape, (1 - p.stemOffset) / (1 - baseSize), 0.0, 0.0, 0.0, customShape);
                }
                else {
                ...
                // If this is the last level before leaves then we need to generate the child
                // points differently
                if (storeN == levels - 1) {
                    if (leaves < 0) {
                        childStems = false;
                        ...
                        childStems = leaves * (0.1 + 0.9 * (branchL / maxbL)) *
                                     shapeRatio(leafDist, (1 - p.offset));
                    }
                }

                // print("n=%d, levels=%d, n'=%d, childStems=%s"%(n, levels, storeN, childStems))

                // Determine the starting and ending radii of the stem using the tapering of the
                // stem
                auto startRad = std::min(
                    p.radiusPar[0] *
                        (std::pow((branchL / p.lengthPar), ratioPower) * radiusTweak[n]),
                    p.radiusPar[1]);
                ...
                // Add the new stem to list of stems to grow and define which bone it will be
                // parented to
                auto stem = stemSpline(newSpline,
                                       curveVal,
                                       curveVar,
        ...
        void perform_pruning(double baseSize,
                             double baseSplits,
                             double childP,
                             Curve &cu,
                             double currentMax,
                             ...
                             double rotateV)
        {
            ...
            while (startPrune && ((currentMax - currentMin) > 0.005)) {
                ...
                splineList[0].segL = stemsegL;       // À FAIRE * uniform(1 - lengthV[n], 1 +
                                                     // lengthV[n]);  // variation for first stem

                // For each of the segments of the stem which must be grown we have to add to each
                // spline in splineList
                for (auto k = 0; k < curveRes[n]; ++k) {
                    // Make a copy of the current list to avoid continually adding to the list
                    ...
                    // For each of the splines in this list set the number of splits and then grow
                    // it
                    for (auto spl : tempList) {
                        ...
                        else if ((n == 0) && (k == int((curveRes[n] - 1) * splitHeight) + 1) &&
                                 (splitVal > 0)) {
                            numSplit = 1;
                        }
                        else {
                        ...
                        // If pruning is enabled then we must to the check to see if the end of the
                        // spline is within the evelope
                        if (prune) {
                            // Check each endpoint to see if it is inside
                            for (auto &s : splineList) {
                                coordMag = (s.spline.bezier_points[-1].co.xy).length;
                                ratio = (scaleVal - s.spline.bezier_points[-1].co.z) /
                                        (scaleVal * max(1 - pruneBase, 1e-6));
                                // Don't think this if part is needed
                                if (n == 0)
                                    and(s.spline.bezier_points[-1].co.z < pruneBase * scaleVal)
                                    {
                                        insideBool =
                                            true;  // Init to avoid UnboundLocalError later
                                    }
                                else {
                                    insideBool = ((coordMag / scaleVal) <
                                                  pruneWidth * shapeRatio(9,
                                                                          ratio,
                                                                          pruneWidthPeak,
                                                                          prunePowerHigh,
                                                                          prunePowerLow));
                                }
                                // If the point is not inside then we adjust the scale and current
                                // search bounds
                                if (!insideBool) {
                                    oldMax = currentMax;
                            ...
                            // we need to make sure it won't be pruned or extended to the edge of
                            // the envelope
                            ...
                            // to make sure we sprout child points to grow the next splines or
                            // leaves
                            if ((((currentMax - currentMin) < 0.005) || !prune) || forceSprout) {
                                ...
                                // print("debug tvals[%d] , splineList[%d], %s" % ( len(tVals),
                                // len(splineList), st.children)) If leaves is -ve then we need to
                                // make sure the only point which sprouts is the end of the spline
                                if
                                    not st.children
                                    {
                                        tVals = [1.0];
                ...
                tVals = [t * (1 - baseSize) + baseSize for t in tVals];
                                ...
                                // For all the splines, we interpolate them and add the new points
                                // to the list of child points
            ...
            for
                s in splineList
                {
                    ...
                    childP.extend(
                        interpStem(s, tVals, s.segMax * s.segL, s.radS, maxOffset, baseSize));
                }
            ...
            if (!prune) {
                startPrune = false;
                        ...
                        class OperatriceCreationArbre final {
                          public
                            OperatriceCorps
                            {
                              public
                                {
                                    ...
                                    OperatriceCreationArbre(Graphe & graphe_parent,
                                                            Noeud & noeud_){
                                        OperatriceCorps(graphe_parent, noeud_){entrees(0);
                                    sorties(1);
                                }
...
#    else
                                    ...
                                    for (auto n = 0; n < params.Levels; ++n) {
                                        ...
                                        // If n is used as an index to access parameters for the
                                        // tree it must be at most 3 or it will reference outside
                                        // the array index
                                        n = std::min(3, n);
                                        auto splitError = 0.0;
                                        ...
                                        // If this is the first level of growth (the trunk) then we
                                        // need some special work to begin the tree
                                        if (n == 0) {
                                            kickstart_trunk(stemList,
                                                            levels,
                                                            leaves,
                                                            branches,
                                                            cu,
                                        ...
                                        // If this isn't the trunk then we may have multiple stem
                                        // to intialise
                                        else {
                                            // For each of the points defined in the list of stem
                                            // starting points we need to grow a stem.
                                            fabricate_stems(addsplinetobone,
                                                            stemList,
                                                            baseSize,
                                                            branches,
                                                            childP,
                                        ...
                                        // Now grow each of the stems in the list of those to be
                                        // extended
                                        for (auto &st : stemList) {
                                            // When using pruning, we need to ensure that the
                                            // random effects will be the same for each iteration
                                            // to make sure the problem is linear
                                            auto randState = 1.0f;  // getstate();
                                            auto startPrune = true;
                                            auto lengthTest = 0.0;
                                            // Store all the original values for the stem to make
                                            // sure we have access after it has been modified by
                                            // pruning
                                            ...
                                            // Now do the iterative pruning, this uses a binary
                                            // search and halts once the difference between upper
                                            // and lower bounds of the search are less than 0.005
                                            ratio,
                                                splineToBone = perform_pruning(baseSize,
                                                                               baseSplits,
                                                                               childP,
                                                                               cu,
                                                                               currentMax,

...
struct decoupeuse_limites<dls::math::vec3f> {
    ...
    explicit decoupeuse_limites(limites3f lmt)
    {
        ...
        limites[1] = limites3f(dls::math::vec3f(centre.x, min.y, min.z),
                               dls::math::vec3f(max.x, centre.y, centre.z));
        limites[2] = limites3f(dls::math::vec3f(centre.x, min.y, centre.z),
                               dls::math::vec3f(max.x, centre.y, max.z));
        limites[3] = limites3f(dls::math::vec3f(min.x, min.y, centre.z),
                               dls::math::vec3f(centre.x, centre.y, max.z));
        limites[4] = limites3f(dls::math::vec3f(min.x, centre.y, min.z),
                               dls::math::vec3f(centre.x, max.y, centre.z));
        limites[5] = limites3f(dls::math::vec3f(centre.x, centre.y, min.z),
                               dls::math::vec3f(max.x, max.y, centre.z));
        limites[6] = limites3f(centre, max);
        limites[7] = limites3f(dls::math::vec3f(min.x, centre.y, centre.z),
                               dls::math::vec3f(centre.x, max.y, max.z));
    }
};
...
struct arbre_Naire {
  ...
  private:
    arbre_Naire(type_limites const &limites, int profondeur_max) : m_profondeur_max(profondeur_max)
    {

...
class OpVentTerrain final : public OperatriceCorps {
  public:
    ...
    OpVentTerrain(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        ...
        for (auto r = 0; r < repetitions; ++r) {
            ...
            for (auto y = 0; y < desc.resolution.y; ++y) {
                for (auto x = 0; x < desc.resolution.x; ++x, ++index) {
                    ...
                    auto high = wlk::echantillonne_lineaire(
                        temp, pos_monde.x + s, pos_monde.y + t);

                    auto normal = calcul_normal(temp, pos_monde);
                    auto facteur =
                        normal.y;  // produit_scalaire(normal, dls::math::vec3f(0.0f, 1.0f, 0.0f);
                    auto hauteur = dls::math::entrepolation_lineaire(h, low, facteur * force2);
                    hauteur += dls::math::entrepolation_lineaire(
                        h, high, std::acos(facteur) * force);
                    hauteur *= 0.5f;

                    if (grille_poids != nullptr) {
                        auto poids = grille_poids->valeur(index);
                        hauteur = dls::math::entrepolation_lineaire(
                            hauteur, h, dls::math::restreint(poids, 0.0f, 1.0f));
                    }
...
class OpErosionTerrain final : public OperatriceCorps {
  public:
    ...
    OpErosionTerrain(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(2);
    }
    ...
    void erode_simple(wlk::grille_dense_2d<float> &grille_entree,
                ...
                for (auto x = 0; x < desc.resolution.x; ++x, ++index) {
                    ...
                    if (grille_poids != nullptr) {
                        ...
                        result = dls::math::entrepolation_lineaire(
                            result, centre, dls::math::restreint(poids, 0.0f, 1.0f));
                    }
    ...
    void erode_complexe(wlk::grille_dense_2d<float> &grille_entree, int iterations)
    {
...
class OpInclineTerrain final : public OperatriceCorps {
  public:
    ...
    OpInclineTerrain(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        m_execute_toujours = true;
        entrees(1);
    }
...
class OpEvalueTerrain final : public OperatriceCorps {
  public:
    ...
    OpEvalueTerrain(Graphe &graphe_parent, Noeud &noeud_) : OperatriceCorps(graphe_parent, noeud_)
    {
        entrees(1);
    }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto y = 0; y < desc.resolution.y; ++y) {
            for (auto x = 0; x < desc.resolution.x; ++x, ++index) {
                ...
                auto idx_pnt = points.ajoute_point(
                    (pos.x - 0.5f) * taille_x, h, (pos.y - 0.5f) * taille_y);

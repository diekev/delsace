...
static auto cherche_index_voisins(Corps const &corps)
{
    ...
    pour_chaque_polygone_ferme(corps, [&](Corps const &corps_entree, Polygone *poly) {
...
static auto cherche_index_adjacents(Corps const &corps)
{
    ...
    pour_chaque_polygone_ferme(corps, [&](Corps const &corps_entree, Polygone *polygone) {
        INUTILISE(corps_entree);
...
static auto cherche_index_bordures(dls::tableau<dls::ensemble<long>> const &voisins,
...
static auto calcule_lissage_normal(AccesseusePointEcriture const &points_entree,
...
static auto calcule_lissage_pondere(AccesseusePointEcriture const &points_entree,
                                    dls::tableau<char> const &bordures,
                                    dls::tableau<dls::ensemble<long>> const &voisins,
                                    dls::tableau<dls::math::vec3f> &deplacement,
                                    bool preserve_bordures)
...
static auto applique_lissage(AccesseusePointEcriture &points_entree,
                             Attribut *attr_N,
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto k = 0; k < iterations; ++k) {
            if (pondere_distance) {
                calcule_lissage_pondere(
                    points_entree, bordures, voisins, deplacement, preserve_bordures);
            }
            else {
                calcule_lissage_normal(
                    points_entree, bordures, voisins, deplacement, preserve_bordures);
            }
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto const &attr : corps_entree->attributs()) {
            ...
            auto nattr = m_corps.ajoute_attribut(
                attr.nom(), attr.type(), attr.dimensions, attr.portee, true);
            *nattr = attr;
        }
        ...
        auto transferante = TransferanteAttribut(
            *corps_entree, m_corps, TRANSFERE_ATTR_PRIMS | TRANSFERE_ATTR_SOMMETS);
...
class OperatriceNormaliseCovariance final : public OperatriceCorps {
  public:
    ...
    static constexpr auto AIDE =
        "Modifie les points pour que la covariance moyenne soit égale à 1.";
...
class OperatriceAligneCovariance final : public OperatriceCorps {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        auto mat = dls::math::mat3x3f(premier.x,
                                      premier.y,
                                      premier.z,
                                      second.x,
                                      second.y,
...
class OperatriceErosionMaillage final : public OperatriceCorps {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        for (auto i = 0; i < iterations; ++i) {
            ...
            for (auto f : polyedre.faces) {
                ...
                do {
                    ...
                    if (paire == nullptr ||
                        dls::outils::possede_drapeau(paire->drapeaux, mi_drapeau::SUPPRIME)) {
        ...
        auto transferante = TransferanteAttribut(*corps_entree,
                                                 m_corps,
                                                 TRANSFERE_ATTR_POINTS | TRANSFERE_ATTR_PRIMS |
...
static auto calcul_barycentre_poly(Corps &corps)
{
    ...
    auto barycentres = corps.ajoute_attribut(
        "barycentre", type_attribut::R32, 3, portee_attr::PRIMITIVE);
...
static auto calcul_centroide_poly(Corps &corps)
{
    ...
    auto centroides = corps.ajoute_attribut(
        "centroide", type_attribut::R32, 3, portee_attr::PRIMITIVE);
...
{
    auto tangeantes = corps.ajoute_attribut(
        "tangeantes", type_attribut::R32, 3, portee_attr::POINT);
    auto points = corps.points_pour_lecture();
...
static auto calcul_angle_diedre(Corps &corps)
{
    ...
    for (auto face : polyedre.faces) {
        ...
        do {
            if (debut->paire != nullptr &&
                !dls::outils::possede_drapeau(debut->drapeaux, mi_drapeau::VALIDE)) {
...
static auto calcul_longueur_aretes(Corps &corps)
{
    ...
    auto attr = corps.ajoute_attribut(
        "longueur_arête", type_attribut::R32, 1, portee_attr::VERTEX);
...
class OpGeometrieMaillage final : public OperatriceCorps {
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        if (type_metrie == "aire") {
        ...
        else if (dls::outils::est_element(type_metrie,
                                          "courbure_min",
                                          "courbure_max",
                                          "direction_min",
                                          "direction_max",
            ...
            auto donnees_ret = calcule_courbure(
                contexte.chef, m_corps, static_cast<double>(rayon));
            ...
            if (donnees_ret.nombre_impossible > 0) {
                this->ajoute_avertissement("Il y a ",
                                           donnees_ret.nombre_impossible,
                                           " points impossibles à calculer. Veuillez modifier le "
                                           "rayon pour stabiliser l'algorithme.");
                return res_exec::ECHOUEE;
            }
            ...
            if (type_metrie == "direction_min") {
                ...
                attr_sortie = m_corps.ajoute_attribut(
                    "gaussien", type_attribut::R32, 1, portee_attr::POINT);

                for (auto i = 0; i < points.taille(); ++i) {
                    attr_sortie->r32(i)[0] = attr_courbure_min->r32(i)[0] *
                                             attr_courbure_max->r32(i)[0];
                }
            }
                ...
                auto attr_courbure_max = m_corps.attribut("courbure_max");
                attr_sortie = m_corps.ajoute_attribut(
                    "moyenne", type_attribut::R32, 1, portee_attr::POINT);
    ...
    void visualise_attribut(Attribut *attr)
    {
        ...
        if (attr->type() == type_attribut::R32) {
            if (attr->dimensions == 1) {
                ...
                for (auto i = 0; i < attr->taille(); ++i) {
                    assigne(attr_C->r32(i),
                            couleur_min_max(attr->r32(i)[0], min_donnees, max_donnees));
                }
            }
  ...
  public:
    ...
    res_exec execute(ContexteEvaluation const &contexte, DonneesAval *donnees_aval) override
    {
        ...
        pour_chaque_polygone_ferme(*corps_entree, [&](Corps const &, Polygone const *poly) {
            ...
            dls::math::vec3f couleurs[3] = {dls::math::vec3f(0.0f, 0.0f, 1.0f),
...
class OpPiecesDetachees final : public OperatriceCorps {
  public:
    ...
    static constexpr auto AIDE =
        "Groupe les polygones du maillage d'entrée selon leur connectivité de sorte que les "
        "polygones ou ensemble de polygones séparées des autres soient dans des groupes "
        "distincts.";

                      ...
                      float dt)
{
    ...
    auto vort_vel = [&](int obpos[6], int i, int j) {
        ...
        //			((abs(objvelocity[(j)][obpos[(i)]]) > FLT_EPSILON) ? objvelocity[(j)][obpos[(i)]] :
        //velocity[(j)][index]);
        //		}
    ...
    for (int z = 1; z < res.z - 1; z++, index += res.x * res.x) {
        for (int y = 1; y < res.y - 1; y++, index += 2) {
            for (int x = 1; x < res.x - 1; x++, index++) {
                ...
                obpos[0] = (est_obstacle(drapeaux, index + dalle_y)) ? index :
                                                                       index + dalle_y;  // up
                obpos[1] = (est_obstacle(drapeaux, index - dalle_y)) ? index :
                                                                       index - dalle_y;  // down
                float dy = (obpos[0] == index || obpos[1] == index) ? dx_inv : mi_dx_inv;

                obpos[2] = (est_obstacle(drapeaux, index + dalle_z)) ? index :
                                                                       index + dalle_z;  // out
                obpos[3] = (est_obstacle(drapeaux, index - dalle_z)) ? index :
                                                                       index - dalle_z;  // in
                float dz = (obpos[2] == index || obpos[3] == index) ? dx_inv : mi_dx_inv;

                obpos[4] = (est_obstacle(drapeaux, index + dalle_x)) ? index :
                                                                       index + dalle_x;  // right
                obpos[5] = (est_obstacle(drapeaux, index - dalle_x)) ? index :
                                                                       index - dalle_x;  // left
                ...
                vorticite.valeur(index) = sqrtf(
                    vorticite_x.valeur(index) * vorticite_x.valeur(index) +
        ...
        for (int y = 1; y < res.y - 1; y++, index += 2) {
            for (int x = 1; x < res.x - 1; x++, index++) {
                ...
                if (magnitude > std::numeric_limits<float>::epsilon()) {
                    auto flame_vort = (fioul != nullptr) ?
                                          fioul->valeur(index) * vorticite_flamme :
                                          0.0f;
                    ...
                    vel.x += (N[1] * vorticite_z.valeur(index) -
                              N[2] * vorticite_y.valeur(index)) *
                             taill_voxel * (eps + flame_vort) * dt;
                    vel.y += (N[2] * vorticite_x.valeur(index) -
                              N[0] * vorticite_z.valeur(index)) *
                             taill_voxel * (eps + flame_vort) * dt;
                    vel.z += (N[0] * vorticite_y.valeur(index) -
                              N[1] * vorticite_x.valeur(index)) *
                             taill_voxel * (eps + flame_vort) * dt;
                }
            }
        }

...
static auto calcul_divergence(wlk::GrilleMAC const &velocite,
                              wlk::grille_dense_3d<int> const &drapeaux)
{
    ...
    boucle_parallele(
        tbb::blocked_range<int>(1, res.z - 1), [&](tbb::blocked_range<int> const &plage) {
                ...
                for (auto y = 1; y < res.y - 1; ++y) {
                    for (auto x = 1; x < res.x - 1; ++x) {
                        ...
                        if (est_obstacle(drapeaux, index + 1))
                            xright = -velocite.valeur(index).x;  // DG: +=
                        if (est_obstacle(drapeaux, index - 1))
                            xleft = -velocite.valeur(index).x;
                        if (est_obstacle(drapeaux, index + res.x))
                            yup = -velocite.valeur(index).y;
                        if (est_obstacle(drapeaux, index - res.x))
                            ydown = -velocite.valeur(index).y;
                        if (est_obstacle(drapeaux, index + taille_dalle))
                            ztop = -velocite.valeur(index).z;
                        if (est_obstacle(drapeaux, index - taille_dalle))
                            zbottom = -velocite.valeur(index).z;

                        //				if(_obstacles[index+1] & 8)			xright	+=
                        //_xVelocityOb.valeur(index + 1);
                        //				if(_obstacles[index-1] & 8)			xleft	+=
                        //_xVelocityOb.valeur(index - 1); 				if(_obstacles[index+res.x] & 8)		yup
                        //+= _yVelocityOb.valeur(index + res.x); 				if(_obstacles[index-res.x] & 8)
                        //ydown	+= _yVelocityOb.valeur(index - res.x);
                        //				if(_obstacles[index+taille_dalle] & 8) ztop    +=
                        //_zVelocityOb.valeur(index + taille_dalle);
                        //				if(_obstacles[index-taille_dalle] & 8) zbottom +=
                        //_zVelocityOb.valeur(index - taille_dalle);

                        divergence.valeur(index) = -dx * 0.5f *
                                                   (xright - xleft + yup - ydown + ztop - zbottom);
...
static auto resoud_pression(wlk::grille_dense_3d<float> &pression,
                            ...
                            float precision)
{
    ...
    auto nouveau_delta = tbb::parallel_reduce(
        tbb::blocked_range<int>(1, res.z - 1), 0.0f, calcul_delta_precond, std::plus<float>());
    ...
    while ((i < iterations) && (residue_max > 0.001f * eps)) {
        ...
        auto alpha = tbb::parallel_reduce(
            tbb::blocked_range<int>(1, res.z - 1), 0.0f, calcul_alpha_loc, std::plus<float>());
        ...
        /* x = x + alpha * d */
        auto calcul_delta_max_r = [&](tbb::blocked_range<int> const &plage,
        ...
        auto p_dm = tbb::parallel_reduce(tbb::blocked_range<int>(1, res.z - 1),
                                         std::pair(0.0f, 0.0f),
                                         calcul_delta_max_r,
                         ...
                         [&](tbb::blocked_range<int> const &plage) {
                             for (auto z = plage.begin(); z < plage.end(); ++z) {
                                 for (auto y = 1; y < res.y - 1; ++y) {
                                     for (auto x = 1; x < res.x - 1; ++x) {
                                         auto const index = x + (y + z * res.y) * res.x;
                                         direction.valeur(index) = h.valeur(index) +
...
static auto projette_solution(wlk::GrilleMAC &velocite,
                ...
                for (auto y = 1; y < res.y - 1; ++y) {
                    for (auto x = 1; x < res.x - 1; ++x) {
                        ...
                        if (!est_obstacle(drapeaux, index)) {
                            ...
                            velocite.valeur(index).x = (vMask[0] * velocite.valeur(index).x) +
                                                       vObst[0];
                            velocite.valeur(index).y = (vMask[1] * velocite.valeur(index).y) +
                                                       vObst[1];
                            velocite.valeur(index).z = (vMask[2] * velocite.valeur(index).z) +
                                                       vObst[2];
                        }
                        else {
                            //					_xVelocity.valeur(index) =
                            //_xVelocityOb.valeur(index); 					_yVelocity.valeur(index) =
                            //_yVelocityOb.valeur(index); 					_zVelocity.valeur(index) =
                            //_zVelocityOb.valeur(index);
                        }
                    }
                }
            }

...
static void verifie_nans(wlk::grille_dense_3d<float> const &grille,
                         char const *message0,
                         char const *nom_grille)
{
...
static void verifie_infs(wlk::grille_dense_3d<float> const &grille,
                         char const *message0,
                         char const *nom_grille)
{
    for (auto i = 0; i < grille.nombre_elements(); ++i) {
...
void InvertCheckFluid(const wlk::grille_dense_3d<int> &flags, wlk::grille_dense_3d<float> &grid)
{
...
double somme_carree(const wlk::grille_dense_3d<float> &grid)
{
    auto somme = 0.0;
...
template <typename T>
auto gridScaledAdd(wlk::grille_dense_3d<T> &grid1, const wlk::grille_dense_3d<T> &grid2, T facteur)
{
...
template <typename T>
auto efface(wlk::grille_dense_3d<T> &grille)
{
...
void InitPreconditionIncompCholesky(wlk::grille_dense_3d<int> const &flags,
                                    wlk::grille_dense_3d<float> &A0,
                                    wlk::grille_dense_3d<float> &Ai,
                                    wlk::grille_dense_3d<float> &Aj,
                                    wlk::grille_dense_3d<float> &Ak,
                                    wlk::grille_dense_3d<float> &orgA0,
                                    wlk::grille_dense_3d<float> &orgAi,
                                    wlk::grille_dense_3d<float> &orgAj,
                                    wlk::grille_dense_3d<float> &orgAk)
{
...
void InitPreconditionModifiedIncompCholesky2(const wlk::grille_dense_3d<int> &flags,
                                             wlk::grille_dense_3d<float> &Aprecond,
                                             wlk::grille_dense_3d<float> &A0,
                                             wlk::grille_dense_3d<float> &Ai,
                                             wlk::grille_dense_3d<float> &Aj,
                                             wlk::grille_dense_3d<float> &Ak)
{
    ...
    while (!iter.fini()) {
        ...
        e -= tau *
             (Ai.valeur(idx - dalle_x) * (Aj.valeur(idx - dalle_x) + Ak.valeur(idx - dalle_x)) *
                  dls::math::carre(Aprecond.valeur(idx - dalle_x)) +
              Aj.valeur(idx - dalle_y) * (Ai.valeur(idx - dalle_y) + Ak.valeur(idx - dalle_y)) *
                  dls::math::carre(Aprecond.valeur(idx - dalle_y)) +
              Ak.valeur(idx - dalle_z) * (Ai.valeur(idx - dalle_z) + Aj.valeur(idx - dalle_z)) *
                  dls::math::carre(Aprecond.valeur(idx - dalle_z)) +
              0.0f);
...
void InitPreconditionMultigrid(GridMg *MG,
                               wlk::grille_dense_3d<float> &A0,
                               wlk::grille_dense_3d<float> &Ai,
                               wlk::grille_dense_3d<float> &Aj,
                               wlk::grille_dense_3d<float> &Ak,
...
//! Apply WT-style ICP
void ApplyPreconditionIncompCholesky(wlk::grille_dense_3d<float> &dst,
                                     wlk::grille_dense_3d<float> &Var1,
                                     ...
                                     wlk::grille_dense_3d<float> &orgAk)
{
    ...
    auto iter_inv = wlk::IteratricePositionInv(limites);
    ...
    while (!iter_inv.fini()) {
        ...
        dst.valeur(idx) = A0.valeur(idx) *
                          (dst.valeur(idx) - dst.valeur(idx + dalle_x) * Ai.valeur(idx) -
...
void ApplyPreconditionModifiedIncompCholesky2(wlk::grille_dense_3d<float> &dst,
                                              wlk::grille_dense_3d<float> &Var1,
                                              const wlk::grille_dense_3d<int> &flags,
                                              wlk::grille_dense_3d<float> &Aprecond,
                                              wlk::grille_dense_3d<float> &A0,
                                              ...
                                              wlk::grille_dense_3d<float> &Ak)
{
    ...
    while (!iter.fini()) {
        ...
        dst.valeur(idx) = p * (Var1.valeur(idx) -
                               dst.valeur(idx - dalle_x) * Ai.valeur(idx - dalle_x) *
                                   Aprecond.valeur(idx - dalle_x) -
                               dst.valeur(idx - dalle_y) * Aj.valeur(idx - dalle_y) *
                                   Aprecond.valeur(idx - dalle_y) -
                               dst.valeur(idx - dalle_z) * Ak.valeur(idx - dalle_z) *
                                   Aprecond.valeur(idx - dalle_z));
    }
    ...
    while (!iter_inv.fini()) {
        ...
        dst.valeur(idx) = p * (dst.valeur(idx) - dst.valeur(idx + dalle_x) * Ai.valeur(idx) * p -
...
void ApplyPreconditionMultigrid(GridMg *pMG,
                                wlk::grille_dense_3d<float> &dst,
                                wlk::grille_dense_3d<float> &Var1)
{
...
double produit_scalaire(wlk::grille_dense_3d<float> const &a, wlk::grille_dense_3d<float> const &b)
{
    auto result = 0.0;
...
float max_abs(wlk::grille_dense_3d<float> const &grille)
{
...
double InitSigma(const wlk::grille_dense_3d<int> &flags,
                 wlk::grille_dense_3d<float> &dst,
                 wlk::grille_dense_3d<float> &rhs,
                 wlk::grille_dense_3d<float> &temp)
{
    auto sigma = 0.0;
...
void UpdateSearchVec(wlk::grille_dense_3d<float> &dst,
                     wlk::grille_dense_3d<float> &src,
                     float factor)
{
...
GridCgInterface::GridCgInterface() : mUseL2Norm(true)
{
}
...
GridCg::GridCg(wlk::grille_dense_3d<float> &dst,
               wlk::grille_dense_3d<float> &rhs,
               wlk::grille_dense_3d<float> &residual,
               wlk::grille_dense_3d<float> &search,
               const wlk::grille_dense_3d<int> &flags,
               wlk::grille_dense_3d<float> &tmp,
               wlk::grille_dense_3d<float> *pA0,
               wlk::grille_dense_3d<float> *pAi,
               wlk::grille_dense_3d<float> *pAj,
               wlk::grille_dense_3d<float> *pAk)
    : GridCgInterface(), mInited(false), mIterations(0), mDst(dst), mRhs(rhs), mResidual(residual),
      ...
      mpPCA0(nullptr), mpPCAi(nullptr), mpPCAj(nullptr), mpPCAk(nullptr), mMG(nullptr),
      mPcMethod(PC_None), mSigma(0.0), mAccuracy(VECTOR_EPSILON), mResNorm(1e20f)
{
}
...
bool GridCg::iterate()
{
    ...
    if (mResNorm < mAccuracy) {
        ...
        std::cerr << "Arrêt au bout de " << mIterations << " car mResNorm (" << mResNorm
                  << ") < mAccuracy (" << mAccuracy << ")!\n";
        return false;
    }
    ...
    std::cerr << "GridCg::iterate i=" << mIterations << " sigmaNew=" << sigmaNew
              << " sigmaLast=" << mSigma << " alpha=" << alpha << " beta=" << beta << "\n";
    mSigma = sigmaNew;

    if (!(static_cast<double>(mResNorm) < 1e35)) {
        if (mPcMethod == PC_MGP) {
            // diverging solves can be caused by the static multigrid mode, we cannot detect this
            // here, though only the pressure solve call "knows" whether the MG is static or
            // dynamics...
            // debMsg("GridCg::iterate: Warning - this diverging solve can be caused by the
            // 'static' mode of the MG preconditioner. If the static mode is active, try switching
            // to dynamic.", 1);
        }
        std::cerr << "GridCg::iterate: The CG solver diverged, residual norm (" << mResNorm
                  << ") > 1e30, stopping.\n";
        return false;
    }

    // std::cerr <<"PB-CG-Norms::p"<<sqrt(GridOpNormNosqrt(mpDst, mpFlags).getValue()) <<"
    // search"<<sqrt(GridOpNormNosqrt(mpSearch, mpFlags).getValue(), CG_DEBUGLEVEL) <<"
    // res"<<sqrt(GridOpNormNosqrt(mpResidual, mpFlags).getValue()) <<"
    // tmp"<<sqrt(GridOpNormNosqrt(mpTmp, mpFlags).getValue()); // debug
    return true;
}
...
void GridCg::setICPreconditioner(PreconditionType method,
                                 wlk::grille_dense_3d<float> *A0,
                                 wlk::grille_dense_3d<float> *Ai,
                                 wlk::grille_dense_3d<float> *Aj,
                                 wlk::grille_dense_3d<float> *Ak)
...
{
    // assertMsg(method==PC_MGP, "GridCg<APPLYMAT>::setMGPreconditioner: Invalid method
    // specified.");
		...
		float cgAccuracy   = 1e-4)
{
	...
	wlk::grille_dense_3d<float> rhs(parent);
	wlk::grille_dense_3d<float> residual(parent), search(parent), tmp(parent);
	wlk::grille_dense_3d<float> A0(parent), Ai(parent), Aj(parent), Ak(parent);

	// setup matrix and boundaries
	wlk::grille_dense_3d<int> flagsDummy(parent);
	flagsDummy.setConst(wlk::grille_dense_3d<int>::TypeFluid);
	...
	if (grid.getType() & GridBase::Typefloat) {
		auto &u = ((wlk::grille_dense_3d<float>&) grid);
		rhs.copie_donnees(u);
	...
	{
		wlk::grille_dense_3d<Vec3>& vec = ((wlk::grille_dense_3d<Vec3>&) grid);
		wlk::grille_dense_3d<float> u(parent);
...
void ApplyMatrix(const wlk::grille_dense_3d<int> &flags,
                 wlk::grille_dense_3d<float> &dst,
                 const wlk::grille_dense_3d<float> &src,
                 wlk::grille_dense_3d<float> &A0,
                 wlk::grille_dense_3d<float> &Ai,
                 wlk::grille_dense_3d<float> &Aj,
                 wlk::grille_dense_3d<float> &Ak)

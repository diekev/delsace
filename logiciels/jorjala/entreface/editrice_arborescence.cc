...
SceneTreeWidgetItem::SceneTreeWidgetItem(BaseDeDonnees *scene, QWidget *parent)
    : QWidget(parent), m_scene(scene), m_visited(false)
{
...
ObjectTreeWidgetItem::ObjectTreeWidgetItem(Objet *scene_node, QTreeWidgetItem *parent)
    : QTreeWidgetItem(parent), m_scene_node(scene_node), m_visited(false)
{
    setChildIndicatorPolicy(QTreeWidgetItem::ShowIndicator);
...
ObjectNodeTreeWidgetItem::ObjectNodeTreeWidgetItem(Noeud *noeud, QTreeWidgetItem *parent)
    : QTreeWidgetItem(parent), m_noeud(noeud)
{
...
EditriceArborescence::EditriceArborescence(Jorjala &jorjala, QWidget *parent)
    : BaseEditrice(jorjala, parent), m_tree_widget(new TreeWidget(this))
{
    m_main_layout->addWidget(m_tree_widget);

    m_tree_widget->set_base(this);

    connect(m_tree_widget,
    ...
    connect(m_tree_widget, SIGNAL(itemSelectionChanged()), this, SLOT(handleItemSelection()));
}
...
void EditriceArborescence::ajourne_etat(int evenement_)
{
    ...
    if (!dls::outils::est_element(
            categorie_evenement(evenement), type_evenement::objet, type_evenement::noeud)) {
        return;
    }

    if (!dls::outils::est_element(
            action_evenement(evenement), type_evenement::ajoute, type_evenement::modifie)) {
        return;
    }

...
VueMonde::VueMonde(kdo::Monde *monde) : m_monde(monde)
{
...
{
    auto const type_texture = static_cast<TypeTexture>(
        0);  // static_cast<TypeTexture>(evalue_enum("type_texture"));
    ...
    if (type_texture == TypeTexture::COULEUR) {
        if (m_monde->texture) {
            ...
            valeur_couleur("couleur",
                           dls::phys::couleur32(couleur[0], couleur[1], couleur[2], 1.0f));
        }
    }
...
EditeurMonde::EditeurMonde(kdo::Koudou *koudou, QWidget *parent)
    : BaseEditrice(*koudou, parent), m_vue(new VueMonde(&koudou->parametres_rendu.scene.monde)),

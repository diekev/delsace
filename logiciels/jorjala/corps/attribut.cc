...
Attribut::Attribut(
    dls::chaine const &name, type_attribut type, int dims, portee_attr portee_, long taille)
    : m_nom(name), m_type(type), portee(portee_), dimensions(dims)
{
    assert(taille >= 0);
    detache();
...
void Attribut::detache()
{
    ...
    if (!m_tampon.unique()) {
        m_tampon = ptr_liste(memoire::loge<type_liste>("attribut", *(m_tampon.get())),
                             supprime_liste);
    }
}
...
TransferanteAttribut::TransferanteAttribut(const Corps &corps_orig,
                                           Corps &corps_dest,
                                           int drapeaux)
{
        ...
        switch (attr.portee) {
            case portee_attr::POINT:
            {
                if ((drapeaux & TRANSFERE_ATTR_POINTS) != 0) {
                    auto nattr = corps_dest.ajoute_attribut(

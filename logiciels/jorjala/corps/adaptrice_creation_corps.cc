...
void AdaptriceCreationCorps::ajoute_sommet(const float x,
                                           const float y,
                                           const float z,
                                           const float w)
{
...
void AdaptriceCreationCorps::ajoute_normal(const float x, const float y, const float z)
{
    if (attribut_normal == nullptr) {
        attribut_normal = corps->ajoute_attribut(
            "N", type_attribut::R32, 3, portee_attr::POINT, true);
    }
...
void AdaptriceCreationCorps::ajoute_polygone(const int *index_sommet,
                                             const int *index_uvs,
                                             const int *index_normaux,
                                             long nombre)
{
    INUTILISE(index_uvs);
    ...
    if (index_normaux != nullptr) {
        ...
        if (normaux_polys) {
            if (attribut_normal_polys == nullptr) {
                attribut_normal_polys = corps->ajoute_attribut(
                    "N_polys", type_attribut::R32, 3, portee_attr::PRIMITIVE);
            }

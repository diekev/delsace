...
Attribut *Corps::ajoute_attribut(dls::chaine const &nom_attribut,
                                 type_attribut type_,
                                 int dimensions,
                                 portee_attr portee,
                                 bool force_vide)
...
{
    auto iter = std::find_if(m_attributs.debut(), m_attributs.fin(), [&](Attribut const &attr) {
...
{
    auto iter = std::find_if(m_groupes_points.debut(),
                             m_groupes_points.fin(),
                             [&](GroupePoint const &groupe) { return groupe.nom == nom_groupe; });
...
GroupePrimitive *Corps::groupe_primitive(const dls::chaine &nom_groupe) const
{
    auto iter = std::find_if(
        m_groupes_prims.debut(), m_groupes_prims.fin(), [&](GroupePrimitive const &groupe) {

...
static double poids_compatibilite(types_entrees const &types_fonctions,
...
static triade_types types_fonctions_finaux(param_entrees const &entrees,
...
signature::signature(param_entrees _entrees_, param_sorties _sorties_)
    : entrees(std::move(_entrees_)), sorties(std::move(_sorties_))
{
}

/* ************************************************************************** */

donnees_fonction *magasin_fonctions::ajoute_fonction(const dls::chaine &nom,
                                                     code_inst type,
                                                     const signature &seing,
                                                     ctx_script ctx)
{
...
donnees_fonction_generation magasin_fonctions::meilleure_candidate(
    dls::chaine const &nom, types_entrees const &types_params)
{
    ...
    for (auto &donnee : tableau) {
        ...
        auto [param_entrees_, param_sorties_, type] = types_fonctions_finaux(
            donnee.seing.entrees, donnee.seing.sorties, types_params);
...
static void enregistre_fonctions_operations_binaires(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction(
        "ajoute",
        code_inst::FN_AJOUTE,
        signature(param_entrees(donnees_parametre("valeur1", type_var::POLYMORPHIQUE),
...
static void enregistre_fonctions_mathematiques(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("valeur", type_var::POLYMORPHIQUE),
    ...
    magasin.ajoute_fonction("aléa_uni",
                            code_inst::FN_ALEA_UNI,
                            signature(param_entrees(donnees_parametre("min", type_var::DEC),
                                                    donnees_parametre("max", type_var::DEC, 1.0f)),
                                      param_sorties(donnees_parametre("valeur", type_var::DEC))),
                            ctx_script::tous);

    magasin.ajoute_fonction("aléa_nrm",
...
static void enregistre_fonctions_vectorielles(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction("sépare_vec2",
                            code_inst::FN_SEPARE_VEC3,
                            signature(param_entrees(donnees_parametre("valeur", type_var::VEC2)),
                                      param_sorties(donnees_parametre("x", type_var::DEC),
                                                    donnees_parametre("y", type_var::DEC))),
                            ctx_script::tous);

    magasin.ajoute_fonction("sépare_vec3",
                            code_inst::FN_SEPARE_VEC3,
    ...
    magasin.ajoute_fonction("combine_vec2",
                            code_inst::FN_COMBINE_VEC2,
                            signature(param_entrees(donnees_parametre("x", type_var::DEC),
                                                    donnees_parametre("y", type_var::DEC)),
                                      param_sorties(donnees_parametre("valeur", type_var::VEC2))),
                            ctx_script::tous);

    magasin.ajoute_fonction("combine_vec3",
    ...
    magasin.ajoute_fonction("base_orthonormale",
                            code_inst::FN_BASE_ORTHONORMALE,
                            signature(param_entrees(donnees_parametre("valeur", type_var::VEC3)),
                                      param_sorties(donnees_parametre("base_a", type_var::VEC3),
                                                    donnees_parametre("base_b", type_var::VEC3))),
                            ctx_script::tous);

    magasin.ajoute_fonction("produit_scalaire",
                            code_inst::FN_PRODUIT_SCALAIRE_VEC3,
                            signature(param_entrees(donnees_parametre("vecteur1", type_var::VEC3),
                                                    donnees_parametre("vecteur2", type_var::VEC3)),
                                      param_sorties(donnees_parametre("valeur", type_var::DEC))),
                            ctx_script::tous);

    magasin.ajoute_fonction("produit_croix",
    ...
    magasin.ajoute_fonction("longueur",
                            code_inst::FN_LONGUEUR_VEC3,
                            signature(param_entrees(donnees_parametre("vecteur", type_var::VEC3)),
                                      param_sorties(donnees_parametre("longeur", type_var::DEC))),
                            ctx_script::tous);

    magasin.ajoute_fonction("normalise",
                            code_inst::FN_NORMALISE_VEC3,
                            signature(param_entrees(donnees_parametre("vecteur", type_var::VEC3)),
                                      param_sorties(donnees_parametre("normal", type_var::VEC3),
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("entrée", type_var::VEC3),
    ...
    magasin.ajoute_fonction("réfléchi",
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(), param_sorties(donnees_parametre("valeur", type_var::VEC3))),
        ctx_script::tous);

    magasin.ajoute_fonction("proj_uv_sphère",
                            code_inst::FN_PROJ_UV_SPHERE,
                            signature(param_entrees(donnees_parametre("u", type_var::DEC),
...
static void enregistre_fonctions_corps(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction("ajoute_point",
    ...
    magasin.ajoute_fonction("ajoute_primitive",
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("index_prim", type_var::ENT32),
    ...
    magasin.ajoute_fonction("ajoute_ligne",
                            code_inst::FN_AJOUTE_LIGNE,
                            signature(param_entrees(donnees_parametre("point0", type_var::VEC3),
...
static void enregistre_fonctions_attributs(magasin_fonctions &magasin)
{
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("nom", type_var::CHAINE),
                                donnees_parametre("index_point", type_var::ENT32)),
                  param_sorties(donnees_parametre("valeur", type_var::DEC))),
        ctx_script::tous);
}
...
static void enregistre_fonctions_colorimetriques(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("avant_plan", type_var::COULEUR),
    ...
    magasin.ajoute_fonction("évalue_courbe_valeur",
...
static void enregistre_fonctions_types(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("tableau", type_var::TABLEAU)),
                  param_sorties(donnees_parametre("index", type_var::ENT32))),
        ctx_script::tous);
    ...
    magasin.ajoute_fonction("taille",
                            code_inst::FN_TAILLE_CHAINE,
                            signature(param_entrees(donnees_parametre("chaine", type_var::CHAINE)),
    ...
    magasin.ajoute_fonction("chaine_vers_décimal",
...
static void enregistre_fonctions_bruits(magasin_fonctions &magasin)
{
    ...
    for (auto paire : paires) {
        magasin.ajoute_fonction(
            paire.first,
            paire.second,
            signature(param_entrees_bruit,
                      param_sorties(donnees_parametre("bruit", type_var::ENT32))),
            ctx_script::tous);
    }

    magasin.ajoute_fonction("évalue_bruit",
                            code_inst::FN_EVALUE_BRUIT,
                            signature(param_entrees(donnees_parametre("bruit", type_var::ENT32),
...
static void enregistre_fonctions_images(magasin_fonctions &magasin)
{
    ...
    magasin.ajoute_fonction(
        ...
        signature(param_entrees(donnees_parametre("image", type_var::ENT32),
    ...
    magasin.ajoute_fonction("projection_sphérique",
                            code_inst::FN_PROJECTION_SPHERIQUE,
                            signature(param_entrees(donnees_parametre("pos", type_var::VEC3)),
                                      param_sorties(donnees_parametre("uv", type_var::VEC2))),
                            ctx_script::tous);

    magasin.ajoute_fonction("projection_cylindrique",
                            code_inst::FN_PROJECTION_CYLINDRIQUE,
                            signature(param_entrees(donnees_parametre("pos", type_var::VEC3)),
                                      param_sorties(donnees_parametre("uv", type_var::VEC2))),
                            ctx_script::tous);

    magasin.ajoute_fonction("projection_caméra",
                            code_inst::FN_PROJECTION_CAMERA,
                            signature(param_entrees(donnees_parametre("caméra", type_var::ENT32),
                                                    donnees_parametre("pos", type_var::VEC3)),
                                      param_sorties(donnees_parametre("uv", type_var::VEC2))),
                            ctx_script::tous);
}

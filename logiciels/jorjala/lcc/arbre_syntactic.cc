...
base::base(ContexteGenerationCode &contexte, DonneesMorceaux const &donnees_, type_noeud type_)
    : donnees(donnees_), type(type_), donnees_type(type_var::INVALIDE)
{
    INUTILISE(contexte);
}
...
void base::imprime_code(std::ostream &os, int profondeur)
{
    ...
    if (this->type != type_noeud::RACINE) {
        os << " : " << donnees.chaine << " (" << chaine_type_var(this->donnees_type) << ')';
    }
...
static auto calcul_conversion(type_var type1, type_var type2)
{
    if (type1 == type2) {
                ...
                bool expr_gauche)
{
    switch (b->type) {
        ...
        case type_noeud::ATTRIBUT:
        {
            ...
            if (donnees == nullptr) {
                if (expr_gauche) {
                    gest_attrs.requiers_attr(
                        dls::chaine(b->chaine()), b->donnees_type, b->pointeur_donnees);
                }
                else {
                    erreur::lance_erreur("Attribut inconnu.",
                                         contexte_generation,
                                         b->donnees,
                ...
                if (expr_gauche) {
                    if (donnees->est_non_modifiable) {
                        erreur::lance_erreur("La propriété n'est pas modifiable.",
                                             contexte_generation,
                                             b->donnees,
                                             erreur::type_erreur::NORMAL);
                    }
        ...
        case type_noeud::VARIABLE:
        {
            if (expr_gauche) {
                ...
                if (contexte_generation.locale_existe(b->chaine())) {
                    ...
                    b->pointeur_donnees = compileuse.donnees().loge_donnees(
                        taille_type(b->donnees_type));
                    contexte_generation.pousse_locale(
                        b->chaine(), b->pointeur_donnees, b->donnees_type);
                }
            }
            else {
        ...
        case type_noeud::VALEUR:
        {
            ...
            switch (b->donnees_type) {
                ...
                case type_var::MAT4:
                {
                    ...
                    if (b->enfants.est_vide()) {
                        ...
                        for (auto const &feuille : feuilles) {
                            compileuse.donnees().stocke(decalage,
                                                        extrait_decimal(feuille->chaine()));
                        }
                ...
                case type_var::CHAINE:
                {
                    ...
                    compileuse.donnees().stocke(
                        ptr, static_cast<int>(contexte_generation.chaines.taille()));
        ...
        case type_noeud::FONCTION:
        {
            ...
            auto donnees_fonc = contexte_generation.fonctions.meilleure_candidate(
                dls::chaine(b->chaine()), types_params);
            ...
            for (auto i = 0; i < taille; ++i) {
                ...
                if ((*enfant)->donnees_type == type_entree) {
                    ...
                    auto ptr = ajoute_conversion(compileuse,
                                                 (*enfant)->donnees_type,
                                                 type_entree,
        ...
        case type_noeud::OPERATION_BINAIRE:
        {
            ...
            auto expr_gauche_ = expr_gauche || dls::outils::est_element(b->identifiant(),
            ...
            if (b->identifiant() == id_morceau::CROCHET_OUVRANT) {
                if (enfant2->donnees_type != lcc::type_var::TABLEAU) {
                    erreur::lance_erreur("Un tableau est requis pour l'opérateur []",
                                         contexte_generation,
                                         enfant2->donnees_morceau());
                }

                if (enfant1->donnees_type != lcc::type_var::ENT32) {
                    erreur::lance_erreur("Un nombre entier est requis dans l'opérateur []",
                                         contexte_generation,
                ...
                if (conversion == conv_op::aucune) {
                    ...
                    decalage1 = ajoute_conversion(compileuse,
                    ...
                    decalage2 = ajoute_conversion(compileuse,
                                                  enfant2->donnees_type,
                                                  enfant1->donnees_type,
            ...
            switch (b->identifiant()) {
                ...
                {
                    erreur::lance_erreur(
                        "Opération inconnue", contexte_generation, b->donnees_morceau());
                }
            }
        ...
        case type_noeud::ASSIGNATION:
        {
            ...
            // l'idée est que les sorties des fonctions sont des pointeurs vers la où se trouve les
            // valeurs
        ...
        case type_noeud::ACCES_MEMBRE_POINT:
        {
            ...
            if (enfant_droite->type == type_noeud::FONCTION) {
                ...
                switch (enfant_gauche->donnees_type) {
                    ...
                    {
                        dls::flux_chaine ss;
                        ss << "La variable '" << enfant_droite->chaine()
                           << "' n'est pas une structure !"
                           << " Accès au membre '" << enfant_gauche->chaine() << "' impossible.";
        ...
        case type_noeud::SI:
        {
            ...
            if (enfant3 != nullptr) {
                ...
                instructions.stocke(decalage_branche_si_faux,
                                    static_cast<int>(instructions.taille()));

...
int main()
{
    ...
    // donnees_module->tampon = TamponSource("offset = traduit($P, 0, 1, -1, 1); $P =
    // bruit_turbulent(7, offset, $P);"); donnees_module->tampon = TamponSource("v0, v1, v2 =
    // sépare_vecteur(v);"); donnees_module->tampon = TamponSource("v0, v1 = base_orthonormale($P);
    // $P = réfléchi(v0, v1);"); donnees_module->tampon = TamponSource("a = vec3(1.5, 6.7, 9.8); b =
    // vec3(25.0, 16.0, 92.0); $P = -(a * b);\n"); donnees_module->tampon = TamponSource("tabl = [4,
    // 3, 5, 7, 9];\n"); donnees_module->tampon = TamponSource("pour i dans 0...10 { $P.x = 2.0 * i;
    // } $P.z = 5.0;\n"); donnees_module->tampon = TamponSource("@C.r = 1.0;\n");
    // donnees_module->tampon = TamponSource("$P *= 2.0;\n");
    // donnees_module->tampon = lng::tampon_source("x = 0; y = 0; pour i dans 0...32 { continue;
    // $P.x = 2.0 * i; x += 5; } retourne; pour i dans 0...32 { arrête; $P.x = 2.0 * i; } \n");
    donnees_module->tampon = lng::tampon_source(
        R"(l = $ligne; tabl = morcelle(l, ","); m3 = extrait_chaine(tabl, 4); $P.x = chaine_vers_décimal(m3);)");
    ...
    try {
        ...
        for (auto entree : entrees) {
            ...
            lcc::execute_pile(
                ctx_exec, ctx_local, compileuse.donnees(), compileuse.instructions(), 0);

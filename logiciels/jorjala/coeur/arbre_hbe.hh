...
double calcul_cout_scission(double const scission,
...
double trouve_meilleure_scission(ArbreHBE::Noeud &noeud,
                                 dls::tableau<long> &references,
                                 Axe const direction,
...
auto construit_arbre_hbe(TypeDelegue const &delegue_prims, unsigned int profondeur_max)
{
    ...
    auto const taille_estimee = feuilles_max + branches_implicites(coeff_arbre, feuilles_max) +
                                coeff_arbre;
    ...
    for (auto couche = 0u; couche < profondeur_max; ++couche) {
        ...
        for (auto i = 0; i < nombre_noeud_couche; ++i) {
            ...
            if (compte_ref <= 5 || (couche == profondeur_max - 1)) {
                ...
                auto meilleure_scission = trouve_meilleure_scission(*noeud,
                                                                    refs_courantes[i],
                                                                    axe_scission,
                                                                    10,
                                                                    boites_alignees,
                ...
                if (compte_gauche == 0 || compte_droite == 0) {
                    ...
                    meilleure_scission = trouve_meilleure_scission(*noeud,
                                                                   refs_courantes[i],
                                                                   axe_scission,
                                                                   10,
                                                                   boites_alignees,
                ...
                if (compte_gauche == 0 || compte_droite == 0) {
                    ...
                    for (auto j = 0; j < noeud->nombre_references; ++j) {
                        ...
                        if (ref.centroide[static_cast<size_t>(axe_scission)] <=
                            meilleure_scission) {
...
template <typename T>
auto fast_ray_nearest_hit(ArbreHBE::Noeud const *node,
                          int *index,
                          dls::phys::rayon<T> const &ray,
                          dls::math::vec3<T> const &idot_axis,
                          T dist)
{
    const T bv[6] = {node->limites.min[0],
                     node->limites.max[0],
                     node->limites.min[1],
                     node->limites.max[1],
                     node->limites.min[2],
 ...
 */
template <typename TypeDelegue>
auto traverse(ArbreHBE const &arbre, TypeDelegue const &delegue, dls::phys::rayond const &rayon)
{
    ...
    while (!file.est_vide()) {
        ...
        if (noeud->est_feuille()) {
            ...
            for (auto e = 0; e < 2; ++e) {
                ...
                auto dist_gauche = fast_ray_nearest_hit(
...
double calcul_point_plus_proche(ArbreHBE::Noeud const &noeud,
                                dls::math::point3d const &point,
                                dls::math::point3d &plus_proche);

template <typename TypeDelegue>
auto cherche_point_plus_proche_ex(ArbreHBE &arbre,
...
template <typename TypeDelegue>
auto cherche_point_plus_proche(ArbreHBE &arbre,
                               TypeDelegue const &delegue,
...
template <typename T>
auto fast_ray_nearest_hit(BVHNode const *node,
                          int *index,
                          dls::phys::rayon<T> const &ray,
                          dls::math::vec3<T> const &idot_axis,
                          T dist)

...
Graphe::Graphe(Noeud &parent) : noeud_parent(parent)
{
}
...
void Graphe::supprime_noeud(Noeud *noeud)
{
    this->noeud_parent.enfants.erase(
        std::find(this->noeud_parent.enfants.debut(), this->noeud_parent.enfants.fin(), noeud));
...
void Graphe::supprime(Noeud *node)
{
    auto iter = std::find_if(
        m_noeuds.debut(), m_noeuds.fin(), [node](Noeud *node_ptr) { return node_ptr == node; });
...
Noeud *trouve_noeud(Graphe::plage_noeud noeuds, const float x, const float y)
{
...
PriseEntree *trouve_prise_entree_pos(Noeud *noeud, const float x, const float y)
{
    auto pred = [&](PriseEntree *prise) { return prise->rectangle.contiens(x, y); };

    auto iter = std::find_if(noeud->entrees.debut(), noeud->entrees.fin(), pred);
...
PriseEntree *trouve_prise_entree(Graphe::plage_noeud noeuds, const float x, const float y)
{
...
PriseSortie *trouve_prise_sortie_pos(Noeud *noeud, const float x, const float y)
{
    auto pred = [&](PriseSortie *prise) { return prise->rectangle.contiens(x, y); };

    auto iter = std::find_if(noeud->sorties.debut(), noeud->sorties.fin(), pred);
...
PriseSortie *trouve_prise_sortie(Graphe::plage_noeud noeuds, const float x, const float y)
{

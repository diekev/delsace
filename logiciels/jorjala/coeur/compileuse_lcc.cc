...
static auto stocke_attributs(gestionnaire_propriete const &gest_attrs,
                             lcc::pile &donnees,
...
static auto charge_attributs(gestionnaire_propriete const &gest_attrs,
                             lcc::pile &donnees,
...
static auto ajoute_attributs_contexte(Corps const &corps,
                                      ...
                                      lcc::ctx_script ctx)
{
    ...
    for (auto &attr : corps.attributs()) {
        if (attr.portee == portee) {
            ...
            gest_attrs.ajoute_attribut(
                attr.nom(), converti_type_attr(attr.type(), attr.dimensions), idx);
        }
    }
}

static auto cree_proprietes_parametres_declares(danjo::Manipulable *manipulable,
                                                ContexteGenerationCode &ctx_gen)
{
...
static auto ajoute_proprietes_extra(danjo::Manipulable *manipulable,
                                    ContexteGenerationCode &ctx_gen,
                                    compileuse_lng &compileuse,
                                    int temps)
{
...
static auto ajoute_proprietes_contexte(lcc::ctx_script contexte,
                                       compileuse_lng &compileuse,
...
{
    lcc::execute_pile(m_ctx_global, ctx_local, donnees_pile, m_compileuse.instructions(), 0);
}
...
CompileuseGrapheLCC::CompileuseGrapheLCC(Graphe &ptr_graphe) : graphe(ptr_graphe)
{
}
...
bool CompileuseGrapheLCC::compile_graphe(ContexteEvaluation const &contexte, Corps *corps)
{
    ...
    if (type_detail == DETAIL_POINTS && corps != nullptr) {
        ...
        for (auto &requete : m_gest_attrs.requetes) {
            ...
            attr = corps->ajoute_attribut(requete->nom, type_attr, dimensions, portee_attr::POINT);
...
bool CompileuseScriptLCC::compile_script(OperatriceImage &op,
                                         Corps &corps,

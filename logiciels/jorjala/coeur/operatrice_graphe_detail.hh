...
class OperatriceGrapheDetail final : public OperatriceCorps {
  ...
  private:
    ...
    void execute_script_sur_points(ChefExecution *chef,
                                   const AccesseusePointLecture &points_entree,
...
class OperatriceFonctionDetail final : public OperatriceImage {
  public:
    ...
    OperatriceFonctionDetail(Graphe &graphe_parent,
                             Noeud &noeud_,
                             dls::chaine const &nom_fonc,
                             lcc::donnees_fonction const *df);
  ...
  private:
    void cree_code_coulisse_processeur(compileuse_lng *compileuse,
                                       lcc::type_var type_specialise,
                                       dls::tableau<int> const &pointeurs);

    void cree_code_coulisse_opengl(DonneesAval *donnees_aval,
                                   lcc::type_var type_specialise,
                                   dls::tableau<int> const &pointeurs,
                                   int temps_courant);
};

...
inline static dls::chaine id_dot_pour_entree(PriseEntree const *prise,
                                             long index,
...
inline static dls::chaine id_dot_pour_sortie(PriseSortie const *prise,
                                             long index,
                                             bool apostrophes = true)
{
    if (index == -1l) {
...
inline void imprime_lien(dls::flux_chaine &flux,
                         ...
                         PriseEntree const *entree)
{
    ...
    flux << "id=\"VAL" << id_dot_pour_noeud(entree->parent, false) << ':'
         << id_dot_pour_entree(entree, -1l, false) << '"';
    flux << ",penwidth=\"" << largeur_stylo << '"';
    flux << "];\n";
    flux << '\n';
}
...
ImprimeuseGraphe::ImprimeuseGraphe(Graphe *graph) : m_graph(graph)
{
}

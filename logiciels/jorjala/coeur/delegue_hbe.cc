...
DeleguePrim::DeleguePrim(const Corps &corps) : m_corps(corps)
{
}
...
DonneesPointPlusProche DeleguePrim::calcule_point_plus_proche(long idx,
                                                              const dls::math::point3d &p) const
{
    auto prim = m_corps.prims()->prim(idx);
    auto points = m_corps.points_pour_lecture();
    ...
    for (auto j = 2; j < poly->nombre_sommets(); ++j) {
        ...
        auto pnt_pls_prch = plus_proche_point_triangle(dls::math::vec3d(p),
                                                       dls::math::vec3d(v0_d),
                                                       dls::math::vec3d(v1_d),
                                                       dls::math::vec3d(v2_d));
...
delegue_arbre_octernaire::delegue_arbre_octernaire(const Corps &c) : corps(c)
{
}
...
limites3f delegue_arbre_octernaire::calcule_limites(long idx) const
{
    ...
    auto limites = limites3f(dls::math::vec3f(constantes<float>::INFINITE),

...
BVHNode *BVHBuild::build_node(const BVHObjectBinning &range, int level)
{
  ...
  if (range.size() < THREAD_TASK_SIZE) {
	...
	task_pool.push([=, this] { thread_build_node(inner, 0, left, level + 1); });
	task_pool.push([=, this] { thread_build_node(inner, 1, right, level + 1); });
  }
                              ...
                              BVHSpatialStorage *storage)
{
  ...
  if (range.size() < THREAD_TASK_SIZE) {
	...
	task_pool.push([=, refs = std::move(left_references), this]() mutable {
      thread_build_spatial_split_node(inner, 0, left, refs, level + 1);
    });
	task_pool.push([=, refs = std::move(right_references), this]() mutable {

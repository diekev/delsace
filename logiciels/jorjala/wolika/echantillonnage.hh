...
inline auto catrom(T p0, T p1, T p2, T p3, T f)
{
    ...
    return MOITIE *
           ((DEUX * p1) + (-p0 + p2) * f + (DEUX * p0 - CINQ * p1 + QUATRE * p2 - p3) * f * f +
...
auto echantillonne_catrom(grille_dense_2d<T> const &grille, float x, float y)
{
    ...
    return catrom(catrom(grille.valeur(i0 + res.x * j0),
                         grille.valeur(i1 + res.x * j0),
                         grille.valeur(i2 + res.x * j0),
                         grille.valeur(i3 + res.x * j0),
                         T(frac_x)),
                  catrom(grille.valeur(i0 + res.x * j1),
                         grille.valeur(i1 + res.x * j1),
                         grille.valeur(i2 + res.x * j1),
                         grille.valeur(i3 + res.x * j1),
                         T(frac_x)),
                  catrom(grille.valeur(i0 + res.x * j2),
                         grille.valeur(i1 + res.x * j2),
                         grille.valeur(i2 + res.x * j2),
                         grille.valeur(i3 + res.x * j2),
                         T(frac_x)),
                  catrom(grille.valeur(i0 + res.x * j3),
                         grille.valeur(i1 + res.x * j3),
                         grille.valeur(i2 + res.x * j3),
                         grille.valeur(i3 + res.x * j3),
                         T(frac_x)),
                  T(frac_y));
}
...
struct Rasteriseur {
  ...
  public:
    explicit Rasteriseur(grille_dense_3d<T> &grille) : m_grille(grille)
    {
    }
    ...
    void ecris_voisin_plus_proche(dls::math::vec3f const &vsp, T valeur) const
    {
        ...
        m_grille.valeur(static_cast<size_t>(dvsp.x),
                        static_cast<size_t>(dvsp.y),
                        static_cast<size_t>(dvsp.z),
                        valeur);
    }
    ...
    void ecris_trilineaire(dls::math::vec3f const &vsp, T valeur) const
    {
        ...
        auto fraction = dls::math::vec3f(1.0f) -
                        (dls::math::vec3f(static_cast<float>(c.x) + 1.0f,
                                          static_cast<float>(c.y) + 1.0f,
...
struct Echantilloneuse {
  ...
  public:
    explicit Echantilloneuse(grille_dense_3d<T> const &grille) : m_grille(grille)
    {
    }
...
template <typename T>
auto reechantillonne(grille_dense_3d<T> const &entree, double taille_voxel)
{

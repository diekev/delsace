		...
		DrapeauxNoeud drapeau)
{
	...
	switch (racine->genre) {
		...
		case GenreNoeud::INSTRUCTION_DISCR_UNION:
		{
			...
			aplatis_arbre(expr->expression_discriminee, arbre_aplatis, DrapeauxNoeud::DROITE_ASSIGNATION);
			arbre_aplatis.ajoute(expr);
	...
	switch (noeud->genre) {
		...
		case GenreNoeud::OPERATEUR_BINAIRE:
		{
			...
			if (expr_bin->op && expr_bin->op->est_arithmetique_pointeur) {
				...
				if (type1->est_pointeur() && type2->est_pointeur()) {
					...
					auto soustraction = assem->cree_expression_binaire(expr_bin->lexeme, type_z64->operateur_sst, comme_type(expr_bin->operande_gauche, type_z64), comme_type(expr_bin->operande_droite, type_z64));
					auto taille_de = assem->cree_litterale_entier(expr_bin->lexeme, type_z64, std::max(type_pointe->taille_octet, 1u));
					auto div = assem->cree_expression_binaire(expr_bin->lexeme, type_z64->operateur_div, soustraction, taille_de);
					...
					auto taille_de = assem->cree_litterale_entier(expr_entier->lexeme, type_entier, std::max(type_pointe->taille_octet, 1u));
					auto mul = assem->cree_expression_binaire(expr_entier->lexeme, type_entier->operateur_mul, expr_entier, taille_de);
					...
					auto arithm = assem->cree_expression_binaire(expr_bin->lexeme, op_arithm, comme_type(expr_pointeur, type_entier), mul);
				...
				if (expr_bin->possede_drapeau(EST_ASSIGNATION_COMPOSEE)) {
					expr_bin->substitution = assem->cree_assignation_variable(expr_bin->lexeme, expr_bin->operande_gauche, expr_bin->substitution);
				}
			...
			if (expr_bin->possede_drapeau(EST_ASSIGNATION_COMPOSEE)) {
				noeud->substitution = assem->cree_assignation_variable(expr_bin->lexeme, expr_bin->operande_gauche, simplifie_operateur_binaire(expr_bin, true));
				return;
			}
		...
		case GenreNoeud::EXPRESSION_REFERENCE_DECLARATION:
		{
			...
			if (decl_ref->drapeaux & EST_CONSTANTE) {
				...
				if (decl_ref->type->est_reel()) {
					expr_ref->substitution = assem->cree_litterale_reel(expr_ref->lexeme, decl_ref->type, decl_const->valeur_expression.reel);
					return;
				}

				expr_ref->substitution = assem->cree_litterale_entier(expr_ref->lexeme, decl_ref->type, static_cast<unsigned long>(decl_const->valeur_expression.entier));
				return;
			}

			if (dls::outils::est_element(decl_ref->genre, GenreNoeud::DECLARATION_ENUM, GenreNoeud::DECLARATION_STRUCTURE)) {
				expr_ref->substitution = assem->cree_reference_type(expr_ref->lexeme, typeuse.type_type_de_donnees(decl_ref->type));
				return;
			}
		...
		case GenreNoeud::EXPRESSION_REFERENCE_MEMBRE:
		{
			...
			if (ref_membre->possede_drapeau(ACCES_EST_ENUM_DRAPEAU)) {
				...
				auto et = assem->cree_expression_binaire(noeud->lexeme, op, accede, valeur_lit_enum);
			...
			if (type_accede->est_tableau_fixe()) {
				...
				noeud->substitution = assem->cree_litterale_entier(noeud->lexeme, noeud->type, static_cast<unsigned long>(taille));
				return;
			}
			...
			if (type_accede->est_enum() || type_accede->est_erreur()) {
				...
				auto valeur_enum = type_enum->membres[ref_membre->index_membre].valeur;
				noeud->substitution = assem->cree_litterale_entier(noeud->lexeme, type_enum, static_cast<unsigned>(valeur_enum));
				return;
			}

			if (type_accede->est_type_de_donnees() && noeud->genre_valeur == GenreValeur::DROITE) {
				noeud->substitution = assem->cree_reference_type(noeud->lexeme, typeuse.type_type_de_donnees(noeud->type));
				return;
			}
		...
		case GenreNoeud::EXPRESSION_TAILLE_DE:
		{
			...
			noeud->substitution = assem->cree_litterale_entier(noeud->lexeme, expr->type, type->taille_octet);
			return;
		}
		case GenreNoeud::EXPRESSION_COMME:
		{
		...
		case GenreNoeud::INSTRUCTION_SI:
		{
			...
			if (si->possede_drapeau(DrapeauxNoeud::DROITE_ASSIGNATION)) {
				...
				auto decl_temp = assem->cree_declaration_variable(si->lexeme, si->type, nullptr, nullptr);
			...
			POUR (assignation->donnees_exprs.plage()) {
				...
				for (auto var : it.variables.plage()) {
					if (var->possede_drapeau(ACCES_EST_ENUM_DRAPEAU)) {
						...
						// À FAIRE : référence
						auto nouvelle_ref = assem->cree_reference_declaration(ref_var->lexeme, ref_var->comme_reference_declaration()->declaration_referee);
						...
						if (it.expression->comme_litterale()->valeur_bool) {
							...
							auto ou = assem->cree_expression_binaire(noeud->lexeme, op, nouvelle_ref, valeur_lit_enum);
							...
							auto valeur_lit_enum = assem->cree_litterale_entier(noeud->lexeme, type_enum, ~static_cast<unsigned>(valeur_enum));
							auto op = type_enum->operateur_etb;
							auto et = assem->cree_expression_binaire(noeud->lexeme, op, nouvelle_ref, valeur_lit_enum);
...
void Simplificatrice::simplifie_boucle_pour(NoeudPour *inst)
{
	...
	auto index_it = idx ? idx->comme_declaration_variable() : assem->cree_declaration_variable(var->lexeme, type_index_it, ident_index_it, zero);
	...
	switch (inst->aide_generation_code) {
		...
		case GENERE_BOUCLE_PLAGE_INDEX:
		{
			...
			condition->condition = assem->cree_expression_binaire(inst->lexeme, op_comp, ref_it, expr_fin);
		...
		case GENERE_BOUCLE_TABLEAU_INDEX:
		{
			...
			if (type_itere->est_tableau_fixe()) {
				...
				expr_taille = assem->cree_litterale_entier(inst->lexeme, typeuse[TypeBase::Z64], static_cast<unsigned long>(taille_tableau));
			}
			else {
				expr_taille = assem->cree_reference_membre(inst->lexeme, expression_iteree, typeuse[TypeBase::Z64], 1);
			}

			auto type_z64 = typeuse[TypeBase::Z64];
			condition->condition = assem->cree_expression_binaire(inst->lexeme, type_z64->operateur_seg, ref_index, expr_taille);
			...
			if (type_itere->est_tableau_fixe()) {
				...
				expr_pointeur = assem->cree_reference_membre(inst->lexeme, expression_iteree, type_compose->membres[0].type, 0);
			}

			NoeudExpression *expr_index = ref_index;

			if (inverse_boucle) {
				expr_index = assem->cree_expression_binaire(inst->lexeme, ref_index->type->operateur_sst, expr_taille, ref_index);
				expr_index = assem->cree_expression_binaire(inst->lexeme, ref_index->type->operateur_sst, expr_index, assem->cree_litterale_entier(ref_index->lexeme, ref_index->type, 1));
			}
			...
			auto assign_it = assem->cree_assignation_variable(inst->lexeme, ref_it, expression_assignee);
		...
		kuri::tableau<NoeudExpressionBinaire> &comparaisons)
{
	...
	if (est_operateur_comp(expr_bin->operande_gauche->lexeme->genre)) {
...
void Simplificatrice::simplifie_retour(NoeudRetour *inst)
{
	...
	POUR (inst->donnees_exprs.plage()) {
		...
		for (auto &var : it.variables.plage()) {
			var = assem->cree_reference_declaration(var->lexeme, var->comme_declaration_variable());
		}
	}
	...
	if (type_sortie->est_rien()) {
		...
		retour->expression = assem->cree_reference_declaration(fonction_courante->param_sortie->lexeme, fonction_courante->param_sortie);
	}
...
NoeudExpression *Simplificatrice::simplifie_operateur_binaire(NoeudExpressionBinaire *expr_bin, bool pour_operande)
{
	if (expr_bin->op->est_basique) {
		 ...
		 * d'assignation. */
		return assem->cree_expression_binaire(expr_bin->lexeme, expr_bin->op, expr_bin->operande_gauche, expr_bin->operande_droite);
	}
...
void Simplificatrice::simplifie_discr_impl(NoeudDiscr *discr)
{
	...
	if (N == DISCR_UNION || N == DISCR_UNION_ANONYME) {
		...
		expression = assem->cree_reference_membre(discr->expression_discriminee->lexeme, discr->expression_discriminee, typeuse[TypeBase::Z32], 1);
	}
	else {
	...
	for (auto i = 0; i < discr->paires_discr.taille(); ++i) {
		...
		for (auto expr : virgule->expressions) {
			...
			if (N == DISCR_ENUM) {
				...
				auto constante = assem->cree_litterale_entier(expr->lexeme, expression->type, static_cast<unsigned long>(valeur));
				...
				auto constante = assem->cree_litterale_entier(expr->lexeme, expression->type, static_cast<unsigned long>(index + 1));
				comparaison.operande_droite = constante;
			}

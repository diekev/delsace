    ...
    switch (noeud->genre) {
        ...
        case GenreNoeud::EXPRESSION_APPEL:
        {
            ...
            if (appel->aide_génération_code == CONSTRUIS_CHAINE) {
                ...
                auto ref_rubrique = assem->crée_référence_rubrique(
                    appel->lexème, ref_temporaire, typeuse.type_ptr_z8, 0);
            ...
            if (appel->noeud_fonction_appelée) {
                ...
                if (appel->expression->est_référence_déclaration()) {
                    if (appel->expression->comme_référence_déclaration()->déclaration_référée !=
                        appel->noeud_fonction_appelée) {
                        appel->expression = assem->crée_référence_déclaration(
                            appel->lexème,
                            appel->noeud_fonction_appelée->comme_déclaration_symbole());
                    }
                }
                else {
        ...
        case GenreNoeud::DÉCLARATION_STRUCTURE:
        {
            ...
            POUR (structure->rubriques) {
                simplifie(it.expression_valeur_défaut);
            }
...
NoeudExpression *Simplificatrice::simplifie_boucle_pour(NoeudPour *inst)
{
    ...
    auto ref_indice = assem->crée_référence_déclaration(it->lexème, indice_it);
    ...
    switch (inst->aide_génération_code) {
        ...
        case GÉNÈRE_BOUCLE_PLAGE_IMPLICITE:
        {
            ...
            condition->condition = assem->crée_expression_binaire(
                inst->lexème, op_comp, ref_indice, nombre_iterations);
            boucle->bloc->expressions->ajoute_au_début(condition);
            ...
            auto inc_it = assem->crée_incrementation(ref_indice->lexème, ref_indice);
            bloc_inc->ajoute_expression(inc_it);
        ...
        case GÉNÈRE_BOUCLE_TABLEAU:
        {
            ...
            if (type_itéré->est_type_tableau_fixe()) {
                auto indexage = assem->crée_indexage(inst->lexème, expression_iteree, zero, true);
            ...
            NoeudExpression *expr_indice = ref_indice;

            if (inverse_boucle) {
                expr_indice = assem->crée_expression_binaire(
                    inst->lexème,
                    ref_indice->type->table_opérateurs->opérateur_sst,
                    expr_taille,
                    ref_indice);
                expr_indice = assem->crée_expression_binaire(
                    inst->lexème,
                    ref_indice->type->table_opérateurs->opérateur_sst,
                    expr_indice,
                    assem->crée_littérale_entier(ref_indice->lexème, ref_indice->type, 1));
            }
    ...
    Lexème const *lexème_référence)
{
    ...
    auto info_rubrique = donne_rubrique_pour_type(type_union, typeuse.type_rien);
    ...
    auto valeur_indice = assem->crée_littérale_entier(
        lexème_référence, typeuse.type_z32, indice_rubrique + 1);

    auto assignation = assem->crée_assignation_variable(
        lexème_référence, ref_rubrique, valeur_indice);
    ...
    switch (type_primitif->genre) {
        ...
        case GenreNoeud::EINI:
        {
            ...
            auto ref_pointeur = assem->crée_référence_rubrique(
                expression->lexème, expression, typeuse.type_ptr_rien, 0);
            auto zéro = assem->crée_littérale_nul(expression->lexème);
            zéro->type = typeuse.type_ptr_rien;
...
NoeudExpression *Simplificatrice::simplifie_tente(NoeudInstructionTente *inst)
{
    ...
    if (expression_tentée->type->est_type_union()) {
        ...
        auto indice_rubrique_erreur = 0u;
        auto indice_rubrique_variable = 1u;
        ...
        if (type_union->rubriques[0].type->est_type_erreur()) {
            ...
            indice_rubrique_erreur = 1u;
            indice_rubrique_variable = 0u;
        }
        ...
        auto accès_rubrique_active = assem->crée_référence_rubrique(
            lexème, référence_erreur, typeuse.type_z32, 1);

        /* si rubrique_active == erreur */
        auto indice = assem->crée_littérale_entier(
            lexème, type_erreur, indice_rubrique_erreur + 1);
        auto op = typeuse.type_z32->table_opérateurs->opérateur_egt;
        assert(op);
        auto comparaison = assem->crée_expression_binaire(
            lexème, op, accès_rubrique_active, indice);
        ...
        extrait_erreur->transformation.indice_rubrique = indice_rubrique_erreur;
        ...
        if (!type_variable->est_type_rien()) {
            ...
            extrait_variable->transformation.indice_rubrique = indice_rubrique_variable;
...
void Simplificatrice::simplifie_position_code_source(NoeudDirectiveIntrospection *directive)
{
    ...
    valeur_chemin_fichier->type = typeuse.type_chaine;
    ...
    auto valeur_colonne = assem->crée_littérale_entier(
        lexème, typeuse.type_z32, static_cast<unsigned>(pos.pos));
    ...
    auto ref_rubrique_fichier = assem->crée_référence_rubrique(
        lexème, ref_position, typeuse.type_chaine, 0);
    auto ref_rubrique_fonction = assem->crée_référence_rubrique(
        lexème, ref_position, typeuse.type_chaine, 1);
    auto ref_rubrique_ligne = assem->crée_référence_rubrique(
        lexème, ref_position, typeuse.type_z32, 2);
    auto ref_rubrique_colonne = assem->crée_référence_rubrique(
        lexème, ref_position, typeuse.type_z32, 3);
    ...
    NoeudExpressionAppel *construction, TypeStructure *type_struct)
{
    ...
    POUR_INDICE (construction->paramètres_résolus) {
        ...
        auto ref_rubrique = assem->crée_référence_rubrique(
            lexème, référence, type_rubrique, indice_it);
 ...
 * chaque structure rencontrée + la rubrique de la structure à l'origine de la rubrique ajouté par
 * emploi.
 *
 * Par exemple, pour :
 *
...
NoeudExpression *Simplificatrice::simplifie_référence_rubrique(
    NoeudExpressionRubrique *ref_rubrique)
{
    ...
    if (ref_rubrique->possède_drapeau(DrapeauxNoeud::ACCES_EST_ENUM_DRAPEAU)) {
        ...
        auto valeur_énum = type_enum->rubriques[ref_rubrique->indice_rubrique].valeur;
    ...
    if (rubrique.est_constant()) {
        if (!rubrique.expression_valeur_défaut) {
            ...
            if (valeur.est_booléenne()) {
                ref_rubrique->substitution = assem->crée_littérale_bool(
                    lexème, typeuse.type_bool, valeur.booléenne());
            }
                ...
                assert_rappel(false, [&]() {
                    dbg() << "La valeur de l'expression de la rubrique constant est invalide\n";
                });
            }
        ...
        simplifie(rubrique.expression_valeur_défaut);
        ref_rubrique->substitution = rubrique.expression_valeur_défaut;
                                                                     ...
                                                                     NoeudExpression *expression)
{
    ...
    auto valeur_énum = type_énum->rubriques[ref_rubrique->indice_rubrique].valeur;
...
static uint64_t valeur_énum(TypeEnum *type_énum, IdentifiantCode *ident)
{
    ...
    return type_énum->rubriques[indice_rubrique].valeur;
}
...
NoeudExpression *Simplificatrice::simplifie_discr_impl(NoeudDiscr *discr)
{
    ...
    if (N == DISCR_UNION || N == DISCR_UNION_ANONYME) {
        ...
        expression = assem->crée_référence_rubrique(
            expression->lexème, expression, typeuse.type_z32, 1);
    }
    ...
    for (auto i = 0; i < discr->paires_discr.taille(); ++i) {
        ...
        for (auto expr : virgule->expressions) {
            ...
            if (N == DISCR_ENUM) {
                ...
                auto indice = donne_rubrique_pour_nom(type_union, expr->ident)->indice_rubrique;
                auto constante = assem->crée_littérale_entier(
                    expr->lexème, expression->type, static_cast<uint64_t>(indice + 1));
                comparaison.opérande_droite = constante;
            }

...
/// same as reset()
Keccak::Keccak(Bits bits) : m_blockSize(static_cast<unsigned>(200 - 2 * (bits / 8))), m_bits(bits)
{
    reset();
}
...
void Keccak::processBlock(const void *data)
{
    ...
    for (unsigned int round = 0; round < KeccakRounds; round++) {
        ...
        for (unsigned int i = 0; i < 5; i++)
            coefficients[i] = m_hash[i] ^ m_hash[i + 5] ^ m_hash[i + 10] ^ m_hash[i + 15] ^
                              m_hash[i + 20];
...
{
    unsigned int blockSize = 200 - 2 * static_cast<unsigned>(m_bits / 8);
...
void Keccak::getHash(char *sortie)
{
    ...
    for (unsigned int i = 0; i < hashLength; i++)
        for (unsigned int j = 0; j < 8; j++)  // 64 bits => 8 bytes
        {
            ...
            *sortie++ = dec2hex[oneByte >> 4];
            *sortie++ = dec2hex[oneByte & 15];
        }

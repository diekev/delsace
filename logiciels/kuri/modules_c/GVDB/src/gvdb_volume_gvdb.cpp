...
void VolumeGVDB::TimerStart()
{
    PERF_START();
}
float VolumeGVDB::TimerStop()
{
    return PERF_STOP();
}
...
VolumeGVDB::VolumeGVDB()
{
    ...
    for (int n = 0; n < 5; n++)
        cuModule[n] = (CUmodule)-1;
    for (int n = 0; n < MAX_FUNC; n++)
        cuFunc[n] = (CUfunction)-1;
    for (int n = 0; n < MAX_CHANNEL; n++) {
        mVDBInfo.volIn[n] = ID_UNDEFL;
        mVDBInfo.volOut[n] = ID_UNDEFL;
    }
...
VolumeGVDB::~VolumeGVDB()
{
    ...
    if (cuVDBInfo != 0) {
        cudaCheck(
            cuMemFree(cuVDBInfo), "VolumeGVDB", "~VolumeGVDB", "cuMemFree", "cuVDBInfo", mbDebug);
    }

    for (int n = 0; n < sizeof(cuModule) / sizeof(cuModule[0]); n++) {
        if (cuModule[n] != (CUmodule)(-1)) {
            cudaCheck(cuModuleUnload(cuModule[n]),
                      "VolumeGVDB",
                      "~VolumeGVDB",
                      "cuModuleUnload",
                      "cuModule[n]",
...
{
    char cptx[512];
    strcpy(cptx, ptx.c_str());
    char cfn[512];
    strcpy(cfn, func.c_str());

    if (cuModule[mid] == (CUmodule)-1)
        cudaCheck(cuModuleLoad(&cuModule[mid], cptx),
                  "VolumeGVDB",
                  "LoadFunction",
                  "cuModuleLoad",
                  cptx,
    ...
    if (cuFunc[fid] == (CUfunction)-1)
        cudaCheck(cuModuleGetFunction(&cuFunc[fid], cuModule[mid], cfn),
                  "VolumeGVDB",
                  "LoadFunction",
                  "cuModuleGetFunction",
                  cfn,
...
void VolumeGVDB::SetCudaDevice(int devid, CUcontext ctx)
{
    ...
    LoadFunction(
        FUNC_SCATTER_DENSITY, "gvdbScatterPointDensity", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    LoadFunction(
        FUNC_SCATTER_AVG_COL, "gvdbScatterPointAvgCol", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    ...
    LoadFunction(
        FUNC_ADD_SUPPORT_VOXEL, "gvdbAddSupportVoxel", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    LoadFunction(
        FUNC_INSERT_SUPPORT_POINTS, "gvdbInsertSupportPoints", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);

    // Topology
    LoadFunction(
        FUNC_FIND_ACTIV_BRICKS, "gvdbFindActivBricks", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    ...
    LoadFunction(
        FUNC_CALC_EXTRA_BRICK_ID, "gvdbCalcExtraBrickId", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);

    LoadFunction(
        FUNC_CALC_INCRE_BRICK_ID, "gvdbCalcIncreBrickId", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    LoadFunction(FUNC_CALC_INCRE_EXTRA_BRICK_ID,
                 "gvdbCalcIncreExtraBrickId",
                 MODL_PRIMARY,
                 CUDA_GVDB_MODULE_PTX);

    LoadFunction(
        FUNC_DELINK_LEAF_BRICKS, "gvdbDelinkLeafBricks", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    ...
    LoadFunction(
        FUNC_INSERT_SUBCELL_FP16, "gvdbInsertSubcell_fp16", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    ...
    LoadFunction(
        FUNC_GATHER_LEVELSET_FP16, "gvdbGatherLevelSet_fp16", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
    ...
    LoadFunction(
        FUNC_UPDATEAPRONFACES_F, "gvdbUpdateApronFacesF", MODL_PRIMARY, CUDA_GVDB_MODULE_PTX);
...
void VolumeGVDB::SetModule(CUmodule module)
{
    ...
    size_t len = 0;
    cudaCheck(cuModuleGetGlobal(&cuScnInfo, &len, module, "scn"),
              "VolumeGVDB",
              "SetModule",
              "cuModuleGetGlobal",
              "cuScnInfo",
    ...
    cudaCheck(cuModuleGetGlobal(&cuXform, &len, module, "cxform"),
              "VolumeGVDB",
              "SetModule",
              "cuModuleGetGlobal",
              "cuXform",
              mbDebug);
    cudaCheck(cuModuleGetGlobal(&cuDebug, &len, module, "cdebug"),
              "VolumeGVDB",
              "SetModule",
              "cuModuleGetGlobal",
              "cuDebug",
...
bool VolumeGVDB::ImportVTK(std::string fname, std::string field, Vector3DI &res)
{
    ...
    while (!feof(fp) && status != 'D') {
        ...
        if (word.compare("DATASET") == 0) {
            word = strSplit(lin, " \n");
            if (word.compare("STRUCTURED_POINTS") == 0)
                continue;
        ...
        if (word.compare("DIMENSIONS") == 0) {
            word = strSplit(lin, " \n");
            strIsNum(word, f);
            res.x = static_cast<int>(f - 1);
            word = strSplit(lin, " \n");
            strIsNum(word, f);
            res.y = static_cast<int>(f - 1);
            word = strSplit(lin, " \n");
            strIsNum(word, f);
            res.z = static_cast<int>(f - 1);
            verbosef("  Res: %d, %d, %d\n", res.x, res.y, res.z);
        ...
        if (word.compare("FIELD") == 0) {
            ...
            word = strSplit(lin, " \n");
            strIsNum(word, f);
            max_cnt = static_cast<int>(f);
            verbosef("  Reading: %s, %ld\n", field.c_str(), max_cnt);
            continue;
        }
        if (word.compare("SPACING") == 0)
            continue;
        if (word.compare("ORIGIN") == 0)
            continue;
        if (word.compare("CELL_DATA") == 0)
            continue;
        if (word.compare("SCALARS") == 0)
            continue;
        if (word.compare("LOOKUP_TABLE") == 0)
            continue;
...
bool VolumeGVDB::LoadVBX(const std::string fname, int force_maj, int force_min)
{
    ...
    for (int n = 0; n < num_grids; n++) {
        ...
        fread(&grid_compress, sizeof(uchar), 1,
              fp);  // grid compression (0=none, 1=blosc, 2=..)
        ...
        fread(&grid_topotype,
              sizeof(uchar),
              1,
              fp);                               // topology type? (0=none, 1=reuse, 2=gvdb, 3=..)
        fread(&grid_reuse, sizeof(int), 1, fp);  // topology reuse
        fread(&grid_layout, sizeof(uchar), 1,
              fp);                              // brick layout? (0=atlas, 1=brick)
        ...
        if (width0[0] != sizeof(nvdb::Node)) {
            gprintf("ERROR: VBX file contains nodes incompatible with current "
                    "gvdb_library.\n");
            gprintf(
                "       Size in file: %d,  Size in library: %d\n", width0[0], sizeof(nvdb::Node));
            gerror();
        }
        ...
        for (int chan = 0; chan < num_chan; chan++) {
            ...
            AddChannel(chan, chan_type, apron, F_LINEAR, F_BORDER,
                       axiscnt);  // provide axiscnt

            mPool->AtlasSetNum(chan,
                               cnt0[0]);  // assumes atlas contains all bricks (all are resident)
            ...
            for (int z = 0; z < axisres.z; z++) {
                ...
                mPool->AtlasWriteSlice(
                    chan,
                    z,
                    static_cast<int>(slice.size),
                    slice.gpu,
                    (uchar *)slice.cpu);  // transfer from GPU, directly into CPU atlas
            }
...
void VolumeGVDB::ClearAtlasAccess()
{
    ...
    for (int chan = 0; chan < num_chan; chan++) {
        if (mVDBInfo.volIn[chan] != ID_UNDEFL) {
            cudaCheck(cuTexObjectDestroy(mVDBInfo.volIn[chan]),
                      "VolumeGVDB",
                      "ClearAtlasAccess",
                      "cuTexObjectDestroy",
                      "volIn",
        ...
        if (mVDBInfo.volOut[chan] != ID_UNDEFL) {
            cudaCheck(cuSurfObjectDestroy(mVDBInfo.volOut[chan]),
                      "VolumeGVDB",
                      "ClearAtlasAccess",
                      "cuSurfObjectDestroy",
                      "volOut",
    ...
    cudaCheck(
        cuCtxSynchronize(), "VolumeGVDB", "ClearAtlasAccess", "cuCtxSynchronize", "", mbDebug);
...
void VolumeGVDB::SetupAtlasAccess()
{
    ...
    for (int chan = 0; chan < num_chan; chan++) {
        ...
        if (atlas.grsc != 0x0) {
            cudaCheck(cuGraphicsMapResources(1, &atlas.grsc, mStream),
                      "VolumeGVDB",
                      "SetupAtlasAccess",
                      "cuGraphicsMapResource",
                      chanmsg,
                      mbDebug);
            cudaCheck(cuGraphicsSubResourceGetMappedArray(&atlas.garray, atlas.grsc, 0, 0),
                      "VolumeGVDB",
                      "SetupAtlasAccess",
        ...
        cudaCheck(cuTexObjectCreate(&mVDBInfo.volIn[chan], &resDesc, &texDesc, NULL),
                  "VolumeGVDB",
                  "SetupAtlasAccess",
                  "cuTexObjectCreate",
                  chanmsg,
                  mbDebug);
        cudaCheck(cuSurfObjectCreate(&mVDBInfo.volOut[chan], &resDesc),
                  "VolumeGVDB",
                  "SetupAtlasAccess",
                  "cuSurfObjectCreate",
                  chanmsg,
        ...
        if (atlas.grsc != 0x0)
            cudaCheck(cuGraphicsUnmapResources(1, &atlas.grsc, mStream),
                      "VolumeGVDB",
                      "SetupAtlasAccess",
...
void VolumeGVDB::FindUniqueBrick(int pNumPnts, int pLevDepth, int &pNumUniqueBrick)
{
    ...
    void *argsFindUnique[5] = {&numPnts,
                               &mAux[AUX_SORTED_LEVXYZ].gpu,
                               &mAux[AUX_MARKER].gpu,
                               &mAux[AUX_UNIQUE_CNT].gpu,
                               &mAux[AUX_LEVEL_CNT].gpu};
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_FIND_UNIQUE], pblks, 1, 1, threads, 1, 1, 0, NULL, argsFindUnique, NULL),
        "VolumeGVDB",
        "FindUniqueBrick",
        "cuLaunch",
        "FUNC_FIND_UNIQUE",
    ...
    cudaCheck(cuMemcpyDtoH(&pNumUniqueBrick, mAux[AUX_UNIQUE_CNT].gpu, sizeof(int)),
              "VolumeGVDB",
              "FindUniqueBrick",
              "cuMemcpyDtoH",
              "AUX_UNIQUE_CNT",
    ...
    void *argsCompactUnique[5] = {&numPnts,
                                  &mAux[AUX_SORTED_LEVXYZ].gpu,
                                  &mAux[AUX_MARKER].gpu,
                                  &mAux[AUX_MARKER_PRESUM].gpu,
                                  &mAux[AUX_UNIQUE_LEVXYZ].gpu};
...
static const int RADIXGROUPS = NUM_THREADS_PER_BLOCK /
                               RADIXTHREADS;  // Number of radix groups per CTA
static const int TOTALRADIXGROUPS = NUM_BLOCKS *
                                    RADIXGROUPS;  // Number of radix groups for each radix
...
static const int PREFIX_NUM_BLOCKS = (PREFIX_NUM_THREADS_PER_SM / PREFIX_NUM_THREADS_PER_BLOCK) *
                                     NUM_SMS;
...
void VolumeGVDB::RadixSortByByte(int pNumPnts, int pLevDepth)
{
    ...
    cudaCheck(cuMemcpyDtoD(mAux[AUX_SORTED_LEVXYZ].gpu,
                           mAux[AUX_BRICK_LEVXYZ].gpu,
...
void VolumeGVDB::ActivateExtraBricksGPU(
    int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos)
{
    ...
    void *argsCalcExtraLevXYZ[11] = {&cuVDBInfo,
                                     &pRadius,
                                     &pNumPnts,
                                     &pRootLev,
                                     &mAux[AUX_RANGE_RES].gpu,
                                     &mAux[AUX_PNTPOS].gpu,
                                     &mAux[AUX_PNTPOS].subdim.x,
                                     &mAux[AUX_PNTPOS].stride,
                                     &pOrig.x,
                                     &mAux[AUX_BRICK_LEVXYZ].gpu,
                                     &mAux[AUX_EXTRA_BRICK_CNT].gpu};
    ...
    cudaCheck(cuMemcpyDtoH(extraLevCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int)),
              "VolumeGVDB",
              "ActivateExtraBricksGPU",
              "cuMemcpyDtoH",
              "AUX_LEVEL_CNT",
    ...
    unsigned short *extraUniBricks = (unsigned short *)malloc(numUniqueExtraBrick * 4 *
                                                              sizeof(unsigned short));
    cudaCheck(cuMemcpyDtoH(extraUniBricks,
                           mAux[AUX_UNIQUE_LEVXYZ].gpu,
                           numUniqueExtraBrick * 4 * sizeof(unsigned short)),
              "VolumeGVDB",
              "ActivateExtraBricksGPU",
              "cuMemcpyDtoH",
              "AUX_UNIQUE_LEVXYZ",
    ...
    for (int lev = 0; lev < pRootLev; lev++) {
        ...
        for (int n = extraLevPrefixSum; n < extraLevCnt[lev] + extraLevPrefixSum; n++) {
            ...
            SetupNode(nodeID,
                      lev,
                      Vector3DI(extraUniBricks[n * 4 + 2] * brickRange.x,
    ...
    for (int lev = pRootLev - 1; lev >= 0; lev--) {
        ...
        cudaCheck(
            cuLaunchKernel(
                cuFunc[FUNC_LINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsLink, NULL),
            "VolumeGVDB",
            "ActivateExtraBricksGPU",
            "cuLaunch",
            "FUNC_LINK_BRICKS",
...
void VolumeGVDB::ActivateIncreBricksGPU(
    int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos, bool bAccum)
{
    if (getNumTotalNodes(0) == 0)
        return;
    ...
    PrepareAux(
        AUX_BRICK_LEVXYZ, pNumPnts * pRootLev * 4 / Incre_ratio, sizeof(unsigned short), true);
    PrepareAux(AUX_EXTRA_BRICK_CNT, 1, sizeof(int), true, true);

    void *argsCalcExtraLevXYZ[12] = {
        &cuVDBInfo,
        &pRadius,
        &pNumPnts,
        &pRootLev,
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_CALC_INCRE_EXTRA_BRICK_ID],
                             pblks,
                             1,
                             1,
                             threads,
                             ...
                             NULL),
              "VolumeGVDB",
              "ActivateIncreBricksGPU",
              "cuLaunch",
              "FUNC_CALC_INCRE_EXTRA_BRICK_ID",
              mbDebug);
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_DELINK_LEAF_BRICKS],
                             pblks,
                             1,
                             1,
                             threads,
              ...
              "ActivateIncreBricksGPU",
              "cuLaunch",
              "FUNC_DELINK_LEAF_BRICKS",
              mbDebug);
    for (int lev = 1; lev < pRootLev; lev++) {
        void *argsDelink[2] = {&cuVDBInfo, &lev};
        cudaCheck(
            cuLaunchKernel(
                cuFunc[FUNC_DELINK_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsDelink, NULL),
            "VolumeGVDB",
            "ActivateIncreBricksGPU",
            "cuLaunch",
            "FUNC_DELINK_BRICKS",
    ...
    for (int ni = 0; ni < mPool->getPoolTotalCnt(0, 0); ni++)
        if (!getNode(0, 0, ni)->mFlags)
            usedNum--;
...
void VolumeGVDB::RebuildTopology(int pNumPnts, float pRadius, Vector3DF pOrig)
{
    ...
    if (mRebuildTopo) {
        ...
        if (pRadius >= 1)
            ActivateExtraBricksGPU(pNumPnts,
                                   pRadius,
                                   pOrig,
                                   depth,
                                   rootPos);  // for radius > 1 and staggered grid
        mRebuildTopo = false;
...
void VolumeGVDB::ActivateBricksGPU(
    int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos)
{
    PUSH_CTX
    ...
    PrepareAux(AUX_BRICK_LEVXYZ,
               pNumPnts * pRootLev * 4,
               sizeof(unsigned short),
               false);  // 4 - lev, x, y, z

    void *argsCalcLevXYZ[8] = {&pNumPnts,
                               &pRootLev,
                               &mAux[AUX_RANGE_RES].gpu,
                               &mAux[AUX_PNTPOS].gpu,
                               &mAux[AUX_PNTPOS].subdim.x,
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_CALC_BRICK_ID], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCalcLevXYZ, NULL),
        "VolumeGVDB",
        "ActivateBricksGPU",
        "cuLaunch",
        "FUNC_CALC_BRICK_ID",
    ...
    cudaCheck(cuMemcpyDtoH(levCnt, mAux[AUX_LEVEL_CNT].gpu, pRootLev * sizeof(int)),
              "VolumeGVDB",
              "ActivateBricksGPU",
              "cuMemcpyDtoH",
              "AUX_LEVEL_CNT",
    ...
    cudaCheck(cuMemcpyDtoH(uniBricks,
                           mAux[AUX_UNIQUE_LEVXYZ].gpu,
                           numUniqueBrick * 4 * sizeof(unsigned short)),
              "VolumeGVDB",
              "ActivateBricksGPU",
              "cuMemcpyDtoH",
              "AUX_UNIQUE_LEVXYZ",
    ...
    for (int lev = 0; lev < pRootLev; lev++) {
        ...
        for (int n = levPrefixSum; n < levCnt[lev] + levPrefixSum; n++) {
            ...
            SetupNode(nodeID,
                      lev,
                      Vector3DI(uniBricks[n * 4 + 2] * brickRange.x,
...
void VolumeGVDB::FindActivBricks(
    int pLev, int pRootlev, int pNumPnts, Vector3DF pOrig, Vector3DI pRootPos)
{
    PUSH_CTX
    ...
    void *args[11] = {&pNumPnts,
                      ...
                      &pOrig.x,
                      &orig_shift,
                      &mAux[AUX_PBRICKDX].gpu};

    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_FIND_ACTIV_BRICKS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "ActivateBricksGPU",
              "cuLaunch",
              "FUNC_FIND_ACTIV_BRICKS",
              mbDebug);

    // retrieve data
    int *brickIdx = new int[pNumPnts];
    cudaCheck(cuMemcpyDtoH(brickIdx, mAux[AUX_PBRICKDX].gpu, pNumPnts * sizeof(int)),
              "VolumeGVDB",
              "ActivateBricksGPU",
              "cuMemcpyDtoH",
              "AUX_PBRICKDX",
    ...
    cudaCheck(cuMemcpyDtoH(pos, mAux[AUX_PNTPOS].gpu, pNumPnts * sizeof(Vector3DF)),
              "VolumeGVDB",
              "ActivateBricksGPU",
              "cuMemcpyDtoH",
              "AUX_PNTPOS",
    ...
    for (int n = 0; n < numActivatingBrick; n++) {
        ...
        SetupNode(nodeID,
                  pLev - 1,
                  Vector3DI(activatingBrickIdx[n] % dim * brickRange.x,
...
void VolumeGVDB::SaveVBX(const std::string fname)
{
...
#endif
    if (major >= 2) {
        fwrite(&use_bitmask, sizeof(uchar), 1,
               fp);  // bitmask usage (GVDB 2.0 or higher)
    }

    //--- grid offset table
    const long grid_table = ftell(fp);  // position of grid table in file
    fwrite(grid_offs.data(),
           sizeof(uint64),
           grid_offs.size(),
           fp);  // grid offsets (populated later)
    ...
    for (int n = 0; n < num_grids; n++) {
        ...
        fwrite(&grid_compress, sizeof(uchar), 1,
               fp);  // grid compression (0=none, 1=blosc, 2=..)
        ...
        fwrite(&grid_topotype,
               sizeof(uchar),
               1,
               fp);                               // topology type? (0=none, 1=reuse, 2=gvdb, 3=..)
        fwrite(&grid_reuse, sizeof(int), 1, fp);  // topology reuse
        fwrite(&grid_layout, sizeof(uchar), 1,
               fp);                              // brick layout? (0=atlas, 1=brick)
    ...
    fwrite(grid_offs.data(), sizeof(uint64), grid_offs.size(),
           fp);  // grid offsets
...
void VolumeGVDB::SetBounds(Vector3DF pMin, Vector3DF pMax)
{
    ...
    if (mVoxRes.x > mVoxResMax.x)
        mVoxResMax.x = mVoxRes.x;
    if (mVoxRes.y > mVoxResMax.y)
        mVoxResMax.y = mVoxRes.y;
    if (mVoxRes.z > mVoxResMax.z)
        mVoxResMax.z = mVoxRes.z;
}
...
void VolumeGVDB::ComputeBounds()
{
    ...
    for (int n = 0; n < mPool->getPoolTotalCnt(0, 0); n++) {
        curr = getNode(0, 0, n);
        if (!curr->mFlags)
            continue;  // inactivated, skip

        if (curr->mPos.x < mVoxMin.x)
            mVoxMin.x = static_cast<float>(curr->mPos.x);
        if (curr->mPos.y < mVoxMin.y)
            mVoxMin.y = static_cast<float>(curr->mPos.y);
        if (curr->mPos.z < mVoxMin.z)
            mVoxMin.z = static_cast<float>(curr->mPos.z);
        if (curr->mPos.x + range.x > mVoxMax.x)
            mVoxMax.x = static_cast<float>(curr->mPos.x + range.x);
        if (curr->mPos.y + range.y > mVoxMax.y)
            mVoxMax.y = static_cast<float>(curr->mPos.y + range.y);
        if (curr->mPos.z + range.z > mVoxMax.z)
            mVoxMax.z = static_cast<float>(curr->mPos.z + range.z);
    }
    mObjMin = mVoxMin;
    mObjMax = mVoxMax;
    mVoxRes = mVoxMax;
...
void vdbSkip(typename GridType::Ptr &grid,
             typename GridType::TreeType::LeafCIter &iter,
             int leaf_start)
{
...
bool VolumeGVDB::LoadVDBInternal(openvdb::GridBase::Ptr &baseGrid)
{
    ...
    const Vector3DF voxelsize = Vector3DF(static_cast<float>(grid->voxelSize().x()),
    ...
    for (leaf_max = 0; iterator.test();) {
        ...
        if (p0.x > vclipmin.x && p0.y > vclipmin.y && p0.z > vclipmin.z && p0.x < vclipmax.x &&
            p0.y < vclipmax.y && p0.z < vclipmax.z) {
            if (leaf_max == 0) {
                mVoxMin = p0;
                mVoxMax = p0;
            }
    ...
    for (int n = leaf_max = 0; iterator.test(); n++) {
        ...
        if (p0.x > vclipmin.x && p0.y > vclipmin.y && p0.z > vclipmin.z && p0.x < vclipmax.x &&
            p0.y < vclipmax.y && p0.z < vclipmax.z) {  // accept condition
    ...
    for (int leaf_cnt = 0; iterator.test();) {
        ...
        if (p0.x > vclipmin.x && p0.y > vclipmin.y && p0.z > vclipmin.z && p0.x < vclipmax.x &&
            ...
            if (isFloat) {
                ...
                src = ConvertToScalar(
                    res0 * res0 * res0, (float *)leafBuffer.data(), srcLengths, mValMin, mValMax);
            }
            ...
            // Progress percent
            leaf_cnt++;
            percent = int(leaf_cnt * 100 / leaf_max);
...
bool VolumeGVDB::LoadVDB(std::string fname)
{
    ...
    for (openvdb::io::File::NameIterator nameIter = vdbfile->beginName();
         nameIter != vdbfile->endName();
         ++nameIter) {
        verbosef("   Grid: %s\n", nameIter.gridName().c_str());
        if (nameIter.gridName().compare(getScene()->mVName) == 0)
            name = getScene()->mVName;
    }
...
void VolumeGVDB::SaveVDB(std::string fname)
{
    ...
    if (logRes == 3) {
        ...
        gprintf("ERROR: Unable to save .vdb file. No configurations available for "
                "given log res.\n");
    }
    return;
#else
...
void VolumeGVDB::Initialize()
{
    ...
    // Create VDB object
    cudaCheck(cuMemAlloc(&cuVDBInfo, sizeof(VDBInfo)),
              "VolumeGVDB",
              "Initialize",
              "cuMemAlloc",
              "cuVDBInfo",
    ...
    mScene->LinearTransferFunc(
        0, 1, Vector4DF(0, 0, 0, 0), Vector4DF(1, 1, 1, 0.1f));  // Default transfer function
...
void VolumeGVDB::AddChannel(
    uchar chan, int dt, int apron, int filter, int border, Vector3DI axiscnt)
{
    PUSH_CTX

    if (axiscnt.x == 0 && axiscnt.y == 0 && axiscnt.z == 0) {
        if (chan == 0)
            axiscnt = mDefaultAxiscnt;
        else
            axiscnt = mPool->getAtlas(0).subdim;
    }
    mApron = apron;

    mPool->AtlasCreate(
        chan, dt, getRes3DI(0), axiscnt, apron, sizeof(AtlasNode), false, mbUseGLAtlas);
...
void VolumeGVDB::FillChannel(uchar chan, Vector4DF val)
{
    ...
    if (val.x == 0 && val.y == 0 && val.z == 0 && val.w == 0) {
        ...
        switch (mPool->getAtlas(chan).type) {
            case T_FLOAT:
                Compute(FUNC_FILL_F, chan, 1, val, false, false);
                break;
            case T_UCHAR:
                Compute(FUNC_FILL_C, chan, 1, val, false, false);
                break;
            case T_UCHAR4:
                Compute(FUNC_FILL_C4, chan, 1, val, false, false);
                break;
        };
    }
...
{
    // This function ensures that the atlas mapping, for unused bricks in the
    // atlas, maps to an undefined value which is checked by kernels.
...
void VolumeGVDB::UpdateAtlas()
{
    ...
    for (uint64 n = 0; n < totalLeafcnt; n++) {
        ...
        if (!node->mFlags)
            continue;
        if (node->mValue.x > atlasmax.x || node->mValue.y > atlasmax.y ||
            node->mValue.z > atlasmax.z) {
            gprintf("ERROR: Node value exceeds atlas res. node: %d, val: %d %d %d, "
                    "atlas: %d %d %d\n",
                    n,
                    node->mValue.x,
                    node->mValue.y,
...
slong VolumeGVDB::Reparent(int lev, slong prevroot_id, Vector3DI pos, bool &bNew)
{
    ...
    ActivateSpace(newroot_id, prevroot_pos, bn,
                  prevroot_id);  // use stopnode to connect paths

    // insert new pos into new root
    uint64 leaf_id = ActivateSpace(
        newroot_id, pos, bNew);  // new path may create new leaf (return bNew)
...
slong VolumeGVDB::ActivateSpace(Vector3DF pos)
{
    ...
    if (node_id == ID_UNDEFL)
        return ID_UNDEFL;
    if (!bnew)
        return node_id;  // exiting node. return
    return ID_UNDEFL;
}
...
slong VolumeGVDB::ActivateSpace(
    slong nodeid, Vector3DI pos, bool &bNew, slong stopnode, int stoplev)
{
    Vector3DI p;
    Vector3DI range;
    slong childid;
    ...
    if (getPosInNode(nodeid, pos, b)) {
        ...
        if (stopnode != ID_UNDEFL) {
            ...
            if (pos.x == sp.x && pos.y == sp.y && pos.z == sp.z && !isOn(nodeid, b) &&
                curr->mLev == stopn->mLev + 1) {
                // if same position as stopnode, bit is not on,
                return InsertChild(nodeid, stopnode, b);
            }
        }
        // check stop level
        if (curr->mLev == stoplev)
            return nodeid;
...
void VolumeGVDB::DebugNode(slong nodeid)
{
    ...
    if (curr->mLev < 4 && curr->mParent != ID_UNDEFL) {
        ...
        Vector3DI p = curr->mPos;
        p -= getNode(curr->mParent)->mPos;
        p *= getRes3DI(curr->mLev + 1);
        p /= range;
...
Node *VolumeGVDB::getChild(Node *curr, uint ndx)
{
    if (curr->mChildList == ID_UNDEFL)
        return 0x0;
    uint64 *clist = mPool->PoolData64(curr->mChildList);
    uint64 *cnext = (uint64 *)mPool->PoolData(
        ElemGrp(curr->mChildList), ElemLev(curr->mChildList), ElemNdx(curr->mChildList) + 1);
    uint64 ch;
#ifdef USE_BITMASKS
    ch = *(clist + ndx);
#else
    ...
    for (; sum < ndx && vox < getVoxCnt(curr->mLev); vox++) {
        if (*(clist + vox) != ID_UNDEF64)
            sum++;
    }
    if (vox >= getVoxCnt(curr->mLev))
        return 0x0;
    ch = *(clist + (vox - 1));
#endif
    return getNode(ch);
    ...
    FILE *fp, unsigned char vpix[], slong &numfaces, int gv[], int *&vgToVert, slong vbase)
{
    ...
    if (vpix[1] == 0) {
        fprintf(fp, "f %lld//0 %lld//0 %lld//0 %lld//0\n", v[1], v[2], v[6], v[5]);
        numfaces++;
    }  // x+
    if (vpix[2] == 0) {
        fprintf(fp, "f %lld//1 %lld//1 %lld//1 %lld//1\n", v[0], v[3], v[7], v[4]);
        numfaces++;
    }  // x-
    if (vpix[3] == 0) {
        fprintf(fp, "f %lld//2 %lld//2 %lld//2 %lld//2\n", v[2], v[3], v[7], v[6]);
        numfaces++;
    }  // y+
    if (vpix[4] == 0) {
        fprintf(fp, "f %lld//3 %lld//3 %lld//3 %lld//3\n", v[1], v[0], v[4], v[5]);
        numfaces++;
    }  // y-
    if (vpix[5] == 0) {
        fprintf(fp, "f %lld//4 %lld//4 %lld//4 %lld//4\n", v[4], v[5], v[6], v[7]);
        numfaces++;
    }  // z+
    if (vpix[6] == 0) {
        fprintf(fp, "f %lld//5 %lld//5 %lld//5 %lld//5\n", v[0], v[1], v[2], v[3]);
        numfaces++;
    }  // z-
}
...
{
    if (vgToVert[gv[0]] == -1) {
        vgToVert[gv[0]] = (int)verts.size();
        verts.push_back(vm);
    }
    if (vgToVert[gv[1]] == -1) {
        vgToVert[gv[1]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(1, 0, 0));
    }
    if (vgToVert[gv[2]] == -1) {
        vgToVert[gv[2]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(1, 1, 0));
    }
    if (vgToVert[gv[3]] == -1) {
        vgToVert[gv[3]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(0, 1, 0));
    }
    if (vgToVert[gv[4]] == -1) {
        vgToVert[gv[4]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(0, 0, 1));
    }
    if (vgToVert[gv[5]] == -1) {
        vgToVert[gv[5]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(1, 0, 1));
    }
    if (vgToVert[gv[6]] == -1) {
        vgToVert[gv[6]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(1, 1, 1));
    }
    if (vgToVert[gv[7]] == -1) {
        vgToVert[gv[7]] = (int)verts.size();
        verts.push_back(vm + Vector3DF(0, 1, 1));
    }
}
...
void VolumeGVDB::AuxGeometryMap(Model *model, int vertaux, int elemaux)
{
    ...
    cudaCheck(cuGraphicsGLRegisterBuffer(
                  &vaux->grsc, model->vertBufferID, CU_GRAPHICS_MAP_RESOURCE_FLAGS_READ_ONLY),
              "VolumeGVDB",
              "AuxGeometryMap",
              "cuGraphicsGLRegisterBuffer",
              "vaux",
              mbDebug);
    cudaCheck(cuGraphicsMapResources(1, &vaux->grsc, 0),
              "VolumeGVDB",
              "AuxGeometryMap",
              "cuGraphicsMapResources",
              "vaux",
              mbDebug);
    cudaCheck(cuGraphicsResourceGetMappedPointer(&vaux->gpu, &vsize, vaux->grsc),
              "VolumeGVDB",
              "AuxGeometryMap",
    ...
    cudaCheck(cuGraphicsGLRegisterBuffer(
                  &eaux->grsc, model->elemBufferID, CU_GRAPHICS_MAP_RESOURCE_FLAGS_READ_ONLY),
              "VolumeGVDB",
              "AuxGeometryMap",
              "cuGraphicsGLRegisterBuffer",
              "eaux",
              mbDebug);
    cudaCheck(cuGraphicsMapResources(1, &eaux->grsc, 0),
              "VolumeGVDB",
              "AuxGeometryMap",
              "cuGraphicsMapResources",
              "eaux",
              mbDebug);
    cudaCheck(cuGraphicsResourceGetMappedPointer(&eaux->gpu, &esize, eaux->grsc),
              "VolumeGVDB",
              "AuxGeometryMap",
              "cuGraphicsResourceGetMappedPointer",
              "eaux",
...
void VolumeGVDB::AuxGeometryUnmap(Model *model, int vertaux, int elemaux)
{
    ...
    cudaCheck(cuGraphicsUnmapResources(1, &vaux->grsc, 0),
              "VolumeGVDB",
              "AuxGeometryUnmap",
              "cuGraphicsUnmapResources",
              "vaux",
              mbDebug);
    cudaCheck(cuGraphicsUnmapResources(1, &eaux->grsc, 0),
              "VolumeGVDB",
              "AuxGeometryUnmap",
              "cuGraphicsUnmapResources",
              "eaux",
    ...
    for (int z = e.imin.z; z <= e.imax.z; z++)
        for (int y = e.imin.y; y <= e.imax.y; y++)
            for (int x = e.imin.x; x <= e.imax.x; x++) {
                ...
                if (!isActive(pos)) {
                    actv = 0;
                    nbr = pos + Vector3DF(-1, 0, 0) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    nbr = pos + Vector3DF(1, 0, 0) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    nbr = pos + Vector3DF(0, -1, 0) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    nbr = pos + Vector3DF(0, 1, 0) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    nbr = pos + Vector3DF(0, 0, -1) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    nbr = pos + Vector3DF(0, 0, 1) * e.cover;
                    if (isActive(nbr))
                        actv++;
                    if (actv > 0) {
                        tagbuf[((z - e.imin.z) * e.ires.y + (y - e.imin.y)) * e.ires.x +
                               (x - e.imin.x)] = 1;
                    }
                }
            }
    ...
    for (int z = e.imin.z; z <= e.imax.z; z++)
        for (int y = e.imin.y; y <= e.imax.y; y++)
            for (int x = e.imin.x; x <= e.imax.x; x++) {
                pos.Set(x, y, z);
                pos *= e.cover;
                if (tagbuf[((z - e.imin.z) * e.ires.y + (y - e.imin.y)) * e.ires.x +
                           (x - e.imin.x)] > 0) {
        ...
        case T_UCHAR:
        {
            for (int z = e.imin.z; z <= e.imax.z; z++)
                for (int y = e.imin.y; y <= e.imax.y; y++)
                    for (int x = e.imin.x; x <= e.imax.x; x++) {
                        uchar vset = *(vdat +
                                       ((z - e.imin.z) * e.ires.y + (y - e.imin.y)) * e.ires.x +
                                       (x - e.imin.x));
                        if (vset > vthresh) {
                            pos.Set(x, y, z);
                            pos *= e.cover;
...
void VolumeGVDB::CheckData(std::string msg, CUdeviceptr ptr, int dt, int stride, int cnt)
{
    ...
    cudaCheck(cuMemcpyDtoH(dat, ptr, stride * cnt),
              "VolumeGVDB",
              "CheckData",
              "cuMemcpyDtoH",
              "",
...
int VolumeGVDB::VoxelizeNode(Node *node,
                             uchar chan,
                             Matrix4F *xform,
                             float bdiv,
                             float val_surf,
                             ...
                             float vthresh)
{
    ...
    PrepareAux(AUX_VOXELIZE,
               e.icnt,
               mPool->getSize(dt),
               true,
               true);  //   Prepare buffer for data retrieve

    Vector3DI block(8, 8, 8);
    Vector3DI grid(
        int(e.ires.x / block.x) + 1, int(e.ires.y / block.y) + 1, int(e.ires.z / block.z) + 1);
    int bmax = static_cast<int>(mAux[AUX_GRIDOFF].lastEle);
    void *args[12] = {&e.vmin,
                      &e.vmax,
                      &e.ires,
                      &mAux[AUX_VOXELIZE].gpu,
                      &dt,
                      &val_surf,
                      &val_inside,
                      &bdiv,
                      &bmax,
                      &mAux[AUX_GRIDCNT].gpu,
                      &mAux[AUX_GRIDOFF].gpu,
                      &mAux[AUX_TRI_BUF].gpu};

    cudaCheck(cuLaunchKernel(cuFunc[FUNC_VOXELIZE],
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
    ...
    mV3D->SurfaceVoxelizeFastGL(e.vmin, e.vmax,
                                xform);  // Voxelize with graphics pipeline
    if (node->mLev == 0) {               // Generate voxels in a brick..
        mPool->AtlasCopyTex(chan, node->mValue,
                            mV3D->getPtr());  //   Copy voxels into GVDB atlas
        cudaCheck(cuCtxSynchronize(),
                  "sync(AtlasCopyData)");  //   Must sync after CUDA copy
    }
        ...
        PrepareAux(AUX_VOXELIZE,
                   ...
                   mPool->getSize(dt),
                   true,
                   true);                          //   Prepare CPU buffer for data retrieve
        mV3D->RetrieveGL(mAux[AUX_VOXELIZE].cpu);  //   Retrieve into CPU buffer
        cnt = ActivateRegionFromAux(e, AUX_VOXELIZE,
                                    dt);  //   Activate nodes at next level down
    }
#endif
...
void VolumeGVDB::SolidVoxelize(
    uchar chan, Model *model, Matrix4F *xform, float val_surf, float val_inside, float vthresh)
{
    PUSH_CTX

    // TimerStart();

    AuxGeometryMap(model, AUX_VERTEX_BUF,
                   AUX_ELEM_BUF);  // Setup VBO for CUDA (interop)

    // Prepare model geometry for use by CUDA
    cudaCheck(cuMemcpyHtoD(cuXform, xform->GetDataF(), sizeof(float) * 16),
              "VolumeGVDB",
              "SolidVoxelize",
              "cuMemcpyHtoD",
              "cuXform",
              mbDebug);  // Send transform

    // Identify model bounding box
    model->ComputeBounds(*xform, 0.1f);
    mObjMin = model->objMin;
    mObjMax = model->objMax;
    mVoxMin = mObjMin;
    mVoxMax = mObjMax;
    mVoxRes = mVoxMax;
    mVoxRes -= mVoxMin;
    ...
    for (int lev = N - 1; lev >= 0; lev--) {  // scan - level N-1 down to 0
        ...
        for (int n = 0; n < node_cnt; n++) {  // get each node at current
            ...
            cnt += VoxelizeNode(node,
                                chan,
                                xform,
                                ydiv,
                                val_surf,
                                val_inside,
                                vthresh);  // Voxelize each node
        }
...
Vector3DI VolumeGVDB::InsertTriangles(Model *model, Matrix4F *xform, float &ydiv)
{
    ...
    PrepareAux(AUX_GRIDCNT, ybins, sizeof(uint), true,
               true);  // allow return to cpu
    ...
    void *args[7] = {&ydiv,
                     ...
                     &vcnt,
                     &ecnt,
                     &mAux[AUX_VERTEX_BUF].gpu,
                     &mAux[AUX_ELEM_BUF].gpu};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_INSERT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "InsertTriangles",
        "cuLaunch",
        "FUNC_INSERT_TRIS",
    ...
    for (int n = 0; n < ybins; n++)
        tri_cnt += cnt[n];  // get total - should return from prefixsum

    // Reset grid counts
    cudaCheck(cuMemsetD8(mAux[AUX_GRIDCNT].gpu, 0, sizeof(uint) * ybins),
              "VolumeGVDB",
              "InsertTriangles",
              "cuMemsetD8",
              "AUX_GRIDCNT",
    ...
    void *args2[10] = {&ydiv,
                       ...
                       &mAux[AUX_ELEM_BUF].gpu};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_SORT_TRIS], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args2, NULL),
        "VolumeGVDB",
        "InsertTriangles",
        "cuLaunch",
        "FUNC_SORT_TRIS",
    ...
    return Vector3DI(ybins,
                     tri_cnt,
                     ecnt);  // return: number of bins, total inserted tris, original tri count
}
...
void VolumeGVDB::SurfaceVoxelizeGL(uchar chan, Model *model, Matrix4F *xform)
{
    ...
    for (int z1 = vstart.z; z1 <= vstop.z; z1++) {
        for (int y1 = vstart.y; y1 <= vstop.y; y1++) {
            for (int x1 = vstart.x; x1 <= vstop.x; x1++) {
                ...
                vmin1 = Vector3DI(x1 + 0, y1 + 0, z1 + 0);
                vmin1 *= vrange1;
                vmax1 = Vector3DI(x1 + 1, y1 + 1, z1 + 1);
                vmax1 *= vrange1;
                    ...
                    for (int y0 = 0; y0 < res1; y0++) {
                        for (int x0 = 0; x0 < res1; x0++) {
                            ...
                            if (vpix[0] > 0) {
                                ...
                                vpix[1] = static_cast<uchar>(
                                    (x0 == res1 - 1) ?
                                        0 :
                                        *(vdat + (z0 * res1 + y0) * res1 + (x0 + 1)));
                                vpix[2] = static_cast<uchar>(
                                    (y0 == res1 - 1) ?
                                        0 :
                                        *(vdat + (z0 * res1 + (y0 + 1)) * res1 + x0));
                                vpix[3] = static_cast<uchar>(
                                    (z0 == res1 - 1) ?
                                        0 :
                                        *(vdat + ((z0 + 1) * res1 + y0) * res1 + x0));
                                vpix[4] = static_cast<uchar>(
                                    (x0 == 0) ? 0 : *(vdat + (z0 * res1 + y0) * res1 + (x0 - 1)));
                                vpix[5] = static_cast<uchar>(
                                    (y0 == 0) ? 0 : *(vdat + (z0 * res1 + (y0 - 1)) * res1 + x0));
                                vpix[6] = static_cast<uchar>(
                                    (z0 == 0) ? 0 : *(vdat + ((z0 - 1) * res1 + y0) * res1 + x0));
                                vpix[7] = (vpix[1] > 0) + (vpix[2] > 0) + (vpix[3] > 0) +
                                          (vpix[4] > 0) + (vpix[5] > 0) + (vpix[6] > 0);
    ...
    cudaCheck(
        cuCtxSynchronize(), "VolumeGVDB", "SurfaceVoxelizeGL", "cuCtxSynchronize", "", mbDebug);
    ...
    if (chan >= mPool->getNumAtlas()) {
        gprintf("  ERROR: Channel %d not defined for SurfaceVoxelizeGL. Call "
                "AddChannel first.\n",
                (int)chan);
        free(vdat);
        return;
...
Vector3DF VolumeGVDB::MemoryUsage(std::string name, std::vector<std::string> &outlist)
{
    ...
    for (int n = 0; n < mPool->getNumAtlas(); n++) {
        ...
        sprintf(str,
                "%s, Voxel Channel %d: %6.2f MB (%4.2f%%)\n",
                name.c_str(),
                n,
                curr,
                float(curr * 100.0 / mem.x));
        outlist.push_back(str);
        vol_total += curr;
        vol_dense += (mVoxResMax.x * mVoxResMax.y * mVoxResMax.z) * bpv;
    }
    sprintf(str,
            "%s, Volume Total: %6.2f MB (%4.2f%%)\n",
            name.c_str(),
            vol_total,
            float(vol_total * 100.0 / mem.x));
    outlist.push_back(str);
    ...
    sprintf(str,
            "%s, Aux Total: %6.2f MB (%4.2f%%)\n",
            name.c_str(),
            aux_total,
            float(aux_total * 100.0 / mem.x));
    outlist.push_back(str);
...
void VolumeGVDB::getUsage(Vector3DF &ext, Vector3DF &vox, Vector3DF &used, Vector3DF &free)
{
    ...
    if (mPool->getNumAtlas() > 0) {
        ...
        vox = Vector3DF(abrk,
                        abrk * leafdim * leafdim * leafdim / 1000000.0f,
                        abrk * 100.0f / vbrk);  // # bricks, # voxels(M), occupancy(%)
    }
    else {
        vox = Vector3DF(0, 0, 0);
    }
    ...
    for (int ai = 0; ai < MAX_AUX; ai++) {
        if (mAux[ai].gpu != 0x0)
            aux_total += float(mAux[ai].size) / MB;
    }
...
void VolumeGVDB::Measure(bool bPrint)
{
    ...
    if (bPrint) {
        ...
        if (mPool->getNumAtlas() > 0) {
            ...
            gprintf("   Atlas Res:     %d x %d x %d  LeafCnt: %d x %d x %d  LeafDim: "
                    "%d^3\n",
                    axisres.x,
                    axisres.y,
                    axisres.z,
            ...
            gprintf("   Vol Extents:   %d bricks,  %5.2f million voxels\n",
                    vbrk,
                    float(vbrk) * leafdim * leafdim * leafdim / 1000000.0f);
            gprintf("   Atlas Storage: %d bricks,  %5.2f million voxels\n",
                    sbrk,
                    float(sbrk) * leafdim * leafdim * leafdim / 1000000.0f);
            gprintf("   Atlas Active:  %d bricks,  %5.2f million voxels\n",
                    abrk,
                    float(abrk) * leafdim * leafdim * leafdim / 1000000.0f);
        ...
        for (int n = 0; n < mPool->getNumAtlas(); n++) {
            ...
            gprintf("     Channel %d:       %6.2f MB (%d bytes/vox)\n",
                    n,
                    float(mPool->getAtlas(n).size) / MB,
                    bpv);
            vol_total += mPool->getAtlas(n).size;
            vol_dense += (mVoxResMax.x * mVoxResMax.y * mVoxResMax.z) * bpv;
        }
        gprintf(
            "   Atlas Total:       %6.2f MB (Dense: %10.2f MB)\n", vol_total / MB, vol_dense / MB);
    }
    POP_CTX
}
...
void VolumeGVDB::PrepareVDB()
{
    ...
    if (mVDBInfo.update) {
        ...
        for (int n = levs - 1; n >= 0; n--) {
            ...
            mVDBInfo.vdel[n] = Vector3DF(getRange(n));
            mVDBInfo.vdel[n] /= Vector3DF(getRes3DI(n));
            ...
            if (mVDBInfo.nodecnt[n] == 1)
                tlev = n;  // get top level for rendering
        }
        ...
        mVDBInfo.brick_res = mPool->getAtlasBrickres(
            0);  // resolution of single brick (sides are all same)
        ...
        PUSH_CTX
        cudaCheck(cuMemcpyHtoD(cuVDBInfo, &mVDBInfo, sizeof(VDBInfo)),
                  "VolumeGVDB",
                  "PrepareVDB",
                  "cuMemcpyHtoD",
                  "cuVDBInfo",
...
void VolumeGVDB::WriteDepthTexGL(int chan, int glid)
{
    ...
    // Setup frame buffer so that we can copy depth info from the depth target
    // bound
...
void VolumeGVDB::WriteRenderTexGL(int chan, int glid)
{
    ...
    if (mRenderBuf[chan].garray == 0) {
        ...
        cudaCheck(cuGraphicsGLRegisterImage(&mRenderBuf[chan].grsc,
                                            glid,
                                            GL_TEXTURE_2D,
                  ...
                  "mRenderBuf",
                  mbDebug);
        cudaCheck(cuGraphicsMapResources(1, &mRenderBuf[chan].grsc, 0),
                  "VolumeGVDB",
                  "WriteRenderTexGL",
                  "cuGraphicsMapResources",
                  "mRenderBuf",
                  mbDebug);
        cudaCheck(cuGraphicsSubResourceGetMappedArray(
                      &mRenderBuf[chan].garray, mRenderBuf[chan].grsc, 0, 0),
                  "VolumeGVDB",
                  "WriteRenderTexGL",
                  "cuGraphicsSubResourceGetMappedArray",
                  "mRenderBuf",
                  mbDebug);
        cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0),
                  "VolumeGVDB",
                  "WriteRenderTexGL",
                  "cuGraphicsUnmapResources",
                  "mRenderBuf",
...
void VolumeGVDB::ReadRenderTexGL(int chan, int glid)
{
    ...
    if (mRenderBuf[chan].garray == 0) {
        ...
        cudaCheck(cuGraphicsSubResourceGetMappedArray(
                  ...
                  "VolumeGVDB",
                  "ReadRenderTexGL",
                  "cuGraphicsSubResourcesGetMappedArray",
                  "mRenderBuf",
                  mbDebug);
        cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0),
                  "VolumeGVDB",
...
void VolumeGVDB::ResizeDepthBuf(int chan, int width, int height)
{
    ...
    cudaCheck(cuGraphicsGLRegisterBuffer(&mRenderBuf[chan].grsc, mRenderBuf[chan].glid, 0),
              "VolumeGVDB",
              "ResizeDepthBuf",
              "cuGraphicsGLRegisterBuffer",
              "",
    ...
    cudaCheck(cuGraphicsMapResources(1, &mRenderBuf[chan].grsc, 0),
              ...
              "",
              mbDebug);
    cudaCheck(cuGraphicsResourceGetMappedPointer(
                  &mRenderBuf[chan].gpu, &mRenderBuf[chan].size, mRenderBuf[chan].grsc),
              "VolumeGVDB",
              "ResizeDepthBuf",
              "cuGraphicsResourcesGetMappedPointer",
              "",
              mbDebug);
    cudaCheck(cuGraphicsUnmapResources(1, &mRenderBuf[chan].grsc, 0),
              "VolumeGVDB",
...
void VolumeGVDB::ResizeRenderBuf(int chan, int width, int height, int byteperpix)
{
    ...
    mRenderBuf[chan].stride = width;
    if (chan == 0)
        getScene()->SetRes(width, height);

    size_t sz = mRenderBuf[chan].size;
    if (mRenderBuf[chan].gpu != 0x0) {
        cudaCheck(cuMemFree(mRenderBuf[chan].gpu),
                  "VolumeGVDB",
                  "ResizeRenderBuf",
                  "cuMemFree",
                  "",
    ...
    cudaCheck(cuMemAlloc(&mRenderBuf[chan].gpu, sz),
              "VolumeGVDB",
              "ResizeRenderBuf",
              "cuMemAlloc",
              "",
    ...
    if (mRenderBuf[chan].garray != 0x0) {
        cudaCheck(cuGraphicsUnregisterResource(mRenderBuf[chan].grsc),
                  "VolumeGVDB",
                  "ResizeRenderBuf",
...
void VolumeGVDB::PrepareRender(int w, int h, char shading)
{
    ...
    mScnInfo.camu = cam->trRayWorld;
    mScnInfo.camu -= mScnInfo.cams;
    mScnInfo.camv = cam->blRayWorld;
    mScnInfo.camv -= mScnInfo.cams;
    ...
    cudaCheck(cuMemcpyHtoD(cuScnInfo, &mScnInfo, sizeof(ScnInfo)),
              "VolumeGVDB",
              "PrepareRender",
              "cuMemcpyHtoD",
              "cuScnInfo",
...
void VolumeGVDB::RenderKernel(CUfunction user_kernel, uchar chan, uchar rbuf)
{
    ...
    cudaCheck(
        cuLaunchKernel(user_kernel, grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "RenderKernel",
        "cuLaunch",
        "(user kernel)",
...
void VolumeGVDB::Render(char shading, uchar chan, uchar rbuf)
{
    ...
    if (shading == SHADE_OFF) {
        PUSH_CTX
        cudaCheck(cuMemsetD8(mRenderBuf[rbuf].gpu,
                             0,
                             static_cast<uint64>(width) * static_cast<uint64>(height) * 4),
                  "VolumeGVDB",
                  "Render",
                  "cuMemsetD8",
                  "(no shading)",
    ...
    switch (shading) {
        ...
        case SHADE_TRILINEAR:
            kern = FUNC_RAYTRILINEAR;
            break;
        case SHADE_TRICUBIC:
            kern = FUNC_RAYTRICUBIC;
            break;
        case SHADE_EMPTYSKIP:
            kern = FUNC_EMPTYSKIP;
            break;
        case SHADE_SECTION2D:
            kern = FUNC_SECTION2D;
            break;
        case SHADE_SECTION3D:
            kern = FUNC_SECTION3D;
            break;
        case SHADE_LEVELSET:
    ...
    cudaCheck(
        cuLaunchKernel(cuFunc[kern], grid.x, grid.y, 1, block.x, block.y, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "Render",
        "cuLaunch",
        mRendName[shading],
...
void VolumeGVDB::Raytrace(DataPtr rays, uchar chan, char shading, int frame, float bias)
{
    ...
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_RAYTRACE], grid.x, 1, 1, block.x, 1, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "Raytrace",
        "cuLaunch",
        "FUNC_RAYTRACE",
...
void VolumeGVDB::UpdateApron(uchar chan, float boundval, bool changeCtx)
{
    ...
    switch (mPool->getAtlas(chan).type) {
        case T_FLOAT:
            kern = FUNC_UPDATEAPRON_F;
            typ[0] = 'F';
            typ[1] = '1';
            break;
        case T_FLOAT3:
            kern = FUNC_UPDATEAPRON_F4;
            typ[0] = 'F';
            typ[1] = '3';
            break;  // F3 is implemented as F4
        case T_FLOAT4:
            kern = FUNC_UPDATEAPRON_F4;
            typ[0] = 'F';
            typ[1] = '4';
            break;
        case T_UCHAR:
            kern = FUNC_UPDATEAPRON_C;
            typ[0] = 'U';
            typ[1] = '1';
            break;
        case T_UCHAR4:
            kern = FUNC_UPDATEAPRON_C4;
            typ[0] = 'U';
            typ[1] = '4';
            break;
    }
    ...
    Vector3DI threadcnt(bricks, brickres,
                        brickres);  // First component is passed directly to grid
    Vector3DI block(6 * mApron, 8, 8);
    Vector3DI grid(
        threadcnt.x, (threadcnt.y + block.y - 1) / block.y, (threadcnt.y + block.z - 1) / block.z);

    // Note: Kernels currently assume apron size of 1.
    void *args[6] = {&cuVDBInfo, &chan, &bricks, &brickres, &brickwid, &boundval};
    cudaCheck(
        cuLaunchKernel(
            cuFunc[kern], grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
        "VolumeGVDB",
        "UpdateApron",
        "cuLaunch",
        typ,
...
void VolumeGVDB::UpdateApronFaces(uchar chan)
{
    ...
    Vector3DI threadcnt(bricks, brickwid,
                        brickwid);  // First component is passed directly to grid
    Vector3DI block(3 * mApron, 8, 8);
    Vector3DI grid(
        threadcnt.x, (threadcnt.y + block.y - 1) / block.y, (threadcnt.z + block.z - 1) / block.z);
...
void VolumeGVDB::ComputeKernel(CUmodule user_module,
                               CUfunction user_kernel,
                               uchar channel,
                               bool bUpdateApron,
                               bool skipOverAprons)
{
    ...
    cudaCheck(
        cuLaunchKernel(
            user_kernel, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
        "VolumeGVDB",
        "ComputeKernel",
        "cuLaunch",
        "(user kernel)",
    ...
    if (bUpdateApron) {
        cudaCheck(
            cuCtxSynchronize(), "VolumeGVDB", "ComputeKernel", "cuCtxSyncronize", "", mbDebug);
...
void VolumeGVDB::Compute(int effect,
                         uchar channel,
                         int num_iterations,
                         ...
                         float boundval)
{
    ...
    for (int n = 0; n < num_iterations; n++) {
        cudaCheck(cuLaunchKernel(cuFunc[effect],
                                 grid.x,
                                 grid.y,
                                 grid.z,
                                 block.x,
        ...
        if (bUpdateApron)
            UpdateApron(channel, boundval);  // update the apron
    }
    POP_CTX
...
void VolumeGVDB::DownsampleCPU(Matrix4F xform,
                               Vector3DI in_res,
                               char in_aux,
                               Vector3DI out_res,
                               Vector3DF out_max,
                               ...
                               Vector3DF outr)
{
    ...
    Vector3DI block(8, 8, 8);
    Vector3DI grid(
        int(out_res.x / block.x) + 1, int(out_res.y / block.y) + 1, int(out_res.z / block.z) + 1);
    ...
    void *args[8] = {&in_res,
                     ...
                     &out_max,
                     &mAux[out_aux].gpu,
                     &mAux[AUX_MATRIX4F].gpu,
                     &inr,
                     &outr};
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_DOWNSAMPLE],
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
...
Vector3DF VolumeGVDB::Reduction(uchar chan)
{
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_REDUCTION],
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
...
void VolumeGVDB::Resample(
    uchar chan, Matrix4F xform, Vector3DI in_res, char in_aux, Vector3DF inr, Vector3DF outr)
{
    PrepareVDB();
    ...
    void *args[8] = {
        &cuVDBInfo, &res, &chan, &in_res, &mAux[in_aux].gpu, &mAux[AUX_MATRIX4F].gpu, &inr, &outr};

    cudaCheck(cuLaunchKernel(cuFunc[FUNC_RESAMPLE],
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
...
void VolumeGVDB::PrefixSum(CUdeviceptr inArray, CUdeviceptr outArray, int numElem)
{
    ...
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], blks1, 1, 1, threads, 1, 1, 0, NULL, argsA, NULL),
        "VolumeGVDB",
        "PrefixSum",
        "cuLaunch",
        "FUNC_PREFIXSUM (in,out->A1)",
        mbDebug);

#if ONE_LEVEL
    void *argsB[5] = {&mAux[AUX_ARRAY1].gpu, &mAux[AUX_SCAN1].gpu, &nptr, &blks1, &zon};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], blks2, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL),
        "cuPrefixSumB",
        "PrefixSum");
    ...
    void *argsB[5] = {
        &mAux[AUX_ARRAY1].gpu, &mAux[AUX_SCAN1].gpu, &mAux[AUX_ARRAY2].gpu, &blks1, &zon};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], blks2, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL),
        "VolumeGVDB",
        "PrefixSum",
        "cuLaunch",
        "FUNC_PREFIXSUM (A1,S1->A2)",
        mbDebug);
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_PREFIXSUM], 1, 1, 1, threads, 1, 1, 0, NULL, argsC, NULL),
              ...
              "PrefixSum",
              "cuLaunch",
              "FUNC_PREFIXSUM (A2,S2->null)",
              mbDebug);

    void *argsD[3] = {&mAux[AUX_SCAN1].gpu, &mAux[AUX_SCAN2].gpu, &blks1};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_PREFIXFIXUP], blks2, 1, 1, threads, 1, 1, 0, NULL, argsD, NULL),
        "VolumeGVDB",
        "PrefixSum",
        "cuLaunch",
        "FUNC_PREFIXFIXUP (S2->S1)",
        mbDebug);
#endif

    void *argsE[3] = {&outArray, &mAux[AUX_SCAN1].gpu, &numElem};
    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_PREFIXFIXUP], blks1, 1, 1, threads, 1, 1, 0, NULL, argsE, NULL),
        "VolumeGVDB",
        "PrefixSum",
        "cuLaunch",
        "FUNC_PREFIXFIXUP (S1->out)",
        mbDebug);
...
void VolumeGVDB::CommitData(DataPtr &dat, int cnt, char *cpubuf, int offs, int stride)
{
    ...
    if (dat.gpu == 0x0) {
        gprintf("ERROR: Buffer not allocated on GPU. May be missing call to "
                "AllocData.\n");
        gerror();
        return;
    }
...
{
    PUSH_CTX
    mPool->CreateMemLinear(
        mTransferPtr, (char *)mScene->getTransferFunc(), 16384 * sizeof(Vector4DF));
...
void VolumeGVDB::SetPoints(DataPtr &pntpos, DataPtr &pntvel, DataPtr &pntclr)
{
    ...
    if (pntvel.gpu == 0)
        CleanAux(AUX_SUBCELL_PNT_VEL);
    if (pntclr.gpu == 0)
        CleanAux(AUX_SUBCELL_PNT_CLR);
}
...
void VolumeGVDB::PrepareAux(int id, int cnt, int stride, bool bZero, bool bCPU)
{
    ...
    if (bZero) {
        cudaCheck(cuMemsetD8(mAux[id].gpu, 0, mAux[id].size),
                  "VolumeGVDB",
                  "PrepareAux",
                  "cuMemsetD8",
                  "",
...
void VolumeGVDB::InsertPointsSubcell(
    int subcell_size, int num_pnts, float pRadius, Vector3DF trans, int &pSCPntsLength)
{
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_SET_FLAG_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSetFlag, NULL),
        "VolumeGVDB",
        "InsertPointsSubcell",
        "cuLaunch",
        "FUNC_SET_FLAG_SUBCELL",
        mbDebug);
    ...
    void *argsCounting[14] = {&cuVDBInfo,
                              &subcell_size,
                              &numSCellPerBrick,
                              &num_pnts,
                              &mAux[AUX_PNTPOS].gpu,
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_COUNT_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsCounting, NULL),
        "VolumeGVDB",
        "InsertPointsSubcell",
        "cuLaunch",
        "FUNC_COUNT_SUBCELL",
    ...
    cudaCheck(cuMemcpyDtoH(&offset_last,
                           mAux[AUX_SUBCELL_PREFIXSUM].gpu + (numSCell - 1) * sizeof(int),
                           sizeof(int)),
              "VolumeGVDB",
              "InsertPointsSubcell",
              "cuMemcpyDtoH",
              "AUX_SUBCELL_PREFIXSUM",
              mbDebug);
    cudaCheck(cuMemcpyDtoH(&cnt_last,
                           mAux[AUX_SUBCELL_CNT].gpu + (numSCell - 1) * sizeof(int),
                           sizeof(int)),
              "VolumeGVDB",
              "InsertPointsSubcell",
              "cuMemcpyDtoH",
    ...
    if (mAux[AUX_PNTVEL].gpu != 0x0)
        PrepareAux(AUX_SUBCELL_PNT_VEL, pSCPntsLength, sizeof(float3), false);
    if (mAux[AUX_PNTCLR].gpu != 0x0)
        PrepareAux(AUX_SUBCELL_PNT_CLR, pSCPntsLength, sizeof(uint), false);
    PERF_POP();

    //////////////////////////////////////////////////////////////////////////
    PERF_PUSH("SC insert");
    void *argsInsert[23] = {&cuVDBInfo,
                            &subcell_size,
                            &numSCellPerBrick,
                            &num_pnts,
                            &mAux[AUX_SUBCELL_CNT].gpu,
                            ...
                            &mAux[AUX_PNTPOS].subdim.x,
                            &mAux[AUX_PNTPOS].stride,
                            &mAux[AUX_SUBCELL_PNT_POS].gpu,
                            &mAux[AUX_PNTVEL].gpu,
                            &mAux[AUX_PNTVEL].subdim.x,
                            &mAux[AUX_PNTVEL].stride,
                            &mAux[AUX_SUBCELL_PNT_VEL].gpu,
                            &mAux[AUX_PNTCLR].gpu,
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_INSERT_SUBCELL], pblks, 1, 1, threads, 1, 1, 0, NULL, argsInsert, NULL),
        "VolumeGVDB",
        "InsertPointsSubcell",
        "cuLaunch",
        "FUNC_INSERT_SUBCELL",
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_CALC_SUBCELL_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSCPos, NULL),
        "VolumeGVDB",
        "InsertPointsSubcell",
        "cuLaunch",
        "FUNC_CALC_SUBCELL_POS",
        mbDebug);
    PERF_POP();
...
void VolumeGVDB::InsertPointsSubcell_FP16(
    int subcell_size, int num_pnts, float pRadius, Vector3DF trans, int &pSCPntsLength)
{
    PrepareVDB();
    ...
    cudaCheck(cuMemcpyDtoH(&tmpa,
              ...
              mbDebug);
    cudaCheck(
        cuMemcpyDtoH(&tmpb, mAux[AUX_SUBCELL_CNT].gpu + (numSCell - 1) * sizeof(int), sizeof(int)),
        "VolumeGVDB",
        "InsertPointsSubcell_FP16",
        "cuMemcpyDtoH",
        "AUX_SUBCELL_CNT",
    ...
    if (mAux[AUX_PNTVEL].gpu != 0x0)
        PrepareAux(AUX_SUBCELL_PNT_VEL, pSCPntsLength, sizeof(ushort3), false);
    if (mAux[AUX_PNTCLR].gpu != 0x0)
        PrepareAux(AUX_SUBCELL_PNT_CLR, pSCPntsLength, sizeof(uint), false);
    PERF_POP();
    ...
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_INSERT_SUBCELL_FP16],
                             pblks,
                             1,
                             1,
                             threads,
...
void VolumeGVDB::MapExtraGVDB(int subcell_size)
{
    ...
    void *argsSCPos[10] = {&cuVDBInfo,
                           ...
                           &mAux[AUX_SUBCELL_MAPPING].gpu,
                           &cuOBSVDBInfo,
                           &mAux[AUX_SUBCELL_OBS_NID].gpu};
    int pblks = int(numSCellMapping / threads) + 1;
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_MAP_EXTRA_GVDB], pblks, 1, 1, threads, 1, 1, 0, NULL, argsSCPos, NULL),
        "VolumeGVDB",
        "MapExtraGVDB",
        "cuLaunch",
        "FUNC_MAP_EXTRA_GVDB",
...
void VolumeGVDB::InsertPoints(int num_pnts, Vector3DF trans, bool bPrefix)
{
    ...
    PrepareAux(AUX_PNODE, num_pnts, sizeof(int),
               false);  // node which each point falls into
    PrepareAux(AUX_PNDX, num_pnts, sizeof(int),
               false);  // index of the point inside that node
    PrepareAux(AUX_GRIDCNT, bricks, sizeof(int),
               true);  // number of points in each brick cell
    PERF_POP();
    ...
    void *args[9] = {&cuVDBInfo,
                     ...
                     &trans.x};
    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_INSERT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "InsertPoints",
              "cuLaunch",
              "FUNC_INSERT_POINTS",
    ...
    if (bPrefix) {
        ...
        void *args[13] = {&cuVDBInfo,
                          ...
                          &mAux[AUX_GRIDCNT].gpu,
                          &mAux[AUX_GRIDOFF].gpu,
                          &mAux[AUX_PNTSORT].gpu,
                          &trans.x};

        cudaCheck(cuLaunchKernel(
                      cuFunc[FUNC_SORT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
                  "VolumeGVDB",
                  "InsertPoints",
                  "cuLaunch",
                  "FUNC_SORT_POINTS",
...
void VolumeGVDB::InsertSupportPoints(int num_pnts, float offset, Vector3DF trans, bool bPrefix)
{
    ...
    void *args[12] = {&num_pnts,
                      ...
                      &mAux[AUX_GRIDCNT].gpu,
                      &mAux[AUX_PNTDIR].gpu,
                      &mAux[AUX_PNTDIR].subdim.x,
                      &mAux[AUX_PNTDIR].stride,
                      &trans.x};
    cudaCheck(
        cuLaunchKernel(
            cuFunc[FUNC_INSERT_SUPPORT_POINTS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "InsertSupportPoints",
        "cuLaunch",
        "FUNC_INSERT_SUPPORT_POINTS",
        mbDebug);
    PERF_POP();
...
void VolumeGVDB::ScalePntPos(int num_pnts, float scale)
{
    ...
    int threads = 512;
    int pblks = int(num_pnts / threads) + 1;
    void *args[5] = {&num_pnts,
                     &mAux[AUX_PNTPOS].gpu,
                     &mAux[AUX_PNTPOS].subdim.x,
                     &mAux[AUX_PNTPOS].stride,
                     &scale};
    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_SCALE_PNT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "ScalePntPos",
              "cuLaunch",
              "FUNC_SCALE_PNT_POS",
...
void VolumeGVDB::GatherDensity(int subcell_size,
                               int num_pnts,
                               float radius,
                               Vector3DF trans,
                               int &pSCPntsLength,
                               ...
                               bool bAccum)
{
    ...
    PUSH_CTX
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_DENSITY],
                             numSCell,
                             1,
                             1,
                             subcell_size,
...
void VolumeGVDB::GatherLevelSet(int subcell_size,
                                int num_pnts,
                                float radius,
                                Vector3DF trans,
                                int &pSCPntsLength,
                                ...
                                bool bAccum)
{
    ...
    PUSH_CTX
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET],
                             numSCell,
                             1,
                             1,
                             subcell_size,
...
void VolumeGVDB::GatherLevelSet_FP16(int subcell_size,
                                     int num_pnts,
                                     float radius,
                                     Vector3DF trans,
                                     int &pSCPntsLength,
                                     ...
                                     int chanClr)
{
    ...
    PUSH_CTX
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_GATHER_LEVELSET_FP16],
                             numSCell,
                             1,
                             1,
                             subcell_size,
...
void VolumeGVDB::GetBoundingBox(int num_pnts, Vector3DF pTrans)
{
    ...
    int threads = 256;
    int pblks = int(num_pnts / threads) + 1;
    void *args[8] = {&num_pnts,
                     &pTrans.x,
                     &mAux[AUX_PNTPOS].gpu,
                     &mAux[AUX_PNTPOS].subdim.x,
                     &mAux[AUX_PNTPOS].stride,
                     &mAux[AUX_WORLD_POS_X].gpu,
                     &mAux[AUX_WORLD_POS_Y].gpu,
                     &mAux[AUX_WORLD_POS_Z].gpu};

    cudaCheck(
        cuLaunchKernel(cuFunc[FUNC_SPLIT_POS], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
        "VolumeGVDB",
        "GetBoundingBox",
        "cuLaunch",
        "FUNC_SPLIT_POS",
    ...
    const int axes[] = {AUX_WORLD_POS_X, AUX_WORLD_POS_Y, AUX_WORLD_POS_Z};

    for (uint32_t i = 0; i < 6; ++i) {
        if (i < 3) {
            gvdbDeviceMinElementF(
                (mAux[AUX_BOUNDING_BOX].gpu + i * sizeof(float)), mAux[axes[i % 3]].gpu, num_pnts);
        }
        else {
            gvdbDeviceMaxElementF(
                (mAux[AUX_BOUNDING_BOX].gpu + i * sizeof(float)), mAux[axes[i % 3]].gpu, num_pnts);
        }
    }
...
void VolumeGVDB::GetMinMaxVel(int num_pnts)
{
    ...
    void *args[8] = {&num_pnts,
                     ...
                     &mAux[AUX_PNTVEL].subdim.x,
                     &mAux[AUX_PNTVEL].stride,
                     &mAux[AUX_WORLD_VEL_X].gpu,
                     &mAux[AUX_WORLD_VEL_Y].gpu,
                     &mAux[AUX_WORLD_VEL_Z].gpu};
    ...
    const int axes[] = {AUX_WORLD_VEL_X, AUX_WORLD_VEL_Y, AUX_WORLD_VEL_Z};

    for (uint32_t i = 0; i < 6; ++i) {
        if (i < 3) {
            gvdbDeviceMinElementF((mAux[AUX_VEL_BOUNDING_BOX].gpu + i * sizeof(float)),
                                  mAux[axes[i % 3]].gpu,
                                  num_pnts);
        }
        else {
            gvdbDeviceMaxElementF((mAux[AUX_VEL_BOUNDING_BOX].gpu + i * sizeof(float)),
                                  mAux[axes[i % 3]].gpu,
                                  num_pnts);
        }
    }
...
void VolumeGVDB::ReadGridVel(int N)
{
    ...
    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_READ_GRID_VEL], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "ReadGridVel",
              "cuLaunch",
              "FUNC_READ_GRID_VEL",
...
void VolumeGVDB::CheckVal(
    float slice, int chanVx, int chanVy, int chanVz, int chanVxOld, int chanVyOld, int chanVzOld)
{
    PUSH_CTX
    ...
    void *args[11] = {&cuVDBInfo,
                      ...
                      &mAux[AUX_OUT1].gpu,
                      &mAux[AUX_OUT2].gpu};
    cudaCheck(cuLaunchKernel(cuFunc[FUNC_CHECK_VAL],
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
...
void VolumeGVDB::ScatterDensity(
    int num_pnts, float radius, float amp, Vector3DF trans, bool expand, bool avgColor)
{
    uint num_voxels;
    ...
    if (mAux[AUX_PNTCLR].gpu != NULL && avgColor) {
        ...
        num_voxels = brickResVec.x * brickResVec.y * brickResVec.z *
                     static_cast<uint>(getNumUsedNodes(0));
        if (mbProfile)
            PERF_PUSH("Prepare Aux");
        PrepareAux(AUX_COLAVG,
                   4 * num_voxels,
                   sizeof(uint),
                   true);  // node which each point falls into
        if (mbProfile)
            PERF_POP();
    }

    void *args[13] = {&num_pnts,
                      &radius,
                      &amp,
                      &mAux[AUX_PNTPOS].gpu,
                      &mAux[AUX_PNTPOS].subdim.x,
                      ...
                      &mAux[AUX_COLAVG].gpu};
    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_SCATTER_DENSITY], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "ScatterPointDensity",
              "cuLaunch",
              "FUNC_SCATTER_DENSITY",
              mbDebug);
    ...
    if (mAux[AUX_PNTCLR].gpu != NULL && avgColor) {
        ...
        cudaCheck(cuLaunchKernel(cuFunc[FUNC_SCATTER_AVG_COL],
                                 pblks_avgcol,
                                 1,
                                 1,
                                 threads_avgcol,
...
//
void VolumeGVDB::ConvertAndTransform(DataPtr &psrc,
                                     char psrcbits,
                                     DataPtr &pdest,
                                     char pdestbits,
                                     int num_pnts,
                                     ...
                                     Vector3DF scal)
{
    ...
    int pblks = int(num_pnts / threads) + 1;
    void *args[9] = {
        &num_pnts, &psrc.gpu, &psrcbits, &pdest.gpu, &pdestbits, &wMin, &wDelta, &trans, &scal};

    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_CONV_AND_XFORM], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "ConvertAndTransform",
              "cuLaunch",
              "FUNC_CONV_AND_XFORM",
...
void VolumeGVDB::AddSupportVoxel(int num_pnts,
                                 float radius,
                                 float offset,
                                 float amp,
                                 Vector3DF trans,
                                 ...
                                 bool avgColor)
{
    ...
    void *args[14] = {&num_pnts,
                      ...
                      &mAux[AUX_COLAVG].gpu};
    cudaCheck(cuLaunchKernel(
                  cuFunc[FUNC_ADD_SUPPORT_VOXEL], pblks, 1, 1, threads, 1, 1, 0, NULL, args, NULL),
              "VolumeGVDB",
              "AddSupportVoxel",
              "cuLaunch",
              "FUNC_ADD_SUPPORT_VOXEL",
              mbDebug);
...
void VolumeGVDB::SetTransform(Vector3DF pretrans, Vector3DF scal, Vector3DF angs, Vector3DF trans)
{
    ...
    // Compute mXform = T R S PT (pretranslate, then scale, then rotate, then
    // translate)
    mXform.Identity();
...
// Node queries
uint64 VolumeGVDB::getNumUsedNodes(int lev)
{
    return mPool->getPoolUsedCnt(0, lev);
}
uint64 VolumeGVDB::getNumTotalNodes(int lev)
{
    return mPool->getPoolTotalCnt(0, lev);
}
Node *VolumeGVDB::getNodeAtLevel(int n, int lev)
{
    return (Node *)(mPool->PoolData(0, lev, n));
}

Vector3DF VolumeGVDB::getWorldMin(Node *node)
{
    return Vector3DF(node->mPos);
}
Vector3DF VolumeGVDB::getWorldMax(Node *node)
{
    return Vector3DF(node->mPos) + getCover(node->mLev);
}

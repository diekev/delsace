...
class GVDB_API VolumeBase {
  public:
    ...
    // Query functions
    void getDimensions(Vector3DF &objmin,
                       Vector3DF &objmax,
                       Vector3DF &voxmin,
                       Vector3DF &voxmax,
                       Vector3DF &voxres);
    ...
    DataPtr getTransferPtr()
    {
        return mTransferPtr;
    }
    // Gets the transfer function as a CUdeviceptr
    CUdeviceptr getTransferFuncGPU()
    {
        return mTransferPtr.gpu;
    }
  ...
  public:
    ...
    std::vector<DataPtr> mRenderBuf;  // Non-owning list of render buffers (since apps can add
                                      // their own render buffers)

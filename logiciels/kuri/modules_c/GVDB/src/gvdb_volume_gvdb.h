...
struct ALIGN(16) VDBInfo {
    ...
    CUtexObject
        volIn[MAX_CHANNEL];  // Texture reference (read plus interpolation) to atlas per channel
...
class GVDB_API VolumeGVDB : public VolumeBase {
  public:
    ...
    void SetEpsilon(float eps, int maxiter)
    {
        mEpsilon = eps;
        mMaxIter = maxiter;
    ...
    char *getDataPtr(int i, DataPtr dat)
    {
        return (dat.cpu + (i * dat.stride));
    }
    ...
    // `mPool->getAtlasResPacked(...)`; otherwise, it uses a grid of size
    // `mPool->getAtlasRes(...)`.
    void ComputeKernel(CUmodule user_module,
    ...
    void DownsampleCPU(Matrix4F xform,
                       Vector3DI in_res,
                       char in_aux,
                       Vector3DI out_res,
                       Vector3DF out_max,
    ...
    void SetChannelDefault(int cx, int cy, int cz)
    {
        mDefaultAxiscnt.Set(cx, cy, cz);
    }
    void SetApron(int n)
    {
        mApron = n;
    }
    void AddChannel(uchar chan,
                    int dt,
                    int apron,
                    int filter = F_LINEAR,
                    int border = F_CLAMP,
                    Vector3DI axiscnt = Vector3DI(0, 0, 0));
    void FillChannel(uchar chan, Vector4DF val);
    void ClearAllChannels();
    void ClearChannel(uchar chan);
    uchar GetChannelType(
        uchar channel);  // Returns the type code (e.g. T_FLOAT) of the given channel.
    slong Reparent(int lev,
                   slong prevroot_id,
                   Vector3DI pos,
                   bool &bNew);  // Reparent tree with new root
    slong ActivateSpace(Vector3DF pos);
    slong ActivateSpace(slong nodeid,
                        Vector3DI pos,
                        bool &bNew,
                        slong stopnode = ID_UNDEFL,
                        int stoplev = 0);  // Active leaf at given location
    ...
    void ActivateExtraBricksGPU(
        int pNumPnts, float pRadius, Vector3DF pOrig, int pRootLev, Vector3DI pRootPos);
    void ActivateIncreBricksGPU(int pNumPnts,
                                float pRadius,
                                Vector3DF pOrig,
                                int pRootLev,
                                Vector3DI pRootPos,
    ...
    void RequestFullRebuild(bool tf)
    {
        mRebuildTopo = tf;
    }
    ...
    int GetLevels()
    {
        return mPool->getNumLevels();
    }

    void ReadGridVel(int N);
    void CheckVal(float slice,
                  int chanVx,
                  int chanVy,
                  int chanVz,
                  int chanVxOld,
    ...
    nvdb::Node *getNode(int grp, int lev, slong ndx)
    {
        return (Node *)mPool->PoolData(Elem(grp, lev, ndx));
    }
    // Gets a Node struct from a pool reference.
    nvdb::Node *getNode(slong nodeid)
    {
        return (Node *)mPool->PoolData(nodeid);
    }
    ...
    bool isLeaf(slong nodeid)
    {
        return ElemLev(nodeid) == 0;
    }
    ...
    // Returns true and sets `bit` to the corresponding bit index if the index-space position `pos`
    // is inside the given node, for false otherwise
    ...
    void getUsage(Vector3DF &ext,
                  Vector3DF &vox,
                  Vector3DF &used,
                  Vector3DF &free);  // Quick memory info
    ...
    void SolidVoxelize(uchar chan,
                       Model *model,
                       Matrix4F *xform,
                       float val_surf,
                       float val_inside,
                       float vthresh = 0.0);
    int VoxelizeNode(Node *node,
                     uchar chan,
                     Matrix4F *xform,
                     float bdiv,
                     float val_surf,
    ...
    int getAtlasGLID(int chan)
    {
        return mPool->getAtlasGLID(chan);
    }
    ...
    void AddSupportVoxel(int num_pnts,
                         float radius,
                         float offset,
                         float amp,
                         Vector3DF trans,
    ...
    void InsertPointsSubcell(
        int subcell_size, int num_pnts, float pRadius, Vector3DF trans, int &pSCPntsLength);
    void InsertPointsSubcell_FP16(
        int subcell_size, int num_pnts, float radius, Vector3DF trans, int &pSCPntsLength);

    void ScalePntPos(int num_pnts, float scale);
    void ScatterDensity(int num_pnts,
                        float radius,
                        float amp,
                        Vector3DF trans,
                        bool expand = true,
                        bool avgColor = false);
    void GatherDensity(int subcell_size,
                       int num_pnts,
                       float radius,
                       Vector3DF trans,
                       int &pSCPntsLength,
                       ...
                       int chanClr,
                       bool bAccum = false);
    void GatherLevelSet(int subcell_size,
                        int num_pnts,
                        float radius,
                        Vector3DF trans,
                        int &pSCPntsLength,
                        ...
                        int chanClr,
                        bool bAccum = false);
    void GatherLevelSet_FP16(int subcell_size,
                             int num_pnts,
                             float radius,
                             Vector3DF trans,
                             int &pSCPntsLength,
                             int chanLevelSet,
                             int chanClr);
    void ConvertAndTransform(DataPtr &psrc,
                             char psrcbits,
                             DataPtr &pdest,
                             char pdestbits,
                             int num_pnts,
    ...
    Vector3DF getBoundMin()
    {
        return mPosMin;
    }
    Vector3DF getBoundMax()
    {
        return mPosMax;
    }
    ...
    uint64 getMaskSize(int lev)
    {
        uint64 sz = getVoxCnt(lev) / 8;
        return (lev == 0) ? 0 : ((sz < 8) ? 8 : sz);
    }  // Mask Size of level

    // VDB Configuration
    void SetVDBConfig(int lev, int i)
    {
        mVCFG[lev] = i;
    }
    ...
    // Gets the branching factor per side (number of child nodes or child voxels per side) of a
    // node at the given level.
    int getRes(int lev)
    {
        return (1 << mLogDim[lev]);
    }
    // Gets the number of child nodes or child voxels of a node at the given level.
    uint64 getVoxCnt(int lev)
    {
        uint64 r = uint64(1) << mLogDim[lev];
        return r * r * r;
    }
    ...
    int getBitPos(int lv, Vector3DI pos)
    {
        int res = getRes(lv);
    ...
    Vector3DF getCover(int lv)
    {
        return Vector3DF(getRange(lv));
    }
    ...
    Vector3DI getRes3DI(int lv)
    {
        int r = (1 << mLogDim[lv]);
        return Vector3DI(r, r, r);
    }
    // Gets a color for the given level.
    Vector3DF getClrDim(int lv)
    {
        return mClrDim[lv];
    }
    // Gets the `id`th auxiliary buffer.
    DataPtr &getAux(int id)
    {
        return mAux[id];
    }

    void SetSimBounds(Vector3DI b)
    {
        mSimBounds = b;
    }
    void SetCollisionObj(VolumeGVDB *obs)
    {
        cuOBSVDBInfo = obs->cuVDBInfo;
        mHasObstacle = true;
    }
    ...
    {
        if (!mbVerbose)
            return;  // check if verbose
        va_list vlist;
        va_start(vlist, fmt);
        gprintf2(vlist, fmt, 0);
    }
    // Gets the total number of nodes at a given level.
    uint64 getNumNodes(int lev)
    {
        return getNumTotalNodes(lev);
    }
    ...
    // Gets the pool reference of the leaf (level 0 node) that is a child of the pool reference
    // `nodeid` and contains the point `pos` (in voxel coordinates). Returns ID_UNDEFL if no such
    // leaf exists.
    uint64 getNodeAtPoint(uint64 nodeid, Vector3DF pos);

    // Gets the inclusive lower bound of the node's bounding box in voxel coordinates (i.e.
    // `node->mPos`). This function's return type may be changed to `Vector3DI` in the future.
    ...
    // Sets the transform from points in GVDB's coordinate system to the application's coordinate
    // system. This transform consists of the following operations in order:
    // - Add `pretrans` to the point
    // - Multiply the result's coordinates by `scal`
    // - Rotate the result around the X axis by angs.x, around the Y axis by angs.x, and then
    // around the Z axis by angs.z (as in `Matrix4F::RotateTZYXS`)
    // - Add `trans` to the result
    void SetTransform(Vector3DF pretrans, Vector3DF scal, Vector3DF angs, Vector3DF trans);
    // Returns the transform as a 4x4 matrix. If `v` is a vector, then computing `v`*`mXform`
    // transforms the point `v` from GVDB's coordinate system to the application's coordinate
    // system.
    ...
    int getMaskBytes(Node *node)
    {
        int r = getRes(node->mLev);
        return imax(((uint64)r * r * r) >> 3, 1);
    }  // divide by bits per byte (2^3=8)
    uint64 getMaskWords(Node *node)
    {
        int r = getRes(node->mLev);
        return imax(((uint64)r * r * r) >> 6, 1);
    }  // divide by bits per 64-bit word (2^6=64)
    uint64 *getMask(Node *node)
    {
        return (uint64 *)&node->mMask;
    }
    int getNumChild(Node *node)
    {
        return (node->mLev == 0) ? 0 : countOn(node);
    }
...
#define B6(n) B4(n), B4(n + 1), B4(n + 1), B4(n + 2)
    ...
    inline uint64 numBitsOff(byte v)
    {
        return numBitsOn((byte)~v);
    }
    ...
    inline uint64 numBitsOff(uint32 v)
    {
        return numBitsOn(~v);
    }
    ...
    inline uint64 numBitsOff(uint64 v)
    {
        return numBitsOn((uint64)~v);
    }
    ...
    {
        assert(v);
        static const byte DeBruijnBitPos[32] = {0,  1,  28, 2,  29, 14, 24, 3,  30, 22, 20,
    ...
    {
        static const byte DeBruijn[32] = {0,  9,  1,  10, 13, 21, 2,  29, 11, 14, 16,
    ...
    uint64 countOff(Node *node)
    {
        return getMaskBytes(node) * 8 - countOn(node);
    }
    ...
    void setOn(Node *node, uint32 n)
    {
        (&node->mMask)[n >> 6] |= uint64(1) << (n & 63);
    }
    void setOff(Node *node, uint32 n)
    {
        (&node->mMask)[n >> 6] &= ~(uint64(1) << (n & 63));
    }
    bool isOn(Node *node, uint64 n)
    {
        return (((uint64 *)&node->mMask)[n >> 6] & (uint64(1) << (n & 63))) != 0;
    }
    bool isOff(Node *node, uint64 n)
    {
        return (((uint64 *)&node->mMask)[n >> 6] & (uint64(1) << (n & 63))) == 0;
    }

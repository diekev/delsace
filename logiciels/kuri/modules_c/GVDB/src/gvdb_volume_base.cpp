...
void VolumeBase::getDimensions(
    Vector3DF &objmin, Vector3DF &objmax, Vector3DF &voxmin, Vector3DF &voxmax, Vector3DF &voxres)
{
    objmin = mObjMin;
    objmax = mObjMax;
    voxmin = mVoxMin;
...
{
    if (m->vertArrayID != -1)
        glDeleteVertexArrays(1, (GLuint *)&m->vertArrayID);
    if (m->vertBufferID != -1)
        glDeleteBuffers(1, (GLuint *)&m->vertBufferID);
    if (m->elemBufferID != -1)
        glDeleteBuffers(1, (GLuint *)&m->elemBufferID);
}
...
void VolumeBase::CommitGeometry(Model *m)
{
    ...
    // Create VAO
    if (m->vertArrayID == -1)
        glGenVertexArrays(1, (GLuint *)&m->vertArrayID);
    glBindVertexArray(m->vertArrayID);

    // Update Vertex VBO
    if (m->vertBufferID == -1)
        glGenBuffers(1, (GLuint *)&m->vertBufferID);

    glNamedBufferDataEXT(
        m->vertBufferID, m->vertCount * m->vertStride, m->vertBuffer, GL_STATIC_DRAW);
    ...
    // Update Element VBO
    if (m->elemBufferID == -1)
        glGenBuffers(1, (GLuint *)&m->elemBufferID);
    glNamedBufferDataEXT(
        m->elemBufferID, m->elemCount * m->elemStride, m->elemBuffer, GL_STATIC_DRAW);

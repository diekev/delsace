...
Allocator::Allocator()
{
    ...
    cudaCheck(cuModuleGetFunction(&cuFillTex, cuAllocatorModule, "kernelFillTex"),
              ...
              "cuFillTex",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuCopyTexC, cuAllocatorModule, "kernelCopyTexC"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuCopyTexC",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuCopyTexF, cuAllocatorModule, "kernelCopyTexF"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuCopyTexF",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuCopyBufToTexC, cuAllocatorModule, "kernelCopyBufToTexC"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuCopyBufToTexC",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuCopyBufToTexF, cuAllocatorModule, "kernelCopyBufToTexF"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuCopyBufToTexF",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuCopyTexZYX, cuAllocatorModule, "kernelCopyTexZYX"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuCopTexZYX",
              mbDebug);
    cudaCheck(cuModuleGetFunction(&cuRetrieveTexXYZ, cuAllocatorModule, "kernelRetrieveTexXYZ"),
              "Allocator",
              "Allocator",
              "cuModuleGetFunction",
              "cuRetrieveTexXYZ",
...
Allocator::~Allocator()
{
    ...
    cudaCheck(cuModuleUnload(cuAllocatorModule),
              "Allocator",
              "~Allocator",
              "cuModuleUnload",
              "cuAllocatorModule",
...
{
    DataPtr *p = &mPool[grp][lev];
    // std::cout << grp << " " << lev << " " << p->gpu << " " << p->lastEle * p->stride <<
    // std::endl;
...
{
    DataPtr *p = &mPool[grp][lev];
    cudaCheck(cuMemcpyDtoH(p->cpu, p->gpu, p->lastEle * p->stride),
              "Allocator",
              "PoolFetch",
              "cuMemcpyDtoH",
              "",
    ...
    for (int grp = 0; grp < MAX_POOL; grp++)
        for (int lev = 0; lev < mPool[grp].size(); lev++) {
            ...
            if (mPool[grp][lev].gpu != 0x0)
                cudaCheck(cuMemFree(mPool[grp][lev].gpu),
                          "Allocator",
                          "PoolReleaseAll",
                          "cuMemFree",
                          "",
...
uint64 Allocator::PoolAlloc(uchar grp, uchar lev, bool bGPU)
{
    ...
    if (p->lastEle >= p->max) {
        ...
        if (p->gpu != 0x0) {
            ...
            CUdeviceptr new_gpu;
            cudaCheck(
                cuMemAlloc(&new_gpu, sz), "Allocator", "PoolAlloc", "cuMemAlloc", "", mbDebug);
            cudaCheck(
                cuMemsetD8(new_gpu, 0, sz), "Allocator", "PoolAlloc", "cuMemsetD8", "", mbDebug);
            cudaCheck(cuMemcpy(new_gpu, p->gpu, p->stride * p->lastEle),
                      "Allocator",
                      "PoolAlloc",
                      "cuMemcpy",
                      "",
...
{
    uchar g = ElemGrp(elem);
    uchar l = ElemLev(elem);
...
void Allocator::CreateMemLinear(
    DataPtr &p, char *dat, int stride, uint64 cnt, bool bCPU, bool bAllocHost)
{
    ...
    if (p.gpu != 0x0)
        cudaCheck(cuMemFree(p.gpu), "Allocator", "CreateMemLinear", "cuMemFree", "", mbDebug);
    cudaCheck(
        cuMemAlloc(&p.gpu, p.size), "Allocator", "CreateMemLinear", "cuMemAlloc", "", mbDebug);
...
void Allocator::FreeMemLinear(DataPtr &p)
{
    if (p.cpu != 0x0)
        free(p.cpu);
    if (p.gpu != 0x0)
        cudaCheck(cuMemFree(p.gpu), "Allocator", "FreeMemLinear", "cuMemFree", "", mbDebug);
    p.cpu = 0x0;
    p.gpu = 0x0;
}
...
{
    cudaCheck(cuMemcpyDtoH(p.cpu, p.gpu, p.size),
              "Allocator",
              "RetrieveMem",
              "cuMemcpyDtoH",
              "",
...
{
    cudaCheck(
        cuMemcpyHtoD(p.gpu, p.cpu, p.size), "Allocator", "CommitMem", "cuMemcpyHtoD", "", mbDebug);
}

void Allocator::AllocateTextureGPU(
    DataPtr &p, uchar dtype, Vector3DI res, bool bGL, uint64 preserve)
{
    // GPU allocate
    if (bGL) {
// OpenGL 3D texture
        ...
        switch (dtype) {
            case T_UCHAR:
                glTexImage3D(
                    GL_TEXTURE_3D, 0, GL_R8, res.x, res.y, res.z, 0, GL_RED, GL_UNSIGNED_BYTE, 0);
                break;
            case T_UCHAR4:
            ...
            case T_FLOAT:
                glTexImage3D(
                    GL_TEXTURE_3D, 0, GL_R32F, res.x, res.y, res.z, 0, GL_RED, GL_FLOAT, 0);
                break;
        };
        ...
        if (preserve > 0 && old_glid != -1) {
            const uint64 bytesPerPlane = static_cast<uint64>(res.x) * static_cast<uint64>(res.y) *
                                         static_cast<uint64>(getSize(dtype));
            const Vector3DI src(
                res.x, res.y, static_cast<int>(preserve / bytesPerPlane));  // src amount to copy
            glCopyImageSubData(old_glid,
                               GL_TEXTURE_3D,
                               0,
                               0,
                               0,
        ...
        if (p.grsc != 0)
            cudaCheck(cuGraphicsUnregisterResource(p.grsc),
                      "Allocator",
                      "AllocateTextureGPU",
                      "cuGraphicsUnregisterResource",
                      "",
                      mbDebug);
        cudaCheck(cuGraphicsGLRegisterImage(
                      &p.grsc, p.glid, GL_TEXTURE_3D, CU_GRAPHICS_REGISTER_FLAGS_SURFACE_LDST),
                  "Allocator",
                  "AllocateTextureGPU",
                  "cuGraphicsRegisterImage",
                  "",
                  mbDebug);
        gchkGL("cuGraphicsGLRegisterImage");
        cudaCheck(cuGraphicsMapResources(1, &p.grsc, 0),
                  "Allocator",
                  "AllocateTextureGPU",
        ...
        cudaCheck(cuSurfObjectCreate(&p.surf_obj, &resDesc),
                  "Allocator",
                  "AllocateTextureGPU",
                  "cuSurfObjectCreate",
                  "",
        ...
        cudaCheck(cuTexObjectCreate(&p.tex_obj, &surfReadDesc, &texReadDesc, nullptr),
                  "Allocator",
                  "AllocateTextureGPU",
        ...
        cudaCheck(cuGraphicsUnmapResources(1, &p.grsc, 0),
                  "Allocator",
                  "AllocateTextureGPU",
                  "cuGraphicsUnmapResources",
                  "",
        ...
        if (res.x > 0 && res.y > 0 && res.z > 0) {
            cudaCheck(cuArray3DCreate(&p.garray, &desc),
                      "Allocator",
                      "AllocateTextureGPU",
                      "cuArray3DCreate",
                      "",
            ...
            if (preserve > 0 && old_array != 0) {
                ...
                Vector3DI grid(
                    int(res.x / block.x) + 1, int(res.y / block.y) + 1, int(res.z / block.z) + 1);

                int dsize = getSize(p.type);
                void *args[3] = {&res, &dsize, &p.surf_obj};
                cudaCheck(cuLaunchKernel(cuFillTex,
                                         grid.x,
                                         grid.y,
                                         grid.z,
                                         block.x,
                ...
                cp.Height = res.y;
                cp.Depth = preserve /
                           (res.x * res.y * getSize(dtype));  // amount to copy (preserve)
                if (cp.Depth < desc.Depth)
                    cudaCheck(cuMemcpy3D(&cp),
                              "Allocator",
                              "AllocateTextureGPU",
                              "cuMemcpy3D",
                              "preserve",
        ...
        if (old_array != 0)
            cudaCheck(cuArrayDestroy(old_array),
                      "Allocator",
                      "AllocateTextureGPU",
                      "cuArrayDestroy",
                      "",
...
void Allocator::AllocateAtlasMap(int stride, Vector3DI axiscnt)
{
    ...
    if (q.gpu != 0x0)
        cudaCheck(cuMemFree(q.gpu), "Allocator", "AllocateAtlasMap", "cuMemFree", "", mbDebug);
    cudaCheck(
        cuMemAlloc(&q.gpu, q.size), "Allocator", "AllocateAtlasMap", "cuMemAlloc", "", mbDebug);
...
bool Allocator::AtlasCreate(uchar chan,
                            uchar dtype,
                            Vector3DI leafdim,
                            Vector3DI axiscnt,
                            char apr,
...
void Allocator::CopyChannel(int chanDst, int chanSrc)
{
    ...
    cp.Height = axisres.y;
    cp.Depth = preserve /
               (axisres.x * axisres.y * getSize(pSrc.type));  // amount to copy (preserve)
...
bool Allocator::AtlasResize(uchar chan, uint64 max_leaf)
{
    ...
    axiscnt.z = int(
        ceil(max_leaf / float(axiscnt.x * axiscnt.y)));  // expand number of bricks along Z-axis
...
void Allocator::AllocateNeighbors(uint64 cnt)
{
    ...
    if (mNeighbors.gpu != 0x0)
        cudaCheck(
            cuMemFree(mNeighbors.gpu), "Allocator", "AllocateNeighbors", "cuMemFree", "", mbDebug);
    cudaCheck(cuMemAlloc(&mNeighbors.gpu, mNeighbors.size),
              "Allocator",
              "AllocateNeighbors",
              "cuMemAlloc",
              "",
...
extern "C" CUresult cudaCopyData(
    cudaArray *dest, int dx, int dy, int dz, int dest_res, cudaArray *src, int src_res);
...
void Allocator::AtlasCopyTex(uchar chan, Vector3DI val, const DataPtr &src)
{
    ...
    Vector3DI block(8, 8, 8);
    Vector3DI grid(int(brickres.x / block.x) + 1,
                   int(brickres.y / block.y) + 1,
                   int(brickres.z / block.z) + 1);

    void *args[3] = {&val, &brickres, (void *)&src.surf_obj};
    switch (mAtlas[chan].type) {
        case T_UCHAR:
            cudaCheck(cuLaunchKernel(cuCopyTexC,
                                     grid.x,
                                     grid.y,
                                     grid.z,
                                     block.x,
            ...
            break;
        case T_FLOAT:
            cudaCheck(cuLaunchKernel(cuCopyTexF,
                                     grid.x,
                                     grid.y,
                                     grid.z,
                                     block.x,
...
void Allocator::AtlasCopyLinear(uchar chan, Vector3DI offset, CUdeviceptr gpu_buf)
{
    ...
    switch (mAtlas[chan].type) {
        case T_UCHAR:
            cudaCheck(cuLaunchKernel(cuCopyBufToTexC,
                                     grid.x,
                                     grid.y,
                                     grid.z,
                                     block.x,
            ...
            break;
        case T_FLOAT:
            cudaCheck(cuLaunchKernel(cuCopyBufToTexF,
                                     grid.x,
                                     grid.y,
                                     grid.z,
                                     block.x,
...
void Allocator::AtlasRetrieveTexXYZ(uchar chan, Vector3DI val, DataPtr &dest)
{
    ...
    cudaCheck(cuLaunchKernel(cuRetrieveTexXYZ,
                             grid.x,
                             grid.y,
                             grid.z,
                             block.x,
...
void Allocator::AtlasCopyTexZYX(uchar chan, Vector3DI val, const DataPtr &src)
{
    ...
    cudaCheck(
        cuLaunchKernel(
            cuCopyTexZYX, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL),
        "Allocator",
        "AtlasCopyTexZYX",
        "cuLaunch",
        "cuCopyTexZYX",
...
{
    Vector3DI res = mAtlas[chan].subdim *
...
void Allocator::AtlasFill(uchar chan)
{
    Vector3DI atlasres = getAtlasRes(chan);
    Vector3DI block(8, 8, 8);
    Vector3DI grid(int(atlasres.x / block.x) + 1,
                   int(atlasres.y / block.y) + 1,
    ...
    cudaCheck(
        cuLaunchKernel(
            cuFillTex, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, mStream, args, NULL),
        "Allocator",
        "AtlasCommitFromCPU",
        "cuLaunch",
        "cuFillTex",
...
void Allocator::AtlasRetrieveSlice(
    uchar chan, int slice, int sz, CUdeviceptr gpu_buf, uchar *cpu_dest)
{
...
void Allocator::AtlasReleaseAll()
{
    for (int n = 0; n < mAtlas.size(); n++) {
        ...
        if (mAtlas[n].surf_obj != 0x0) {
            cudaCheck(cuSurfObjectDestroy(mAtlas[n].surf_obj),
                      "Allocator",
                      "AtlasReleaseAll",
                      "cuSurfObjectDestroy",
                      "",
        ...
        if (mAtlas[n].tex_obj != 0x0) {
            cudaCheck(cuTexObjectDestroy(mAtlas[n].tex_obj),
                      "Allocator",
                      "AtlasReleaseAll",
                      "cuTexObjectDestroy",
                      "",
        ...
        if (mAtlas[n].grsc != 0x0) {
            cudaCheck(cuGraphicsUnregisterResource(mAtlas[n].grsc),
                      "Allocator",
                      "AtlasReleaseAll",
                      "cuGraphicsUnregisterResource",
                      "",
        ...
        if (mAtlas[n].garray != 0x0 && mAtlas[n].glid == -1) {
            cudaCheck(cuArrayDestroy(mAtlas[n].garray),
                      "Allocator",
                      "AtlasReleaseAll",
                      "cuArrayDestroy",
                      "",
...
// Free opengl memory
    ...
    for (int n = 0; n < mAtlasMap.size(); n++) {
        ...
        if (mAtlasMap[n].gpu != 0x0) {
            cudaCheck(cuMemFree(mAtlasMap[n].gpu),
                      "Allocator",
                      "AtlasReleaseAll",
                      "cuMemFree",
                      "AtlasMap",
...
bool cudaCheck(CUresult launch_stat,
               const char *obj,
               const char *method,
               const char *apicall,
               const char *arg,
...
void StartCuda(
    int devsel, CUcontext ctxsel, CUdevice &dev, CUcontext &ctx, CUstream *strm, bool verbose)
{
    // NOTES:
    ...
    // - Once app created CUDA context, OptiX will latch onto the existing CUDA context owned by
    // run-time API
    // - Alternatively, OptiX can create CUDA context. Then set runtime API to it. (This is how
    // Ocean SDK sample works.)
    ...
    if (devsel == GVDB_DEV_CURRENT) {
        ...
        cudaCheck(cuCtxGetCurrent(&ctx),
                  "(global)",
                  "StartCuda",
                  "cuCtxGetCurrent",
                  "GVDB_DEV_CURRENT",
                  false);
        cudaCheck(cuCtxGetDevice(&dev),
                  "(global)",
                  "StartCuda",
                  "cuCtxGetDevice",
                  "GVDB_DEV_CURRENT",
    ...
    if (devsel >= 0) {
        ...
        cudaCheck(cuCtxCreate(&ctx, CU_CTX_SCHED_AUTO, dev),
                  "(global)",
                  "StartCuda",
                  "cuCtxCreate",
                  "",

...
class GVDB_API PivotX {
  public:
    ...
    PivotX(Vector3DF &f, Vector3DF &t, Vector3DF &s, Vector3DF &a)
    {
        from_pos = f;
        to_pos = t;
        scale = s;
    ...
    void setPivot(Vector3DF &pos, Vector3DF &ang)
    {
        from_pos = pos;
        ang_euler = ang;
    }
    void setPivot(PivotX piv)
    {
        from_pos = piv.from_pos;
        to_pos = piv.to_pos;
        ang_euler = piv.ang_euler;
        updateTform();
    }
    void setPivot(PivotX &piv)
    {
    ...
    void setAng(float rx, float ry, float rz)
    {
        ang_euler.Set(rx, ry, rz);
        updateTform();
    }
    void setAng(Vector3DF &a)
    {
        ang_euler = a;
        updateTform();
    }
    ...
    void updateTform();
    void setTform(Matrix4F &t)
    {
        trans = t;
    }
    inline Matrix4F &getTform()
    {
        return trans;
    }
    inline float *getTformData()
    {
        return trans.GetDataF();
    }

    // Pivot
    PivotX getPivot()
    {
        return PivotX(from_pos, to_pos, scale, ang_euler);
    }
    Vector3DF &getPos()
    {
        return from_pos;
...
class GVDB_API Camera3D : public PivotX {
  public:
    enum eProjection { Perspective = 0, Parallel = 1 };
    Camera3D();
    ...
    void setTile(float x1, float y1, float x2, float y2)
    {
        mTile.Set(x1, y1, x2, y2);
        updateMatricies();
    }
    ...
    void getVectors(Vector3DF &dir, Vector3DF &up, Vector3DF &side)
    {
        dir = dir_vec;
        up = up_vec;
        side = side_vec;
    ...
    Matrix4F &getInvViewProjMatrix()
    {
        return invviewproj_matrix;
    }
    ...
    {
        return tileproj_matrix;
    }
    Matrix4F &getFullProjMatrix()
    {
        return proj_matrix;
    }

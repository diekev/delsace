...
extern void StartCuda(
    int devsel, CUcontext ctxsel, CUdevice &dev, CUcontext &ctx, CUstream *strm, bool verbose);
extern GVDB_API bool cudaCheck(CUresult e,
                               const char *obj,
                               const char *method,
                               const char *apicall,
                               const char *arg,
                               bool bDebug);
extern GVDB_API Vector3DF
cudaGetMemUsage();  // Returns (used MBs, free MBs, total MBs) of GPU memory.
...
inline uint64 Elem(uchar grp, uchar lev, uint64 ndx)
{
    return uint64(grp) | (uint64(lev) << 8) | (uint64(ndx) << 16);
}
inline uchar ElemGrp(uint64 id)
{
    return uchar(id & 0xFF);
}
inline uchar ElemLev(uint64 id)
{
    return uchar((id >> 8) & 0xFF);
}
inline uint64 ElemNdx(uint64 id)
{
    return id >> 16;
}
...
class Allocator {
  public:
    ...
    // Pool functions
    void PoolCreate(
        uchar grp, uchar lev, uint64 width, uint64 initmax, bool bGPU);  // create a pool
    ...
    uint64 getPoolUsedCnt(uchar grp, uchar lev)
    {
        return mPool[grp][lev].usedNum;
    }
    uint64 getPoolTotalCnt(uchar grp, uchar lev)
    {
        return mPool[grp][lev].lastEle;
    }
    uint64 getPoolMax(uchar grp, uchar lev)
    {
        return mPool[grp][lev].max;
    }
    char *getPoolCPU(uchar grp, uchar lev)
    {
        return mPool[grp][lev].cpu;
    }
    uint64 getPoolSize(uchar grp, uchar lev)
    {
        return mPool[grp][lev].size;
    }
    CUdeviceptr getPoolGPU(uchar grp, uchar lev)
    {
        return mPool[grp][lev].gpu;
    }
    ...
    void SetPoolUsedCnt(uchar grp, uchar lev, uint64 cnt)
    {
    ...
    void AtlasCommitFromCPU(uchar chan,
                            uchar *src);  // host-to-device copy from 3D to 3D (entire vol)
    void AtlasAppendLinearCPU(
        uchar chan,
        int n,
        float *src);  // CPU only, append 3D data linearly to end of atlas (no GPU update)
    void AtlasCopyTex(uchar chan,
                      Vector3DI val,
                      const DataPtr &src);  // device-to-device copy 3D sub-vol into 3D
    void AtlasCopyTexZYX(
        uchar chan,
        Vector3DI val,
        const DataPtr &src);  // device-to-device copy 3D sub-vol into 3D, ZYX order
    ...
    void CreateMemLinear(
        DataPtr &p, char *dat, int stride, uint64 cnt, bool bCPU, bool bAllocHost = false);
    ...
    CUdeviceptr getAtlasMapGPU(uchar chan)
    {
        return (mAtlasMap.size() == 0) ? 0 : mAtlasMap[chan].gpu;
    }

    // Neighbor Table
    void AllocateNeighbors(uint64 cnt);
    void CommitNeighbors();
    DataPtr *getNeighborTable()
    {
        return &mNeighbors;
    }
    ...
    int getNumAtlas()
    {
        return (int)mAtlas.size();
    }
    DataPtr getAtlas(uchar chan)
    {
        return mAtlas[chan];
    }
    float *getAtlasCPU(uchar chan)
    {
        return (float *)mAtlas[chan].cpu;
    }
    int getAtlasGLID(uchar chan)
    {
        return mAtlas[chan].glid;
    }
    uint64 getAtlasSize(uchar chan)
    {
        return (uint64)mAtlas[chan].size;
    }
    Vector3DI getAtlasPos(uchar chan, uint64 id);
    Vector3DI getAtlasRes(uchar chan);
    Vector3DI getAtlasPackres(uchar chan);
    Vector3DI getAtlasCnt(uchar chan)
    {

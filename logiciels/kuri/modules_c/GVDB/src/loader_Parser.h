...
// Originally by Chris Wyman (June 9th, 2010)
//
...
class Parser {
  public:
    ...
    char *GetQualifiedFileName(void)
    {
        return fileName;
    }
  ...
  protected:
    char *__ReadLine(
        void);  // A simple call to fgets, storing data internally, and increment our line counter

    void CloseFile(
        void);  // Derived classes may want to go ahead and close the file when they're ready
...
class CallbackParser : public Parser {
  ...
  protected:
    int numCallbacks;
    char *callbackTokens[32];  // Hardcoded to a limit of 32 callbacks.  Silently fails if using
                               // more.

...
#include "Geometry_traits.h"  // kévin FEVV/Wrappings/
#include "properties.h"       // kévin FEVV/Wrappings/
#include <list>
#include <stack>
...
class Enriched_Triangle : public AABB_Kernel::Triangle_3 {
  public:
    ...
    Enriched_Triangle(Facet_handle &_f)
        : AABB_Kernel::Triangle_3(to_K(_f->facet_begin()->vertex()->point() +
...
class BoolPolyhedra {
  ...
  private:
    ...
    void FindCouples()
    {
        // A AABB-tree is built on the facets of one of the polyhedra. A collision test is done
        // with each facet of the other polyhedron.
    ...
    void CutIntersectedFacets()
    {
        ...
        // every intersected facet is triangulated if at least one of the intersections is a
        // segment
        for (FacetId Facet = 0; Facet != m_Inter_tri.size(); ++Facet) {
            if (!m_Inter_tri[Facet].CutList.empty()) {
                ...
                // add the list of intersection points (only happens in case of intersection of two
                // edges)
                ...
                for (int i = 0; i != (int)TriCut.CutList.size(); ++i) {
                    T.add_segment(m_InterPts[TriCut.CutList[i][0]],
                                  m_InterPts[TriCut.CutList[i][1]],
                                  TriCut.CutList[i][0],
                                  TriCut.CutList[i][1]);
                }
                ...
                if (IsExt[0])
                    he->opposite()->facet()->IsExt = true;
                if (IsExt[1])
                    he->next()->opposite()->facet()->IsExt = true;
                if (IsExt[2])
                    he->next()->next()->opposite()->facet()->IsExt = true;
            }
        }
    }
    ...
    void PropagateFacets()
    {
        ...
        for (pFacet = m_pA->facets_begin(); pFacet != m_pA->facets_end(); pFacet++) {
            if (pFacet->IsOK)
                tmpTriangles.push(pFacet);
        }

        // while the stack is not empty, we look the three neighboring facets
        // if these facets has not been validated (IsOK == false), the facet is validated and added
        // to the stack if this facet is taged as a part of the result (IsExt == true), the facet
        ...
        while (!tmpTriangles.empty()) {
            ...
            if (!nf->IsOK) {
                ...
                tmpTriangles.push(nf);
                if (nf->IsExt)
                    add_facet_to_solution(nf, false);
            }
            nf = f->facet_begin()->next()->opposite()->facet();
            if (!nf->IsOK) {
                nf->IsOK = true;
    ...
    void InterTriangleTriangle(FacetId &A, FacetId &B)
    {
        ...
        // compute the position of the three vertices of each triangle regarding the plane of the
        // other positive if the vertex is above negative if the vertex is under zero if the vertex
        // is exactly on the triangle
        ...
        posAbin = ((posA[0] > 0) ? 32 : 0) + ((posA[0] < 0) ? 16 : 0) + ((posA[1] > 0) ? 8 : 0) +
                  ((posA[1] < 0) ? 4 : 0) + ((posA[2] > 0) ? 2 : 0) + ((posA[2] < 0) ? 1 : 0);

        posBbin = ((posB[0] > 0) ? 32 : 0) + ((posB[0] < 0) ? 16 : 0) + ((posB[1] > 0) ? 8 : 0) +
                  ((posB[1] < 0) ? 4 : 0) + ((posB[2] > 0) ? 2 : 0) + ((posB[2] < 0) ? 1 : 0);

        // if the intersection is not a segment, the intersection is not computed
        // the triangles intersects on a point (one vertex on the plane and the two others under or
        // above
        if (posAbin == 5 || posAbin == 10 || posAbin == 17 || posAbin == 34 || posAbin == 20 ||
            posAbin == 40 || posBbin == 5 || posBbin == 10 || posBbin == 17 || posBbin == 34 ||
            posBbin == 20 || posBbin == 40)
            return;
        // no possible intersection (one of the triangle is completely under or above the other
        if (posAbin == 42 || posAbin == 21 || posBbin == 42 || posBbin == 21)
            return;
        // the triangles are coplanar
        if (posAbin == 0)
            return;

        // if an edge of a triangle is on the plane of the other triangle, it is necessary to
        // verify if the two polyhedra are intersecting on these edges, or if it only is a contact
        // to know if the intersection between the triangles must be computed or not. "edgeA" and
        //"edgeB" are codes 0 : the first edge is on the plane 1 : the second edge is on the plane
        // 2 : the third edge is on the plane
        // 3 : there is no edge on the plane
        unsigned short edgeA = 3, edgeB = 3;
        if (posAbin == 1 || posAbin == 2)
            edgeA = 1;  // points 0 and 1 on the plane
        else if (posAbin == 16 || posAbin == 32)
            edgeA = 2;  // points 1 and 2 on the plane
        else if (posAbin == 4 || posAbin == 8)
            edgeA = 0;  // points 2 and 0 on the plane
        if (posBbin == 1 || posBbin == 2)
            edgeB = 1;  // points 0 and 1 on the plane
        else if (posBbin == 16 || posBbin == 32)
            edgeB = 2;  // points 1 and 2 on the plane
        else if (posBbin == 4 || posBbin == 8)
            edgeB = 0;  // points 2 and 0 on the plane
        ...
        if (edgeA != 3 && edgeB == 3) {
            ...
            // if p is negative, the two triangles of the first polyhedron (including edgeA) are on
            // the same side so there is no intersection
            if (p < 0)
                stop = true;
            ...
            // the intersection between fA2 and fB is the same so this couple is removed from the
            // list
        ...
        // if an edge of the second triangle is on the plane
            ...
            // the intersection between fA and fB2 is the same so this couple is removed from the
            // list
        ...
        // if an edge of each triangle is on the plane of the other
            ...
            num_type posA2_A, posB_A, posB2_A, posB_B2, posA_B, posB2_B, posB_A2, posB2_A2,
                posA2_B, posA2_B2;
            ...
            if (nAcnB2 == CGAL::NULL_VECTOR && nA2cnB == CGAL::NULL_VECTOR && nAnB2 * nA2nB > 0)
                stop = true;
            ...
            // if Intersection == false, fA and fA2 are both inside or outside the second
            // polyhedron.
            if (!Intersection)
                stop = true;

            // the intersection between (fA, fB2), (fA2, fB) and (fA2, fB2) are the same so these
            // couples are removed from the list
            rmCouple(fA->Label, fB2->Label);
            rmCouple(fA2->Label, fB->Label);
        ...
        switch (posBbin) {
            ...
            case 41:
                ...
                InterTriangleSegment(&inter[1]);
                break;
            // particular cases : one point on the plane, one point one one side and one point on
            // the other side
            case 6:
            case 9:
                inter[0].he = heB[2];
        ...
        if (IsSegment(inter)) {
            ...
            if (CGAL::cross_product(nA, nB) * (m_InterPts[ptInter[1]] - m_InterPts[ptInter[0]]) *
                    ((invert_direction == true) ? -1 : 1) >
                0) {
                switch (m_BOOP) {
                    case UNION:
    ...
    void IsInTriangle(Info_Inter *inter)
    {
        ...
        // Crash si nous utilisons x == 0 directement.
        // - kévin
        static auto zero = num_type(0);
        // creation of the code for the location of the intersection
        inter->res = 0;
    ...
    bool IsSegment(Info_Inter *inter)
    {
        ...
        if (inter[1].Id != 0xFFFFFFFF) {
            ...
            // if a point or an Id has already be founded, we founded the two distinct valid points
            // (the intersection is a segment) (it is not possible that the two first points are
            // the
            // same)
            if (point || id)
                return true;
        ...
        if (inter[2].Id != 0xFFFFFFFF) {
            ...
            // if a point or a different Id has already be founded, we founded the two distinct
            // valid points (the intersection is a segment) (it is not possible that the two first
            // points are the same)
            if (point || (id && Id != inter[2].Id))
                return true;
            id = true;
            Id = inter[2].Id;
        }
            ...
            // if an Id or a different point has already be founded, we founded the two distinct
            // valid points (the intersection is a segment) (it is not possible that the two first
            // points are the same)
            if ((point && pt != inter[2].pt) || id)
                return true;
            point = true;
            pt = inter[2].pt;
        }
        ...
        if (inter[3].Id != 0xFFFFFFFF) {
            ...
            // valid points (the intersection is a segment) (it is not possible that the two first
            // points are the same)
            if ((point && pt != inter[3].pt) || id)
                return true;
        }
        return false;
    }

    /*! \brief Extracts the segment from a set of four intersection points and store these points
     * in the list of intersecion points \n There must be two valid and distinct points in the set
    ...
    void Store_Intersection(Info_Inter *inter)
    {
        ...
        // if the intersection is on the vertex pointed by the halfedge (he), we update the Id
        // (Label) of this vertex
        if (inter->IsOnVertex)
            he->vertex()->Label = I;

        // the intersection is memorized for each possible couple of (facet, halfedge) concerned by
        // the intersection if the intersection is exactly on the vertex pointed by the halfedge
        // (he), it is necessary to take account of every halfedge pointing to this vertex
        switch (inter->res) {
            ...
            case 1:  // Intersection on the first halfedge of the facet
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    m_Inter_tri[f->Label].RefInter[he->Label] = I;
                    m_Inter_tri[f->Label].RefInter[he->opposite()->Label] = I;
                    m_Inter_tri[f->facet_begin()->opposite()->facet()->Label].RefInter[he->Label] =
                        I;
                    m_Inter_tri[f->facet_begin()->opposite()->facet()->Label]
                        .RefInter[he->opposite()->Label] = I;
                    m_Inter_tri[he->facet()->Label].RefInter[f->facet_begin()->Label] = I;
                    m_Inter_tri[he->facet()->Label].RefInter[f->facet_begin()->opposite()->Label] =
                        I;
                    m_Inter_tri[he->opposite()->facet()->Label].RefInter[f->facet_begin()->Label] =
                        I;
                    m_Inter_tri[he->opposite()->facet()->Label]
                        .RefInter[f->facet_begin()->opposite()->Label] = I;
                }
                else {
                    Halfedge_around_vertex_circulator H_circ = he->vertex_begin(),
                                                      H_end = he->vertex_begin();
                    do {
                        ...
                        m_Inter_tri[f->Label].RefInter[H_circ->opposite()->Label] = I;
                        m_Inter_tri[f->facet_begin()->opposite()->facet()->Label]
                            .RefInter[H_circ->Label] = I;
                        m_Inter_tri[f->facet_begin()->opposite()->facet()->Label]
                            .RefInter[H_circ->opposite()->Label] = I;
                        m_Inter_tri[H_circ->facet()->Label].RefInter[f->facet_begin()->Label] = I;
                        m_Inter_tri[H_circ->facet()->Label]
                            .RefInter[f->facet_begin()->opposite()->Label] = I;
                        H_circ++;
                    } while (H_circ != H_end);
                }
            ...
            case 2:  // Intersection on the second halfedge of the facet
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    m_Inter_tri[f->Label].RefInter[he->Label] = I;
                    m_Inter_tri[f->Label].RefInter[he->opposite()->Label] = I;
                    m_Inter_tri[f->facet_begin()->next()->opposite()->facet()->Label]
                        .RefInter[he->Label] = I;
                    m_Inter_tri[f->facet_begin()->next()->opposite()->facet()->Label]
                        .RefInter[he->opposite()->Label] = I;
                    m_Inter_tri[he->facet()->Label].RefInter[f->facet_begin()->next()->Label] = I;
                    m_Inter_tri[he->facet()->Label]
                        .RefInter[f->facet_begin()->next()->opposite()->Label] = I;
                    m_Inter_tri[he->opposite()->facet()->Label]
                        .RefInter[f->facet_begin()->next()->Label] = I;
                    m_Inter_tri[he->opposite()->facet()->Label]
                        .RefInter[f->facet_begin()->next()->opposite()->Label] = I;
                }
                else {
            ...
            case 3:  // Intersection on the first and second halfedge of the facet (vertex pointed
                     // by the first halfedge)
            {
            ...
            {
                m_Inter_tri[f->Label].PtList.insert(I);
                m_Inter_tri[f->facet_begin()->next()->next()->opposite()->facet()->Label]
                    .PtList.insert(I);
                if (!inter->IsOnVertex) {
                    m_Inter_tri[he->facet()->Label].PtList.insert(I);
                    m_Inter_tri[he->opposite()->facet()->Label].PtList.insert(I);
                    ...
                    m_Inter_tri[f->facet_begin()->next()->next()->opposite()->facet()->Label]
                        .RefInter[he->opposite()->Label] = I;
                    m_Inter_tri[he->facet()->Label]
                        .RefInter[f->facet_begin()->next()->next()->Label] = I;
                    m_Inter_tri[he->facet()->Label]
                        .RefInter[f->facet_begin()->next()->next()->opposite()->Label] = I;
                    m_Inter_tri[he->opposite()->facet()->Label]
                        .RefInter[f->facet_begin()->next()->next()->Label] = I;
                    m_Inter_tri[he->opposite()->facet()->Label]
                        .RefInter[f->facet_begin()->next()->next()->opposite()->Label] = I;
            ...
            case 5:  // Intersection on the first and third halfedge of the facet (vertex pointed
                     // by the third halfedge)
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    Halfedge_around_vertex_circulator
                        H_circ = f->facet_begin()->next()->next()->vertex_begin(),
                    ...
                    do {
                        Halfedge_around_vertex_circulator
                            F_circ = f->facet_begin()->next()->next()->vertex_begin(),
            ...
            case 6:  // Intersection on the second and third halfedge of the facet (vertex pointed
                     // by the second halfedge)
            {
                ...
                if (!inter->IsOnVertex) {
                    ...
                    Halfedge_around_vertex_circulator
                        H_circ = f->facet_begin()->next()->vertex_begin(),
                    ...
                    do {
                        Halfedge_around_vertex_circulator
                            F_circ = f->facet_begin()->next()->vertex_begin(),
    ...
    {
        // if the facet contains an intersection point but no intersection segment, the facet must
        // be triangulate before
        if (pFacet->Label < m_Inter_tri.size()) {
            ...
            // the facet is added to the result. If the facet belongs to the second polyhedron, and
            // if the Boolean operation is a Subtraction, it is necessary to invert the orientation
            // of the facet.
            if (m_BOOP == MINUS && !facet_from_A)
                ppbuilder.add_triangle(pFacet, true);
            else
                ppbuilder.add_triangle(pFacet, false);
        }
...
#    endif
    ...
    /*! \brief Writes a report containing the computation time of the diffrent parts of the
     * algorithm \param m_out : The result polyhedron*/
    void WriteData(const EnrichedPolyhedron &m_out)
    {
        ...
        ofstrtime << "   Initialization :             " << tr(duration_Init) << " s" << std::endl;
        ofstrtime << " + Finding the Intersections :  " << tr(duration_FindCouples) << " s"
                  << std::endl;
        ofstrtime << " + Compute the Intersections :  " << tr(duration_ComputeIntersections)
                  << " s" << std::endl;
        ofstrtime << " + Cut the Intersected Facets : " << tr(duration_CutIntersectedFacets)
                  << " s" << std::endl;
        ofstrtime << " + Complete the result :        " << tr(duration_PropagateFacets) << " s"
                  << std::endl;
        ofstrtime << " + Create the polyhedron :      " << tr(duration_delegate) << " s"
                  << std::endl;
        ofstrtime << "---------------------------------------" << std::endl;
        ofstrtime << " Total :                        " << tr(duration_total) << " s" << std::endl;
        ...
        ofstrtime << "Number of Facets :                   " << m_pA->size_of_facets()
                  << std::endl;
        ...
        ofstrtime << "Number of Facets :                   " << m_pB->size_of_facets()
                  << std::endl;
        ...
        ofstrtime << "Number of Facets :                   " << m_out.size_of_facets()
                  << std::endl;

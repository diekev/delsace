...
/// (refer to \ref GenericPropertyMapConceptPage)
inline bool has_map(const PMapsContainer &pmaps, const std::string &map_name)
{
...
class Assoc_property_map : public boost::associative_property_map<std::map<KeyType, ValueType>> {
  public:
    // default constructor
    Assoc_property_map(void) : map_ptr(new std::map<KeyType, ValueType>)
    {
        ...
        // object
        boost::associative_property_map<std::map<KeyType, ValueType>>::operator=(
            boost::associative_property_map<std::map<KeyType, ValueType>>(*map_ptr));
    ...
    void infos(void)
    {
        std::cout << "this=" << this << "  map.size()=" << (*map_ptr).size() << std::endl;
    }

  private:
    boost::shared_ptr<std::map<KeyType, ValueType>> map_ptr;  // the real associative container
};
...
struct Vertex_pmap_traits {
    typedef typename boost::graph_traits<MeshT>::vertex_descriptor vertex_key_type;
    typedef typename FEVV::Assoc_property_map<vertex_key_type, ValueT> pmap_type;
...
struct Halfedge_pmap_traits {
    typedef typename boost::graph_traits<MeshT>::halfedge_descriptor halfedge_key_type;
    typedef typename FEVV::Assoc_property_map<halfedge_key_type, ValueT> pmap_type;
...
struct _PMap_traits<MeshT, FEVV::halfedge_normal_t> {
    typedef typename FEVV::Geometry_traits<MeshT>::Vector value_type;
    typedef typename Halfedge_pmap_traits<MeshT, value_type>::pmap_type pmap_type;
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename PropertyT, typename MeshT>
typename PMap_traits<PropertyT, MeshT>::pmap_type make_property_map(PropertyT /*p*/,
                                                                    const MeshT &m)
{
#if 0
	// DBG
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename PropertyT, typename MeshT>
typename PMap_traits<PropertyT, MeshT>::pmap_type get_property_map(PropertyT p,
                                                                   const MeshT &,
                                                                   const PMapsContainer &pmaps)
{
    ...
    return boost::any_cast<typename PMap_traits<PropertyT, MeshT>::pmap_type>(pmaps.at(map_name));
}

/// Store a property map in the property maps container
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename PropertyT, typename MeshT>
void put_property_map(PropertyT p,
                      const MeshT &,
...
/// if it exists
/// (refer to \ref GenericPropertyMapConceptPage)
inline void remove_property_map_by_name(const std::string &name, PMapsContainer &pmaps)
{
    pmaps.erase(name);
}
...
/// (refer to \ref GenericPropertyMapConceptPage)
inline std::vector<std::string> list_property_maps(const PMapsContainer &pmaps)
{
...
/// (refer to \ref GenericPropertyMapConceptPage)
inline void print_property_maps(const PMapsContainer &pmaps)
{
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename MeshT, typename ValueT>
typename Vertex_pmap_traits<MeshT, ValueT>::pmap_type make_vertex_property_map(const MeshT &m)
{
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename MeshT, typename ValueT>
typename Face_pmap_traits<MeshT, ValueT>::pmap_type make_face_property_map(const MeshT &m)
{
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename MeshT, typename ValueT>
typename Edge_pmap_traits<MeshT, ValueT>::pmap_type make_edge_property_map(const MeshT &m)
{
...
/// (refer to \ref GenericPropertyMapConceptPage)
template <typename MeshT, typename ValueT>
typename Halfedge_pmap_traits<MeshT, ValueT>::pmap_type make_halfedge_property_map(const MeshT &m)
{

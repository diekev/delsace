...
class Enriched_face_base : public Fb {
  ...
  public:
    ...
    Enriched_face_base(Vertex_handle v0,
                       Vertex_handle v1,
                       Vertex_handle v2,
                       Face_handle n0,
                       Face_handle n1,
...
//       is not a public release).
     ...
     */
    typedef typename CGAL::Constrained_Delaunay_triangulation_2<K, Tri_DS, Itag>
  ...
  public:
    ...
    Triangulation(Halfedge_handle &he, Vector_exact &norm_dir)
    {
        ...
        v2 = add_new_pt(point_to_exact(he->next()->vertex()->point()),
                        he->next()->vertex()->Label);
        v3 = add_new_pt(point_to_exact(he->next()->next()->vertex()->point()),
                        he->next()->next()->vertex()->Label);
        ...
        if (v2->get_Label() == 0xFFFFFFFF)
            v2->set_Label(0xFFFFFFFE);
        if (v3->get_Label() == 0xFFFFFFFF)
            v3->set_Label(0xFFFFFFFD);
    }

    /*!
     * \brief Compute the orthogonal projection of a point to the plane defined by the longest
     * coordinate of the normal vector \param p : the point (in 3d) \return The projection as a 2d
     * point
     */
    ...
    std::vector<std::vector<uint64_t>> get_triangles(bool inv_triangles, bool *IsExt)
    {
        ...
        // the constrained segments are oriented, we search the triangle (c1, c2, X), (X, c1, c2)
        // or (c2, X, c1) where c1 and c2 are the two points related to the last constrained
        // segment added, and X another point this triangle belongs to the result (thanks to the
        ...
        // dans le cas particulier ou la frontiere se trouve exactement sur un bord de la
        // triangulation, et que ce triangle n'appartient pas a la triangulation, on d�marrera avec
        // l'autre triangle incluant le segment c1, c2 (et donc, n'appartenant pas au r�sultat

        // if the segment is exactly on the border of the triangulation, the triangle could be
        // outside the triangulation in that case, we will search the other triangle including the
        // points c1 and c2 this triangle does not belong to the result
        ...
        // we decide for all the triangles, if they belongs to the result, starting from the first
        // triangle f, by moving on the triangulation using the connectivity between the triangles.
        ...
        while (!sfh.empty()) {
            ...
            if (f->get_Ext()) {
                ...
                if (f->has_vertex(v1, i) &&
                    f->neighbor(f->ccw(i))->has_vertex(ct.infinite_vertex()))
                    IsExt[0] = true;
                if (f->has_vertex(v2, i) &&
                    f->neighbor(f->ccw(i))->has_vertex(ct.infinite_vertex()))
                    IsExt[1] = true;
                if (f->has_vertex(v3, i) &&
                    f->neighbor(f->ccw(i))->has_vertex(ct.infinite_vertex()))
                    IsExt[2] = true;
            ...
            for (i = 0; i != 3; i++) {
                if (!(f->neighbor(i)->get_OK() ||
                      f->neighbor(i)->has_vertex(ct.infinite_vertex()))) {

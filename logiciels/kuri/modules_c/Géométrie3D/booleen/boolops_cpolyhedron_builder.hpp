...
class CPolyhedron_from_polygon_builder_3 : public CGAL::Modifier_base<HDS> {
  ...
  public:
    ...
    {
        // initially, the label of the vertices is 0xFFFFFFFF. if a vertex is added to the result,
        // the tag is set to the number of vertices added.
        ...
        if (!invert) {
            ...
            if (he->next()->next()->vertex()->Label == 0xFFFFFFFF)
                add_vertex(he->next()->next()->vertex()->point(),
    ...
    const std::vector<Point_3> &get_vertices() const
    {
        return m_Sorted_vertices;
    }

    /*! \brief List of the facets*/
    const std::vector<Indices> &get_triangles() const
    {
        return m_Facets_indices;
    }

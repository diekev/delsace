...
template <typename T, int Size>
struct vec_base : public vec_struct_base<T, Size> {
...
#define BLI_ENABLE_IF_VEC(_size, _test) int S = _size, BLI_ENABLE_IF((S _test))
    ...
    vec_base(vec_base<U, 2> xy, T z, T w)
        : vec_base(
              static_cast<T>(xy.x), static_cast<T>(xy.y), static_cast<T>(z), static_cast<T>(w))
    {
    }
    ...
    vec_base(T x, vec_base<U, 2> yz, T w)
        : vec_base(
              static_cast<T>(x), static_cast<T>(yz.x), static_cast<T>(yz.y), static_cast<T>(w))
    {
    }
    ...
    vec_base(T x, T y, vec_base<U, 2> zw)
        : vec_base(
              static_cast<T>(x), static_cast<T>(y), static_cast<T>(zw.x), static_cast<T>(zw.y))
    {
    }
...
#undef BLI_ENABLE_IF_VEC
    ...
    template <typename U>
    explicit vec_base(const vec_base<U, Size> &vec)
    {
        for (int i = 0; i < Size; i++) {

                        ...
                        if (etris != nullptr) {
                            for (int i : etris->index_range()) {
                                ...
                                if (t_other != tcand && pinfo.tri_is_assigned(t_other)) {
                                    ...
                                    if (pinfo.patch_patch_edge(cur_patch_index, p_other).v0() ==
                                        nullptr) {
                                        pinfo.add_new_patch_patch_edge(
                                            cur_patch_index, p_other, e);
                                        if (dbg_level > 1) {
                                            std::cout << "added patch_patch_edge ("
                                                      << cur_patch_index << "," << p_other
                                                      << ") = " << e << "\n";
                                        }
                                    }
                                }
        ...
        if (*r_follow_cell == NO_INDEX && *rnext_prev_cell == NO_INDEX) {
            ...
            *rnext_prev_cell = c;
            ...
            if (dbg_level > 0) {
                ...
                std::cout << "  p" << r_index << "." << (r_flipped ? "cell_below" : "cell_above")
                          << " = c" << c << "\n";
                std::cout << "  p" << rnext_index << "."
            ...
            *r_follow_cell = c;
            ...
            if (*r_follow_cell != *rnext_prev_cell) {
                ...
                if (follow_cell_num_patches >= prev_cell_num_patches) {
                    if (dbg_level > 0) {
                        std::cout << " merge cell " << *rnext_prev_cell << " into cell "
                                  << *r_follow_cell << "\n";
                    }
                ...
                if (dbg_level > 0) {
                    std::cout << " merge cell " << *r_follow_cell << " into cell "
                              << *rnext_prev_cell << "\n";
                }
                                                            ...
                                                            IMeshArena *arena)
{
    ...
    for (int comp_other : components.index_range()) {
        ...
        if (dbg_level > 0) {
            std::cout << "closest tri to comp=" << comp << " in comp_other=" << comp_other
                      << " is t" << nearest_tri << "\n";
            const Face *tn = tm.face(nearest_tri);
            std::cout << "tri = " << tn << "\n";
            std::cout << "  (" << tn->vert[0]->co << "," << tn->vert[1]->co << ","
                      << tn->vert[2]->co << ")\n";
        }
                                    ...
                                    IMeshArena *arena)
{
    ...
    for (int comp : comp_cont.index_range()) {
        if (comp_cont[comp].size() == 0) {
            ...
            if (dbg_level > 0) {
                std::cout << "merge comp " << comp << "'s ambient cell=" << comp_ambient
                          << " to cell " << cont_cell << "\n";
            }
                                          ...
                                          IMeshArena *arena)
{
    ...
    for (int p : pinfo.index_range()) {
        const Patch &patch = pinfo.patch(p);
        if (cinfo.cell(patch.cell_above).zero_volume() ||
            cinfo.cell(patch.cell_below).zero_volume()) {
            adj_to_zv[p] = true;
        }
        else {
            adj_to_zv[p] = false;
                                   ...
                                   Array<float> &in_shape)
{
    ...
    const float test_rays[rays_num][3] = {{r1, ra, rb},
                                          {-r1, -ra, -rb},
                                          {rb, r1, ra},
                                          {-rb, -r1, -ra},
                                          {ra, rb, r1},
    ...
    for (int i = 0; i < rays_num; ++i) {
        ...
        BLI_bvhtree_ray_cast_all(
            tree, co, test_rays[i], 0.0f, FLT_MAX, inside_shape_callback, &data);
        if (dbg_level > 0) {
    ...
    threading::parallel_for(IndexRange(tm.face_size()), grainsize, [&](IndexRange range) {
        ...
        for (int t : range) {
            ...
            for (int other_shape = 0; other_shape < nshapes; ++other_shape) {
                ...
                if (dbg_level > 0) {
                    std::cout << "test point is " << (inside ? "inside" : "outside")
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            if (me_index == -1) {
                ...
                if (dbg_level > 1) {
                    ...
                    std::cout << "  len_squared = " << new_me.len_squared
                              << "  orig = " << new_me.orig << ", is_intersect"
                              << new_me.is_intersect << ", dissolvable = " << new_me.dissolvable
                              << "\n";
                }
            }
            ...
            if (me.dissolvable && tri.edge_orig[i] != NO_INDEX) {
                if (dbg_level > 1) {
                    std::cout << "reassigning orig to " << tri.edge_orig[i]
                              << ", dissolvable = false\n";
                }
    ...
    for (int f : imesh_out.face_index_range()) {
        ...
        for (int i : face.index_range()) {
            ...
            if (dissolve[v_index]) {
                ...
                if (f_n1 != nullptr) {
                    /* Already has a neighbor in another face; can't dissolve unless they are the
                     * same. */
                                                 ...
                                                 IMeshArena *arena)
{
    ...
    for (int in_f : imesh_in.face_index_range()) {
        ...
        face_output_face[in_f] = merge_tris_for_face(
            face_output_tris[in_f], tm_out, imesh_in, arena);

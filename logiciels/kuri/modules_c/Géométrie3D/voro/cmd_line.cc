...
enum blocks_mode { none, length_scale, specified };
...
void cmd_line_output(c_loop &vl,
                     c_class &con,
                     const char *format,
                     FILE *outfile,
                     FILE *gnu_file,
        ...
        if (vl.start())
            do
                if (con.compute_cell(c, vl)) {
                    vl.pos(pid, x, y, z, r);
                    if (outfile != nullptr)
                        c.output_custom(format, pid, x, y, z, r, outfile);
                    if (gnu_file != nullptr)
                        c.draw_gnuplot(x, y, z, gnu_file);
                    if (povp_file != nullptr) {
                        fprintf(povp_file, "// id %d\n", pid);
                        if (ps == 4)
                            fprintf(povp_file, "sphere{<%g,%g,%g>,%g}\n", x, y, z, r);
                        else
                            fprintf(povp_file, "sphere{<%g,%g,%g>,s}\n", x, y, z);
                    }
...
int main(int argc, char **argv)
{
    ...
    pre_container *pcon = nullptr;
    pre_container_poly *pconp = nullptr;
    wall_list wl;
    ...
    if (bm == none) {
        if (polydisperse) {
            pconp = new pre_container_poly(
                ax, bx, ay, by, az, bz, xperiodic, yperiodic, zperiodic);
            ...
            if (ls < tolerance) {
                ...
                if (ls < 0) {
                    ...
                    fprintf(
                        ...
                        "the particle length scale, or recompile with a different limit.\n",
                        tolerance);
                }
                wl.deallocate();
                return VOROPP_CMD_LINE_ERROR;
        ...
        if (nxf * nyf * nzf > max_regions) {
            fprintf(stderr,
                    ...
                    "Either increase the particle length scale, or recompile with an "
                    "increased\nmaximum.",
                    max_regions);
            wl.deallocate();
    ...
    const char *c_str = (custom_output == 0 ? (polydisperse ? "%i %q %v %r" : "%i %q %v") :
                                              argv[custom_output]);
    ...
    if (polydisperse) {
        if (ordered) {
            particle_order vo;
            container_poly con(
                ax, bx, ay, by, az, bz, nx, ny, nz, xperiodic, yperiodic, zperiodic, init_mem);
            con.add_wall(wl);
            if (bm == none) {
                pconp->setup(vo, con);
                delete pconp;
                ...
                con.import(vo, argv[i + 6]);

            c_loop_order vlo(con, vo);
            cmd_line_output(
                vlo, con, c_str, outfile, gnu_file, povp_file, povv_file, verbose, vol, vcc, tp);
        }
        else {
            container_poly con(
                ax, bx, ay, by, az, bz, nx, ny, nz, xperiodic, yperiodic, zperiodic, init_mem);
            ...
            c_loop_all vla(con);
            cmd_line_output(
                vla, con, c_str, outfile, gnu_file, povp_file, povv_file, verbose, vol, vcc, tp);
        }
    }
    else {
        if (ordered) {

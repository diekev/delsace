 ...
 *                particle. */
container_periodic_base::container_periodic_base(double bx_,
                                                 double bxy_,
                                                 double by_,
                                                 double bxz_,
                                                 double byz_,
    ...
    : unitcell(bx_, bxy_, by_, bxz_, byz_, bz_),
      voro_base(nx_, ny_, nz_, bx_ / nx_, by_ / ny_, bz_ / nz_), ey(int(max_uv_y *ysp + 1)),
      ez(int(max_uv_z *zsp + 1)), wy(ny + ey), wz(nz + ez), oy(ny + 2 * ey), oz(nz + 2 * ez),
      oxyz(nx * oy * oz), id(new int *[oxyz]), p(new double *[oxyz]), co(new int[oxyz]),
      mem(new int[oxyz]), img(new char[oxyz]), init_mem(init_mem_), ps(ps_)
{
    int i, j, k, l;
 ...
 *			    coordinate directions.
 * \param[in] init_mem_ the initial memory allocation for each block. */
container_periodic::container_periodic(double bx_,
                                       double bxy_,
                                       double by_,
                                       double bxz_,
                                       double byz_,
 ...
 *			    coordinate directions.
 * \param[in] init_mem_ the initial memory allocation for each block. */
container_periodic_poly::container_periodic_poly(double bx_,
                                                 double bxy_,
                                                 double by_,
                                                 double bxz_,
                                                 double byz_,
 ...
 * 			  in, with (0,0,0) corresponding to the primary domain.
 */
void container_periodic_poly::put(
    int n, double x, double y, double z, double r, int &ai, int &aj, int &ak)
{
    int ijk;
    put_locate_block(ijk, x, y, z, ai, aj, ak);
    id[ijk][co[ijk]] = n;
 ...
 * \param[in] r the radius of the particle. */
void container_periodic_poly::put(
    particle_order &vo, int n, double x, double y, double z, double r)
{
    int ijk;
    put_locate_block(ijk, x, y, z);
    id[ijk][co[ijk]] = n;
 ...
 * \return True if the particle can be successfully placed into the container,
 * false otherwise. */
void container_periodic_base::put_locate_block(
    int &ijk, double &x, double &y, double &z, int &ai, int &aj, int &ak)
{
...
inline void container_periodic_base::remap(int &ai,
                                           int &aj,
                                           int &ak,
                                           int &ci,
                                           int &cj,
...
bool container_periodic::find_voronoi_cell(
    double x, double y, double z, double &rx, double &ry, double &rz, int &pid)
{
 ...
 * \return True if a particle was found. If the container has no particles,
 * then the search will not find a Voronoi cell and false is returned. */
bool container_periodic_poly::find_voronoi_cell(
    double x, double y, double z, double &rx, double &ry, double &rz, int &pid)
{
    int ai, aj, ak, ci, cj, ck, ijk;
    particle_record w;
            ...
            for (i = 0; i < nx; i++, l++)
                if (mem[l] > 0) {
                    ...
                    for (pp = p[l], c = 0; c < co[l]; c++, pp += ps)
                        if (*pp < mix || *pp > max || pp[1] < miy || pp[1] > may || pp[2] < miz ||
                            pp[2] > maz)
...
void container_periodic_base::create_vertical_image(int di, int dj, int dk)
{
    ...
    int fi = qi - qidiv * nx, fj = qj - qjdiv * ny, fijk = fi + nx * (fj + oy * (dk - ima * nz)),
        fijk2;
    ...
    if (fj == wy - 1) {
        ...
        int dqidiv = step_div(qi, nx) - qidiv;
        qidiv += dqidiv;
        fi = qi - qidiv * nx;
        fijk += fi;
        disx += bxy + bx * dqidiv;

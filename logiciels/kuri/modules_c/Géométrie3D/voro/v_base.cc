...
voro_base::voro_base(int nx_, int ny_, int nz_, double boxx_, double boxy_, double boxz_)
    : nx(nx_), ny(ny_), nz(nz_), nxy(nx_ * ny_), nxyz(nxy * nz_), boxx(boxx_), boxy(boxy_),
      boxz(boxz_), xsp(1 / boxx_), ysp(1 / boxy_), zsp(1 / boxz_),
    ...
    for (zlo = 0, zhi = zstep, lz = 0; lz < wl_hgrid; zlo = zhi, zhi += zstep, lz++) {
        for (ylo = 0, yhi = ystep, ly = 0; ly < wl_hgrid; ylo = yhi, yhi += ystep, ly++) {
            for (xlo = 0, xhi = xstep, lx = 0; lx < wl_hgrid; xlo = xhi, xhi += xstep, lx++) {
                minr = large_number;
                for (q = static_cast<int>(e[0] + 1); q < wl_seq_length; q++) {
                    f = e[q];
                    i = static_cast<int>((f & 127) - 64);
                    j = static_cast<int>((f >> 7 & 127) - 64);
                    k = static_cast<int>((f >> 14 & 127) - 64);
                    if ((f & b2) == b2) {
...
void voro_base::compute_minimum(double &minr,
                                double &xlo,
                                double &xhi,
                                double &ylo,
                                double &yhi,

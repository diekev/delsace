...
voro_compute<c_class>::voro_compute(c_class &con_, int hx_, int hy_, int hz_)
    : con(con_), boxx(con_.boxx), boxy(con_.boxy), boxz(con_.boxz), xsp(con_.xsp), ysp(con_.ysp),
      zsp(con_.zsp), hx(hx_), hy(hy_), hz(hz_), hxy(hx_ * hy_), hxyz(hxy * hz_), ps(con_.ps),
      id(con_.id), p(con_.p), co(con_.co), bxsq(boxx * boxx + boxy * boxy + boxz * boxz), mv(0),
      qu_size(3 * (3 + hxy + hz * (hx + hy))), wl(con_.wl), mrad(con_.mrad),
      mask(new uint32_t[hxyz]), qu(new int[qu_size]), qu_l(qu + qu_size)
{
    reset_mask();
}
...
template <class c_class>
inline void voro_compute<c_class>::scan_all(
    int ijk, double x, double y, double z, int di, int dj, int dk, particle_record &w, double &mrs)
{
    auto in_block = false;

    for (int l = 0; l < co[ijk]; l++) {
        auto x1 = p[ijk][ps * l] - x;
        auto y1 = p[ijk][ps * l + 1] - y;
        auto z1 = p[ijk][ps * l + 2] - z;
        auto rs = con.r_current_sub(x1 * x1 + y1 * y1 + z1 * z1, ijk, l);
...
template <class c_class>
void voro_compute<c_class>::find_voronoi_cell(
    double x, double y, double z, int ci, int cj, int ck, int ijk, particle_record &w, double &mrs)
{
 ...
 */
template <class c_class>
inline void voro_compute<c_class>::scan_bits_mask_add(
    uint32_t q, uint32_t *mijk, int ei, int ej, int ek, int *&qu_e)
{
    const uint32_t b1 = 1 << 21, b2 = 1 << 22, b3 = 1 << 24, b4 = 1 << 25, b5 = 1 << 27,
                   b6 = 1 << 28;
    if ((q & b2) == b2) {
        if (ei > 0) {
            *(mijk - 1) = mv;
...
bool voro_compute<c_class>::compute_cell(v_cell &c, int ijk, int s, int ci, int cj, int ck)
{
    ...
    auto next_count = 3;
    auto count_p = (const_cast<int *>(count_list));
        ...
        if (co[ijk] > 0) {
            ...
            if (!con.r_ctest(crs, mrs)) {
                ...
                do {
                    ...
                    if (con.r_scale_check(rs, mrs, ijk, l) &&
        ...
        if (ei > i) {
            ...
            if (ej > j) {
                ...
                if (ek > k) {
                    ...
                    voro_fatal_error("Compute cell routine revisiting central block, which should "
                                     "never\nhappen.",
...
template <class c_class>
template <class v_cell>
bool voro_compute<c_class>::corner_test(
    v_cell &c, double xl, double yl, double zl, double xh, double yh, double zh)
{
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::edge_x_test(
    v_cell &c, double x0, double yl, double zl, double x1, double yh, double zh)
{
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::edge_y_test(
    v_cell &c, double xl, double y0, double zl, double xh, double y1, double zh)
{
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::edge_z_test(
    v_cell &c, double xl, double yl, double z0, double xh, double yh, double z1)
{
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::face_x_test(
    v_cell &c, double xl, double y0, double z0, double y1, double z1)
{
    con.r_prime(xl * xl);
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::face_y_test(
    v_cell &c, double x0, double yl, double z0, double x1, double z1)
{
    con.r_prime(yl * yl);
...
template <class c_class>
template <class v_cell>
inline bool voro_compute<c_class>::face_z_test(
    v_cell &c, double x0, double y0, double zl, double x1, double y1)
{
    con.r_prime(zl * zl);
 ...
 */
template <class c_class>
bool voro_compute<c_class>::compute_min_max_radius(int di,
                                                   int dj,
                                                   int dk,
                                                   double fx,
                                                   double fy,
    ...
    if (di > 0) {
        ...
        if (dj > 0) {
            ...
            if (dk > 0) {
                ...
                crs = 0;
                voro_fatal_error("Min/max radius function called for central block, which should "
                                 "never\nhappen.",
                                 VOROPP_INTERNAL_ERROR);
            }
...
template <class c_class>
bool voro_compute<c_class>::compute_min_radius(
    int di, int dj, int dk, double fx, double fy, double fz, double mrs)
{
    double t, crs;
...
template bool voro_compute<container>::compute_cell(
    voronoicell_neighbor &, int, int, int, int, int);
template void voro_compute<container>::find_voronoi_cell(
    double, double, double, int, int, int, int, particle_record &, double &);
...
template bool voro_compute<container_poly>::compute_cell(
    voronoicell_neighbor &, int, int, int, int, int);
template void voro_compute<container_poly>::find_voronoi_cell(
    double, double, double, int, int, int, int, particle_record &, double &);

// Explicit template instantiation
template voro_compute<container_periodic>::voro_compute(container_periodic &, int, int, int);
template voro_compute<container_periodic_poly>::voro_compute(container_periodic_poly &,
                                                             int,
                                                             int,
                                                             int);
template bool voro_compute<container_periodic>::compute_cell(
    voronoicell &, int, int, int, int, int);
template bool voro_compute<container_periodic>::compute_cell(
    voronoicell_neighbor &, int, int, int, int, int);
template void voro_compute<container_periodic>::find_voronoi_cell(
    double, double, double, int, int, int, int, particle_record &, double &);
template bool voro_compute<container_periodic_poly>::compute_cell(
    voronoicell &, int, int, int, int, int);
template bool voro_compute<container_periodic_poly>::compute_cell(
    voronoicell_neighbor &, int, int, int, int, int);
template void voro_compute<container_periodic_poly>::find_voronoi_cell(
    double, double, double, int, int, int, int, particle_record &, double &);

...
class voronoicell_base {
  public:
    ...
    voronoicell_base(voronoicell_base const &) = default;
    voronoicell_base(voronoicell_base &&) = default;

    voronoicell_base &operator=(voronoicell_base const &) = default;
    voronoicell_base &operator=(voronoicell_base &&) = default;
    ...
    void init_tetrahedron_base(double x0,
                               double y0,
                               double z0,
                               double x1,
                               double y1,
    ...
    void output_custom(
        const char *format, int i, double x, double y, double z, double r, FILE *fp = stdout);
    ...
    inline int cycle_up(int a, int pi)
    {
        return a == nu[pi] - 1 ? 0 : a + 1;
    }
    /** This is a simple inline function for picking out the index
    ...
    inline int cycle_down(int a, int pi)
    {
        return a == 0 ? nu[pi] - 1 : a - 1;
    }
...
class voronoicell : public voronoicell_base {
  public:
    ...
    inline voronoicell &operator=(voronoicell &c)
    {
     ...
     */
    inline void init_tetrahedron(double x0,
                                 double y0,
                                 double z0,
                                 double x1,
                                 double y1,
  ...
  private:
    ...
    inline void n_set_pointer(int pi, int n){};
    ...
    inline void n_neighbors(dls::tableau<int> &v)
    {
        v.efface();
    };
...
class voronoicell_neighbor : public voronoicell_base {
  public:
    ...
    voronoicell_neighbor(voronoicell_neighbor const &) = default;
    voronoicell_neighbor(voronoicell_neighbor &&) = default;

    ~voronoicell_neighbor();
    voronoicell_neighbor &operator=(voronoicell &c);
    voronoicell_neighbor &operator=(voronoicell_neighbor &c);
    ...
    void init_tetrahedron(double x0,
                          double y0,
                          double z0,
                          double x1,
                          double y1,

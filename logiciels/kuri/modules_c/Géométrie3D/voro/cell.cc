...
voronoicell_neighbor &voronoicell_neighbor::operator=(voronoicell &c)
{
    voronoicell_base *vb = (static_cast<voronoicell_base *>(&c));
 ...
 * class, extending memory allocation if necessary.
 * \param[in] c the class to copy. */
voronoicell_neighbor &voronoicell_neighbor::operator=(voronoicell_neighbor &c)
{
    voronoicell_base *vb = (static_cast<voronoicell_base *>(&c));
    check_memory_for_copy(*this, vb);
    copy(vb);
...
void voronoicell_base::add_memory(vc_class &vc, int i, int *stackp2)
{
    ...
    if (mem[i] == 0) {
        ...
        if (mem[i] > max_n_vertices)
            voro_fatal_error("Point memory allocation exceeded absolute maximum",
                             VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_vertices(vc_class &vc)
{
    ...
    if (i > max_vertices)
        voro_fatal_error("Vertex memory allocation exceeded absolute maximum",
                         VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_vorder(vc_class &vc)
{
    ...
    if (i > max_vertex_order)
        voro_fatal_error("Vertex order memory allocation exceeded absolute maximum",
                         VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_ds(int *&stackp)
{
    ...
    if (current_delete_size > max_delete_size)
        voro_fatal_error("Delete stack 1 memory allocation exceeded absolute maximum",
                         VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::add_memory_ds2(int *&stackp2)
{
    ...
    if (current_delete2_size > max_delete2_size)
        voro_fatal_error("Delete stack 2 memory allocation exceeded absolute maximum",
                         VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
...
void voronoicell_base::init_base(
...
void voronoicell_base::init_tetrahedron_base(double x0,
                                             double y0,
                                             double z0,
                                             double x1,
                                             double y1,
 ...
 * \param[in,out] up */
template <class vc_class>
inline bool voronoicell_base::search_for_outside_edge(vc_class &vc, int &r_up)
{
    ...
    for (dsp = ds; dsp < stackp; dsp++) {
        ...
        for (edp = ed[cp]; edp < ed[cp] + nu[cp]; edp++) {
            ...
            if (qp != -1 && ed[qp][nu[qp]] != -1) {
                if (stackp == stacke) {
                    int dis = static_cast<int>(stackp - dsp);
                    add_memory_ds(stackp);
                    dsp = ds + dis;
                }
                *(stackp++) = qp;
        ...
        for (j = 0; j < nu[i]; j++) {
            ...
            if (k >= 0) {
                ...
                while (m != i) {
                    ...
                    wy = pts[3 * m + 1] - pts[1];
                    wz = pts[3 * m + 2] - pts[2];
                    vol += ux * vy * wz + uy * vz * wx + uz * vx * wy - uz * vy * wx -
                           uy * vx * wz - ux * vz * wy;
                    k = m;
                    l = n;
                    vx = wx;
        ...
        for (j = 0; j < nu[i]; j++) {
            ...
            if (k >= 0) {
                ...
                while (m != i) {
                    ...
                    wy = pts[3 * m + 1] - pts[1];
                    wz = pts[3 * m + 2] - pts[2];
                    tvol = ux * vy * wz + uy * vz * wx + uz * vx * wy - uz * vy * wx -
                           uy * vx * wz - ux * vz * wy;
                    vol += tvol;
                    cx += (wx + vx - ux) * tvol;
                    cy += (wy + vy - uy) * tvol;
...
inline void voronoicell_base::reset_edges()
{
    ...
    for (i = 0; i < p; i++)
        for (j = 0; j < nu[i]; j++) {
            if (ed[i][j] >= 0)
                voro_fatal_error("Edge reset routine found a previously untested edge",
...
int voronoicell_base::check_marginal(int n, double &ans)
{
    ...
    if (n_marg == current_marginal) {
        ...
        if (current_marginal > max_marginal)
            voro_fatal_error("Marginal case buffer allocation exceeded absolute maximum",
                             VOROPP_MEMORY_ERROR);
#if VOROPP_VERBOSE >= 2
 ...
 * \return False if the plane does not intersect the plane, true if it does. */
inline bool voronoicell_base::plane_intersects_track(
    double x, double y, double z, double rsq, double g)
{
...
void voronoicell_base::output_custom(
    const char *format, int i, double x, double y, double z, double r, FILE *fp)
{
        ...
        if (*fmp == '%') {
            ...
            switch (*fmp) {
                ...
                case 'p':
                    ...
                    break;
                case 'P':
                    output_vertices(x, y, z, fp);
                    break;
                case 'o':
                    output_vertex_orders(fp);
                    break;
                case 'm':
                    ...
                    // Edge-related output
                case 'g':
                    fprintf(fp, "%d", number_of_edges());
                    break;
                case 'E':
                    fprintf(fp, "%g", total_edge_distance());
                    break;
                case 'e':
                    face_perimeters(vd);
                    ...
                    // Face-related output
                case 's':
                    fprintf(fp, "%d", number_of_faces());
                    break;
                case 'F':
                    fprintf(fp, "%g", surface_area());
                    break;
                case 'A':
                {
                    face_freq_table(vi);
 ...
 * \param[in] (ymin,ymax) the minimum and maximum y coordinates.
 * \param[in] (zmin,zmax) the minimum and maximum z coordinates. */
void voronoicell_neighbor::init(
    double xmin, double xmax, double ymin, double ymax, double zmin, double zmax)
{
 ...
 * \param (x2,y2,z2) a position vector for the third vertex.
 * \param (x3,y3,z3) a position vector for the fourth vertex. */
void voronoicell_neighbor::init_tetrahedron(double x0,
                                            double y0,
                                            double z0,
                                            double x1,
                                            double y1,
...
template bool voronoicell_base::nplane(
    voronoicell_neighbor &, double, double, double, double, int);

...
class voro_compute {
  public:
    ...
    voro_compute(voro_compute const &) = default;
    voro_compute &operator=(voro_compute const &) = default;

    template <class v_cell>
    bool compute_cell(v_cell &c, int ijk, int s, int ci, int cj, int ck);
    void find_voronoi_cell(double x,
                           double y,
                           double z,
                           int ci,
                           int cj,
  ...
  private:
    ...
    bool compute_min_max_radius(int di,
                                int dj,
                                int dk,
                                double fx,
                                double fy,
    ...
    {
        for (uint32_t *mp(mask); mp < mask + hxyz; mp++)
            *mp = 0;
    }
};

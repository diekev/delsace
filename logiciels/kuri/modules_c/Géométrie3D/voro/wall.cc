 ...
 * \return True if the point is inside, false if the point is outside.
 */
wall_cylinder::wall_cylinder(
    double xc_, double yc_, double zc_, double xa_, double ya_, double za_, double rc_, int w_id_)
    : w_id(w_id_), xc(xc_), yc(yc_), zc(zc_), xa(xa_), ya(ya_), za(za_),
      asi(1.0 / (xa_ * xa_ + ya_ * ya_ + za_ * za_)), rc(rc_)
{
}
 ...
 */
wall_cone::wall_cone(
    double xc_, double yc_, double zc_, double xa_, double ya_, double za_, double ang, int w_id_)
    : w_id(w_id_), xc(xc_), yc(yc_), zc(zc_), xa(xa_), ya(ya_), za(za_),
      asi(1.0 / (xa_ * xa_ + ya_ * ya_ + za_ * za_)), gra(std::tan(ang)), sang(std::sin(ang)),
      cang(std::cos(ang))
{
}

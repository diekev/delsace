...
#define VERS_CHAINE_DLS(x) dls::chaine((x).pointeur, (x).taille())
#define VERS_CHAINE_KURI(x) kuri::chaine((x).c_str(), (x).taille())
                                                            ...
                                                            kuri::chaine_statique nom)
{
    ...
    auto tableau = cherche_tableau(dico, VERS_CHAINE_DLS(nom));
    ...
    if (tableau != nullptr) {
        for (auto objet : tableau->valeur) {
            ...
            résultat.ajoute(VERS_CHAINE_KURI(obj_chaine->valeur));
        }
    }
...
{
    auto objet = cherche_chaine(dico, VERS_CHAINE_DLS(nom));
    if (objet != nullptr) {
        return VERS_CHAINE_KURI(objet->valeur);
    }
    return "";
}
...
static auto analyse_configuration(const char *chemin)
{
    ...
    if (obj_fichier->type == tori::type_objet::CHAINE) {
        config.fichier = VERS_CHAINE_KURI(static_cast<tori::ObjetChaine *>(obj_fichier)->valeur);
        ...
        config.fichier = VERS_CHAINE_KURI(nom_fichier_tmp);
        config.fichier_tmp = VERS_CHAINE_KURI(nom_fichier_tmp);
...
static kuri::chaine convertis_type(CXType const &type, dico_typedefs const &typedefs);

static kuri::chaine convertis_type_fonction(CXType const &type, dico_typedefs const &typedefs)
{
...
static kuri::chaine determine_operateur_binaire(CXCursor cursor, CXTranslationUnit trans_unit)
{
    ...
    if (expr != nullptr) {
        ...
        return VERS_CHAINE_KURI(op->getOpcodeStr().str());
    }
...
static kuri::chaine donne_chaine_pour_litérale(CXCursor cursor, CXTranslationUnit trans_unit)
{
    ...
    if (expr != nullptr) {
        if (cursor.kind == CXCursorKind::CXCursor_IntegerLiteral) {
            ...
            return VERS_CHAINE_KURI(stream.str());
        }
    }
    ...
    if (cursor.kind == CXCursorKind::CXCursor_CXXBoolLiteralExpr) {
        ...
        résultat = convertis_chaine(clang_getTokenSpelling(trans_unit, tokens[0]));
    }
...
static auto trouve_décalage(CXToken *tokens,
                            unsigned nombre_tokens,
                            int décalage,
                            CXTranslationUnit trans_unit)
{
    if (compare_token(tokens[décalage], trans_unit, ";")) {
        return 0;
    }

    auto dec = 0;

    for (auto i = décalage; i < nombre_tokens; ++i) {
...
static auto tokens_typealias(CXCursor cursor, CXTranslationUnit trans_unit, dico_typedefs &dico)
{
    ...
    for (auto i = 3u; i < nombre_tokens; ++i) {
        ...
        morceaux.ajoute(convertis_chaine(spelling));
                                              ...
                                              kuri::tableau<CXCursor> const &enfants)
{
    ...
    auto décalage_enfant = 0;

    auto décalage = 2;

    auto dec = trouve_décalage(tokens, nombre_tokens, décalage, trans_unit);

    if (dec == 0) {
        décalage += 1;
    }
    else {
        res.enfant_init = &enfants[décalage_enfant++];
        décalage += dec + 1;
    }

    dec = trouve_décalage(tokens, nombre_tokens, décalage, trans_unit);

    if (dec == 0) {
        décalage += 1;
    }
    else {
        res.enfant_comp = &enfants[décalage_enfant++];
        décalage += dec + 1;
    }

    if (!compare_token(tokens[décalage], trans_unit, ")")) {
        res.enfant_inc = &enfants[décalage_enfant++];
    }

    clang_disposeTokens(trans_unit, tokens, nombre_tokens);

    res.enfant_bloc = &enfants[décalage_enfant];
            ...
            case CXCursorKind::CXCursor_VarDecl:
            {
                ...
                if (nombre_enfants == 1) {
                    variable->expression = parse_expression(
                        enfants[décalage], trans_unit, flux_sortie);
                }
                syntaxeuse.ajoute_au_noeud_courant(variable);
                break;
            }
        ...
        switch (cursor.kind) {
            ...
            case CXCursorKind::CXCursor_UnaryExpr:
            {
                ...
                if (chn == "sizeof") {
                    ...
                    flux_sortie << convertis_type_sizeof(cursor, trans_unit, typedefs);
                    flux_sortie << ")";
                }
                else {
                    convertis_enfants(cursor, trans_unit, flux_sortie);
            ...
            case CXCursorKind::CXCursor_CXXNewExpr:
            {
                ...
                if (enfants.est_vide()) {
                    flux_sortie << convertis_type(cxtype, typedefs, true);
                }
                else {
                    /* tableau */
                    flux_sortie << '[';
                                        ...
                                        std::ostream &flux_sortie)
    {
        ...
        for (auto i = 0; i < enfants.taille(); ++i) {
            ...
            if (est_methode_cpp && param.kind == CXCursorKind::CXCursor_TypeRef) {
                ...
                flux_sortie << virgule;
                flux_sortie << "this : &";
                flux_sortie << convertis_type(param, typedefs);
#endif
                continue;
            }
...
void imprime_ligne(std::string tampon, uint32_t ligne, uint32_t colonne, uint32_t décalage)
{
...
int main(int argc, char **argv)
{
    ...
    if (nombre_diagnostics != 0) {
        ...
        for (auto i = 0u; i < nombre_diagnostics; ++i) {
            ...
            CXFile file = clang_getFile(unit, config.fichier);
    ...
    if (config.fichier_est_composite && config.fichier_tmp.taille() != 0) {
        kuri::chemin_systeme::supprime(config.fichier_tmp);
    }

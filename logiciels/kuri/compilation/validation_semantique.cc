...
RésultatValidation Sémanticienne::valide(UnitéCompilation *unité)
{
    m_unité = unité;
...
MétaProgramme *Sémanticienne::crée_métaprogramme_pour_directive(NoeudDirectiveExécute *directive)
{
    ...
    if (type_expression != m_espace->typeuse.type_rien) {
    ...
    auto métaprogramme = m_compilatrice.crée_métaprogramme(m_espace);
    métaprogramme->fonction = decl_entête;
    métaprogramme->directive = directive;
    directive->métaprogramme = métaprogramme;
...
RésultatValidation Sémanticienne::valide_sémantique_noeud(NoeudExpression *noeud)
{
    switch (noeud->genre) {
        ...
        {
            auto fini_execution = m_espace->interface_kuri->decl_fini_execution_kuri;
        ...
        {
            auto init_execution = m_espace->interface_kuri->decl_init_execution_kuri;
        ...
        case GenreNoeud::DIRECTIVE_EXÉCUTE:
        {
            ...
            if (noeud_directive->ident == ID::assert_) {
                if (type_expression != m_espace->typeuse.type_bool) {
            ...
            auto métaprogramme = crée_métaprogramme_pour_directive(noeud_directive);
        ...
        case GenreNoeud::DIRECTIVE_INSÈRE:
        {
            ...
            if (type != m_espace->typeuse.type_chaine) {
                m_espace->rapporte_erreur(
            ...
            if (insère->fichier == nullptr) {
                auto fichier = m_compilatrice.crée_fichier_pour_insère(m_espace, insère);
        ...
        case GenreNoeud::EXPRESSION_PRISE_ADRESSE:
        {
            ...
            if (type_opérande->est_type_type_de_données()) {
                ...
                }
                {
                    ...
                    type_connu = m_espace->typeuse.type_pointeur_pour(type_connu);
                }
        ...
        case GenreNoeud::EXPRESSION_NÉGATION_LOGIQUE:
        {
            ...
            négation->type = m_espace->typeuse.type_bool;
            break;
        }
        case GenreNoeud::EXPRESSION_INDEXAGE:
        {
            ...
            switch (type_gauche->genre) {
                ...
                case GenreNoeud::TABLEAU_FIXE:
                {
                    ...
                    auto res = évalue_expression(m_espace, droite->bloc_parent, droite);
                    ...
                    if (!res.est_erroné && res.valeur.est_entière()) {
                        ...
                        expr->aide_génération_code = IGNORE_VÉRIFICATION;
                    }
                ...
                {
                    expr->type = m_espace->typeuse.type_z8;
                    break;
                }
                default:
                {
        ...
        case GenreNoeud::INSTRUCTION_SAUFSI_STATIQUE:
        {
            ...
            if (inst->visité == false) {
                auto res = évalue_expression(m_espace, inst->bloc_parent, inst->condition);
        ...
        {
            noeud->type = m_espace->typeuse.type_ptr_nul;
            break;
        }
        case GenreNoeud::EXPRESSION_TAILLE_DE:
        {
            auto expr = noeud->comme_taille_de();
            expr->type = m_espace->typeuse.type_n32;
        ...
        case GenreNoeud::EXPRESSION_CONSTRUCTION_TABLEAU:
        {
            ...
            noeud->type = m_espace->typeuse.type_tableau_fixe(type_feuille,
        ...
        case GenreNoeud::EXPRESSION_INFO_DE:
        {
            ...
            switch (type->genre) {
                ...
                {
                    type_info_type = m_espace->typeuse.type_info_type_;
                    break;
                }
                ...
                {
                    type_info_type = m_espace->typeuse.type_info_type_structure;
                    break;
                }
                case GenreNoeud::DÉCLARATION_UNION:
                {
                    type_info_type = m_espace->typeuse.type_info_type_union;
                    break;
                }
                case GenreNoeud::TABLEAU_DYNAMIQUE:
                case GenreNoeud::TABLEAU_FIXE:
                {
                    type_info_type = m_espace->typeuse.type_info_type_tableau;
                    break;
                }
                case GenreNoeud::TYPE_TRANCHE:
                {
                    type_info_type = m_espace->typeuse.type_info_type_tranche;
                    break;
                }
                case GenreNoeud::FONCTION:
                {
                    type_info_type = m_espace->typeuse.type_info_type_fonction;
                    break;
                }
                ...
                {
                    type_info_type = m_espace->typeuse.type_info_type_enum;
                    break;
                }
                case GenreNoeud::DÉCLARATION_OPAQUE:
                {
                    type_info_type = m_espace->typeuse.type_info_type_opaque;
                    break;
                }
                case GenreNoeud::VARIADIQUE:
                {
                    type_info_type = m_espace->typeuse.type_info_type_variadique;
                    break;
                }
            ...
            noeud->type = m_espace->typeuse.type_pointeur_pour(type_info_type);
        ...
        case GenreNoeud::INSTRUCTION_POUSSE_CONTEXTE:
        {
            ...
            assert(m_espace->typeuse.type_contexte);
            assert(m_espace->globale_contexte_programme);

            if (!m_espace->globale_contexte_programme->possède_drapeau(
                    DrapeauxNoeud::DECLARATION_FUT_VALIDEE)) {
                return Attente::sur_déclaration(m_espace->globale_contexte_programme);
            }

            if (variable->type != m_espace->typeuse.type_contexte) {
                m_espace
        ...
        case GenreNoeud::EXPANSION_VARIADIQUE:
        {
            ...
            if (type_expr->est_type_type_de_données()) {
                ...
                if (type_expr->est_type_tableau_fixe()) {
                    ...
                    type_expr = m_espace->typeuse.crée_type_tranche(
        ...
        case GenreNoeud::RÉFÉRENCE_OPÉRATEUR_UNAIRE:
        {
            ...
            auto opérateurs = m_espace->opérateurs.verrou_lecture();
    ...
    NoeudExpressionRubrique *expression_rubrique)
{
    ...
    if (structure->est_référence_déclaration()) {
        ...
        if (decl->est_déclaration_module()) {
            ...
            contexte_recherche_symbole.fichier = m_espace->fichier(
...
RésultatValidation Sémanticienne::valide_entête_opérateur(NoeudDéclarationEntêteFonction *decl)
{
    ...
    if (type_résultat == m_espace->typeuse.type_rien &&
        !est_assignation_composée(decl->lexème->genre)) {
    ...
    if (est_opérateur_bool(decl->lexème->genre) && type_résultat != m_espace->typeuse.type_bool) {
    ...
    NoeudDéclarationOpérateurPour *opérateur)
{
    ...
    auto table_opérateurs = m_espace->opérateurs->donne_ou_crée_table_opérateurs(type_itéré);
    ...
    NoeudDéclarationEntêteFonction *decl)
{
    ...
    if (decl->ident == ID::principale) {
        ...
        if (decl->param_sortie->type != m_espace->typeuse.type_rien) {
            m_espace->rapporte_erreur(
                decl->param_sortie,
    ...
    NoeudDéclarationEntêteFonction *decl)
{
    ...
    auto decl_existante = decl->bloc_parent->déclaration_pour_ident(decl->ident);
...
RésultatValidation Sémanticienne::valide_arbre_aplatis(NoeudExpression *déclaration)
{
    aplatis_arbre(déclaration, m_arbre_courant);

    TENTE(valide_arbre_aplatis(déclaration, m_arbre_courant));
...
RésultatValidation Sémanticienne::valide_expression_retour(NoeudInstructionRetour *inst)
{
    ...
    if (fonction) {
        ...
        type_sortie = m_espace->typeuse.type_rien;
    }
    ...
    if (inst->expression == nullptr) {
        inst->type = m_espace->typeuse.type_rien;
    ...
    if (est_corps_texte) {
        ...
        inst->type = m_espace->typeuse.type_chaine;
        return CodeRetourValidation::OK;
    }
                                                               ...
                                                               NoeudBloc *bloc_recherche)
{
    ...
    auto fichier = m_espace->fichier(expr->lexème->fichier);
 ...
 auto déclarations = kuri::tablet<NoeudDéclaration *, 10>();
	trouve_déclarations_dans_bloc_ou_module(déclarations, bloc_recherche, expr->ident, fichier);

	if (déclarations.taille() == 0) {
    ...
    if (decl == nullptr) {
        ...
        if (site_monomorphisation) {
            fichier = m_espace->fichier(site_monomorphisation->lexème->fichier);
            decl = trouve_dans_bloc_ou_module(
...
RésultatValidation Sémanticienne::valide_type_opaque(NoeudDéclarationTypeOpaque *decl)
{
    ...
    if (type_opacifie->est_type_polymorphique()) {
        ...
        m_espace->graphe_dépendance->connecte_type_type(decl, type_opacifie);
    }
...
RésultatValidation Sémanticienne::valide_fonction(NoeudDéclarationCorpsFonction *decl)
{
    ...
    if (est_corps_texte) {
         ...
         */
        auto métaprogramme = m_compilatrice.métaprogramme_pour_fonction(entête);
        m_compilatrice.gestionnaire_code->requiers_compilation_métaprogramme(m_espace,
                                                                             métaprogramme);
    }
...
RésultatValidation Sémanticienne::valide_énum_impl(NoeudEnum *decl)
{
    ...
    m_espace->opérateurs->ajoute_opérateur_basique_enum(m_espace->typeuse, decl);
    ...
    POUR (*decl->bloc->expressions.verrou_ecriture()) {
        ...
        if (expr != nullptr) {
            auto res = évalue_expression(m_espace, decl->bloc, expr);
...
RésultatValidation Sémanticienne::valide_énum(NoeudEnum *decl)
{
    ...
    if (decl->est_type_erreur()) {
        ...
        decl->type_sous_jacent = m_espace->typeuse.type_n32;
    }
    else {
        decl->type_sous_jacent = m_espace->typeuse.type_z32;
    }
...
struct ConstructriceRubriquesTypeComposé {
  ...
  public:
    ConstructriceRubriquesTypeComposé(TypeComposé &type_composé, NoeudBloc const *bloc)
    ...
    void ajoute_rubrique_invisible(Typeuse &typeuse)
    {
        m_rubriques_non_constant += 1;
        /* Ajoute une rubrique, d'un octet de taille. */
        m_type_composé.rubriques.ajoute(
            {nullptr, typeuse.type_bool, ID::chaine_vide, 0, 0, nullptr});
    }
...
static bool la_rubrique_référence_le_type_par_valeur(TypeComposé const *type_composé,
...
static void rapporte_erreur_type_rubrique_invalide(EspaceDeTravail *espace,
                                                   TypeComposé const *type_composé,
...
RésultatValidation Sémanticienne::valide_structure(NoeudStruct *decl)
{
    ...
    if (decl->est_externe && decl->bloc == nullptr) {
        decl->drapeaux |= DrapeauxNoeud::DECLARATION_FUT_VALIDEE;
        /* INITIALISATION_TYPE_FUT_CRÉÉE est à cause de attente_sur_type_si_drapeau_manquant */
    ...
    if (constructrice.donne_compte_rubriques_non_constant() == 0) {
        if (!decl->est_externe) {
            constructrice.ajoute_rubrique_invisible(m_espace->typeuse);
        }
    }
...
RésultatValidation Sémanticienne::valide_union(NoeudUnion *decl)
{
    ...
    if (!decl->est_nonsure) {
        crée_type_structure(m_espace->typeuse, type_union, type_union->décalage_indice);
    }
...
RésultatValidation Sémanticienne::valide_déclaration_constante(NoeudDéclarationConstante *decl)
{
    ...
    if (!expression->type->est_type_type_de_données()) {
        ...
        auto res_exec = évalue_expression(m_espace, decl->bloc_parent, expression);
...
void Sémanticienne::rapporte_erreur_type_opération(const Type *type_gauche,
                                                   ...
                                                   NoeudExpression *noeud)
{
    erreur::lance_erreur_type_opération(type_gauche, type_droite, *m_espace, noeud);
}
                                                         ...
                                                         TransformationType const &transformation)
{
    ...
    if (transformation.type == TypeTransformation::CONVERTIS_ENTIER_CONSTANT) {
        ...
        visite_noeud(
            expression, PréférenceVisiteNoeud::ORIGINAL, true, [&](NoeudExpression const *noeud) {
    ...
    GenreLexème const opération_droite, GenreLexème const opération_gauche)
{
    ...
    const GenreLexème opérations_compatibles[][2] = {
        {GenreLexème::INFERIEUR, GenreLexème::INFERIEUR_ÉGAL},
        {GenreLexème::SUPERIEUR, GenreLexème::SUPERIEUR_ÉGAL},
    };
...
RésultatValidation Sémanticienne::valide_opérateur_binaire_chaine(NoeudExpressionBinaire *expr)
{
    ...
    expr->type = m_espace->typeuse.type_bool;
    expr->op = candidat.op;
...
RésultatValidation Sémanticienne::valide_opérateur_binaire_type(NoeudExpressionBinaire *expr)
{
    ...
    switch (expr->lexème->genre) {
        ...
        case GenreLexème::EGALITE:
        {
            ...
            auto op = m_espace->opérateurs->op_comp_égal_types;
            expr->type = op->type_résultat;
            expr->op = op;
            return CodeRetourValidation::OK;
        }
        ...
        {
            // XXX - aucune raison de prendre un verrou ici
            auto op = m_espace->opérateurs->op_comp_diff_types;
            expr->type = op->type_résultat;
            expr->op = op;
            return CodeRetourValidation::OK;
        }
...
RésultatValidation Sémanticienne::valide_expression_logique(NoeudExpressionLogique *logique)
{
    ...
    logique->type = m_espace->typeuse.type_bool;
    return CodeRetourValidation::OK;
}
    ...
    kuri::Synchrone<RegistreDesOpérateurs> &registre)
{
    ...
    if (itéré->est_plage()) {
        return TypageItérandeBouclePour{
            GÉNÈRE_BOUCLE_PLAGE, type_variable_itérée, type_variable_itérée};
    }
    ...
    if (type_variable_itérée->est_type_tableau_dynamique() ||
        ...
        return TypageItérandeBouclePour{GÉNÈRE_BOUCLE_TABLEAU, type_itérateur, type_index};
    }
    ...
    if (est_type_entier(type_variable_itérée) ||
        ...
        if (type_variable_itérée->est_type_entier_constant()) {
            type_itérateur = typeuse.type_z32;
        }

        return TypageItérandeBouclePour{
            GÉNÈRE_BOUCLE_PLAGE_IMPLICITE, type_itérateur, type_itérateur};
    }
...
RésultatValidation Sémanticienne::valide_instruction_pour(NoeudPour *inst)
{
    ...
    auto const résultat_typage_itérande = détermine_typage_itérande(
        m_contexte, expression, m_espace->typeuse, m_espace->opérateurs);
...
RésultatValidation Sémanticienne::valide_instruction_si(NoeudSi *inst)
{
    ...
    if (type_inféré->est_type_entier_constant()) {
        type_inféré = m_espace->typeuse.type_z32;
    }
    ...
    NoeudDirectiveDépendanceBibliothèque *noeud)
{
    ...
    contexte_recherche_symbole.fichier = m_espace->fichier(noeud->lexème->fichier);
    contexte_recherche_symbole.fonction_courante = fonction_courante();
    ...
    NoeudExpressionTypeTableauFixe *expr)
{
    ...
    auto res = évalue_expression(m_espace, expression_taille->bloc_parent, expression_taille);
    ...
    NoeudExpressionConstructionTableauTypé *tableau)
{
    ...
    tableau->type = m_espace->typeuse.type_tableau_fixe(type_élément,
                                                        feuilles->expressions.taille());

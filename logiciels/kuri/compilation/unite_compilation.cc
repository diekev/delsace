...
static void chaine_attente_recursive(Enchaineuse &fc, UnitéCompilation const *unité, int tabs)
{
    auto const chaine_tabs = donne_chaine_tabulation(tabs * 4);

    auto attentes = unité->donne_attentes();
    fc << chaine_tabs << "Unité pour espace \"" << unité->espace->nom
       << "\" (état : " << unité->donne_état() << ")\n";

    fc << chaine_tabs << "- historique :\n";

    POUR (unité->donne_historique()) {

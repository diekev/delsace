...
void ÉtatChargementFichiers::ajoute_unité_pour_validation_entête(UnitéCompilation *unité)
{
    enfile(&file_unités_validation_entêtes, unité);
}

void ÉtatChargementFichiers::supprime_unité_pour_validation_entête(UnitéCompilation *unité)
{
    défile(&file_unités_validation_entêtes, unité);
}

void ÉtatChargementFichiers::enfile(UnitéCompilation **file, UnitéCompilation *unité)
{
    if (*file == nullptr) {
        *file = unité;
        return;
...
void ÉtatChargementFichiers::défile(UnitéCompilation **file, UnitéCompilation *unité)
{
    if (unité->précédente) {
...
void GestionnaireCode::typage_terminé(UnitéCompilation *unité)
{
    ...
    if (unité->noeud->est_entete_fonction()) {
        m_état_chargement_fichiers.supprime_unité_pour_validation_entête(unité);
        flush_noeuds_à_typer();
    }

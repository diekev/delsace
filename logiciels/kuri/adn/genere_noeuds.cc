...
struct GeneratriceCodeCPP {
	...
	void genere_fichier_entete_arbre_syntaxique(FluxSortieCPP &os)
	{
		...
		os << "template <typename T> using tableau_synchrone = dls::outils::Synchrone<kuri::tableau<T, int>>;\n";
		...
		os << "void imprime_arbre_substitue(const NoeudExpression *racine, std::ostream &os, int profondeur);\n\n";
		os << "void imprime_arbre(NoeudExpression const *racine, std::ostream &os, int profondeur, bool substitution = false);\n\n";

		// Copie de l'arbre
		os << "NoeudExpression *copie_noeud(AssembleuseArbre *assem, NoeudExpression const *racine, NoeudBloc *bloc_parent);\n\n";
	...
	void genere_impression_arbre_syntaxique(FluxSortieCPP &os)
	{
		...
		os << "void imprime_arbre(NoeudExpression const *racine, std::ostream &os, int profondeur, bool substitution)\n";
		os << "{\n";
		os << "\tif (!racine) {\n";
		os << "\t\treturn;\n";
		os << "\t}\n";
			...
			genere_code_pour_enfant(os, it, false, [&os](ProteineStruct &, Membre const &membre)
			{
				if (membre.type->est_tableau()) {
					...
					if (type_tableau->est_synchrone) {
						os << "\t\t\tPOUR ((*racine_typee->" << membre.nom << ".verrou_lecture())) {\n";
					}
					else {
					...
					os << "\t\t\timprime_arbre(racine_typee->" << membre.nom << ", os, profondeur + 1, substitution);\n";
				}
			});
		...
		POUR (proteines_struct) {
			...
			genere_code_pour_enfant(os, it, false, [&os](ProteineStruct &, Membre const &membre)
			{
				if (membre.type->est_tableau()) {
					...
					os << "\t\t\t\tconst auto etendue_" << membre.nom << " = calcule_etendue_noeud(it);\n";
					os << "\t\t\t\tetendue.fusionne(etendue_" << membre.nom << ");\n";
					os << "\t\t\t}\n";
				}
				else {
					os << "\t\t\tconst auto etendue_" << membre.nom << " = calcule_etendue_noeud(racine_typee->" << membre.nom << ");\n";
					os << "\t\t\tetendue.fusionne(etendue_" << membre.nom << ");\n";
				}
			});
	...
	{
		os << "NoeudExpression *copie_noeud(AssembleuseArbre *assem, const NoeudExpression *racine, NoeudBloc *bloc_parent)\n";
		os << "{\n";
		os << "\tif(!racine) {\n";
		os << "\t\treturn nullptr;\n";
		os << "\t}\n";
		...
		POUR (proteines_struct) {
			...
			if (nom_genre.nom_cpp() == "DECLARATION_CORPS_FONCTION") {
				os << "\t\t\tassert_rappel(false, [&]() { std::cerr << \"Tentative de copie d'un corps de fonction seul\\n\"; });\n";
				os << "\t\t\tbreak;\n";
				os << "\t\t}\n";
				continue;
			}

			os << "\t\t\tnracine = assem->cree_noeud<GenreNoeud::" << nom_genre << ">(racine->lexeme);\n";
			...
			if (nom_genre.nom_cpp() == "DECLARATION_STRUCTURE") {
				...
				os << "\t\t\t\tcopie->bloc_constantes = copie_noeud(assem, orig->bloc_constantes, bloc_parent)->comme_bloc();\n";
				os << "\t\t\t\tbloc_parent = copie->bloc_constantes;\n";
				os << "\t\t\t}\n";
			}
			else if (nom_genre.nom_cpp() == "DECLARATION_ENTETE_FONCTION") {
				os << "\t\t\tcopie->bloc_constantes = assem->cree_bloc_seul(nullptr, bloc_parent);\n";
				os << "\t\t\tcopie->bloc_parametres = assem->cree_bloc_seul(nullptr, copie->bloc_constantes);\n";
			...
			it->pour_chaque_enfant_recursif([&](const Membre &enfant){
				...
				// Les corps des fonctions sont copiées en même temps, et non à travers un appel séparé
				if (nom_genre.nom_cpp() == "DECLARATION_ENTETE_FONCTION" && nom_enfant.nom_cpp() == "corps") {
					return;
				}
			...
			else if (dls::outils::est_element(nom_genre.nom_cpp(), "INSTRUCTION_POUR", "INSTRUCTION_BOUCLE", "INSTRUCTION_REPETE", "INSTRUCTION_TANTQUE")) {
	...
	void genere_fichier_entete_noeud_code(FluxSortieCPP &os)
	{
		...
		POUR (proteines_struct) {
			...
			if (!it->accede_nom_genre().est_nul()) {
				os << "\t" << it->accede_nom_code() << "() { genre = GenreNoeud::" << it->accede_nom_genre() << "; }\n";
			...
			for (auto &membre : it->membres()) {
				...
				if (nom_membre.nom_cpp() == "valeur" && nom_code.nom_cpp() == "NoeudCodeLitteraleChaine") {
				...
				if (membre.type->est_pointeur() || (membre.type->est_tableau() && membre.type->comme_tableau()->type_pointe->est_pointeur())) {
					os << " *";
				}
		...
		POUR (proteines_struct) {
			...
			os << "inline const " << nom_noeud << " *NoeudCode::comme_" << nom_comme << "() const\n";
			os << "{\n";
			os << "\tassert_rappel(est_" << nom_comme << R"((), [this]() { std::cerr << "Le genre de noeud est " << this->genre << "\n"; }))" << ";\n";
		...
		os << "\tNoeudExpression *convertis_noeud_code(EspaceDeTravail *espace, GeranteChaine &gerante_chaine, NoeudCode *racine);\n\n";
		os << "\tNoeudCode *convertis_noeud_syntaxique(EspaceDeTravail *espace, NoeudExpression *racine);\n\n";
	...
	void genere_fichier_source_noeud_code(FluxSortieCPP &os)
	{
		...
		os << "NoeudCode *ConvertisseuseNoeudCode::convertis_noeud_syntaxique(EspaceDeTravail *espace, NoeudExpression *racine)\n";
		os << "{\n";
		os << "\tif (!racine) {\n";
		os << "\t\treturn nullptr;\n";
		os << "\t}\n";
		...
		POUR (proteines_struct) {
			...
			os << "\t\t\tauto n = noeuds_code_" << it->accede_nom_comme() << ".ajoute_element();\n";
			// Renseigne directement le noeud code afin d'éviter les boucles infinies résultant en des surempilages
			// d'appels quand nous convertissons notamment les entêtes et les corps de fonctions qui se référéncent
			// mutuellement.
			os << "\t\t\tracine->noeud_code = n;\n";

			genere_code_pour_enfant(os, it, true, [&os, it, this](ProteineStruct &, Membre const &membre)
			{
				const auto nom_membre = membre.nom;
				...
				const auto desc_type = table_desc.valeur_ou(membre.type->accede_nom().nom_kuri(), nullptr);

				if (!desc_type) {
					if (membre.type->est_tableau()) {
						const auto type_tableau = membre.type->comme_tableau();
						if (type_tableau->est_synchrone) {
							os << "\t\t\tPOUR (*racine_typee->" << nom_membre << ".verrou_lecture()) {\n";
						}
						else {
						...
						if (nom_membre.nom_cpp() == "valeur" && it->accede_nom_code().nom_cpp() == "NoeudCodeLitteraleChaine") {
							os << "\t\t\tn->valeur = { &racine_typee->lexeme->chaine[0], racine_typee->lexeme->chaine.taille() };\n";
						}
						else {
							os << "\t\t\tn->" << nom_membre << " = racine_typee->" << nom_membre << ";\n";
						}
					}
				...
				if (membre.type->est_tableau()) {
					...
					os << "\t\t\t\tn->" << nom_membre << ".ajoute(convertis_noeud_syntaxique(espace, it)";

					if (desc_type->accede_nom_code().nom_cpp() != "NoeudCode" && desc_type->accede_nom_code().nom_cpp() != "") {
					...
					os << "\t\t\t\tn->" << nom_membre << " = convertis_noeud_syntaxique(espace, racine_typee->" << nom_membre << ")";
					if (desc_type->accede_nom_code().nom_cpp() != "NoeudCode" && desc_type->accede_nom_code().nom_cpp() != "") {
						os << "->comme_" << desc_type->accede_nom_comme() << "()";
					}
			...
			os << "\t\t\tif (racine_typee->ident) { n->nom = racine_typee->ident->nom; } else if (racine_typee->lexeme) { n->nom = racine_typee->lexeme->chaine; }\n";
		...
		os << "NoeudExpression *ConvertisseuseNoeudCode::convertis_noeud_code(EspaceDeTravail *espace, GeranteChaine &gerante_chaine, NoeudCode *racine)\n";
		os << "{\n";
		os << "\tif (!racine) {\n";
		os << "\t\treturn nullptr;\n";
		os << "\t}\n";
		...
		POUR (proteines_struct) {
			...
			os << "\t\t\tauto n = noeuds_expr_" << it->accede_nom_comme() << ".ajoute_element();\n";
			...
			genere_code_pour_enfant(os, it, true, [&os, it, this](ProteineStruct &, Membre const &membre)
			{
				...
				if (!desc_type) {
					if (membre.type->est_tableau()) {
						os << "\t\t\tPOUR (racine_typee->" << nom_membre << ") {\n";
						os << "\t\t\t\tn->" << nom_membre << membre.type->accesseur() << "ajoute(it);\n";
						os << "\t\t\t}\n";
					}
					else {
						if (nom_membre.nom_cpp() == "valeur" && it->accede_nom_code().nom_cpp() == "NoeudCodeLitteraleChaine") {
							os << "\t\tn->valeur = gerante_chaine.ajoute_chaine(racine_typee->valeur);\n";
						}
						else {
							os << "\t\t\tn->" << nom_membre << " = racine_typee->" << nom_membre << ";\n";
						}
				...
				if (membre.type->est_tableau()) {
					...
					os << "\t\t\tif (racine_typee->" << nom_membre << ") {\n";
					os << "\t\t\t\tn->" << nom_membre << " = convertis_noeud_code(espace, gerante_chaine, racine_typee->" << nom_membre << ")";
	...
	void genere_code_pour_enfant(FluxSortieCPP &os, ProteineStruct *racine, bool inclus_code, std::function<void(ProteineStruct &, const Membre &)> rappel)
	{
...
NoeudBloc *AssembleuseArbre::empile_bloc(Lexeme const *lexeme)
{
	...
	return bloc;
		...
		POUR (proteines_struct) {
			...
			os << it->nom() << " *AssembleuseArbre::cree_" << it->accede_nom_comme() << "(const Lexeme *lexeme)\n";
			os << "{\n";
			os << "\treturn cree_noeud<GenreNoeud::" << nom_genre << ">(lexeme)->comme_" << it->accede_nom_comme() << "();\n";
			os << "}\n";
		}
	}
	...
	NoeudExpression *cree_noeud(Lexeme const *lexeme)
	{
		...
		POUR (proteines_struct) {
			...
			os << "\t" << it->nom() << " *cree_" << it->accede_nom_comme() << "(const Lexeme *lexeme);\n";
		}
	...
	void genere_fichier_source_allocatrice(FluxSortieCPP &os)
	{
		...
		POUR (proteines_struct) {
			...
			os << "\tstats_arbre.fusionne_entree({"
			   << '"' << it->nom() << '"' << ", "
		...
		POUR (proteines_struct) {
			...
			{
				if (membre.type->est_tableau()) {
					const auto nom_tableau = enchaine("taille_max_", it->accede_nom_comme().nom_cpp(), "_", membre.nom.nom_cpp());
		...
		POUR (proteines_struct) {
			...
			it->pour_chaque_membre_recursif([&](const Membre &membre) {
				if (membre.type->est_tableau()) {
					const auto nom_membre = membre.nom;
					const auto nom_tableau = enchaine("taille_max_", it->accede_nom_comme().nom_cpp(), "_", nom_membre.nom_cpp());
	...
	void genere_fichier_entete_allocatrice(FluxSortieCPP &os)
	{
		...
		os << "\ttableau_page<Monomorphisations<NoeudDeclarationEnteteFonction>> m_monomorphisations_fonctions{};\n";
		...
		POUR (proteines_struct) {
			...
			if (nom_genre.nom_cpp() == "DECLARATION_ENTETE_FONCTION") {
				...
				os << "\t\t\t\treturn m_noeuds_" << it->accede_nom_comme() << ".ajoute_element();\n";
			}
...
struct GeneratriceCodeKuri {
	void genere_fichier_kuri_noeud_code(FluxSortieKuri &os, kuri::tableau<Proteine *> const &proteines)
	{

...
void ajoute_calque_projection_triplanaire(Maillage *maillage)
{
    ...
    for (auto i = 0; i < nombre_polygones; ++i) {
        ...
        auto const angle_xz = std::abs(
            poly->nor.y);  // abs(produit_scalaire(poly->nor, dls::math::vec3f(0.0, 1.0, 0.0)));
        auto const angle_yz = std::abs(
            poly->nor.x);  // abs(produit_scalaire(poly->nor, dls::math::vec3f(1.0, 0.0, 0.0)));
        ...
        for (size_t j = 0; j < poly->res_v; ++j) {
            for (size_t k = 0; k < poly->res_u; ++k) {
                ...
                auto const couleur = (couleur_xy * angle_xy + couleur_xz * angle_xz +
                                      couleur_yz * angle_yz) *
                                     poids;
...
void assigne_texels_resolution(Maillage *maillage, unsigned int texels_par_cm)
{
    ...
    for (auto i = 0; i < maillage->nombre_polygones(); ++i) {
        ...
        auto const res_u = static_cast<unsigned int>(cote0 * 100.0f *
                                                     static_cast<float>(texels_par_cm));
        auto const res_v = static_cast<unsigned int>(cote1 * 100.0f *
                                                     static_cast<float>(texels_par_cm));
...
void Maillage::ajoute_quad(const long s0, const long s1, const long s2, const long s3)
{
    ...
    for (long i = 0; i < nombre_sommet; ++i) {
        ...
        auto iter = m_tableau_arretes.trouve(
            std::make_pair(arrete->s[1]->index, arrete->s[0]->index));
        ...
        m_tableau_arretes.insere(
            {std::make_pair(arrete->s[0]->index, arrete->s[1]->index), arrete});

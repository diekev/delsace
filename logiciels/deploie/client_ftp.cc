 ...
 * @endcode
 */
bool CFTPClient::InitSession(const std::string& strHost,
...
bool CFTPClient::Info(const std::string& strRemoteFile, struct FileInfo& oFileInfo) const
{
	...
	if (CURLE_OK == res)
	{
		...
		{
			oFileInfo.tFileMTime = lFileTime;
			bRes = true;
		}
 ...
 * @endcode
 */
bool CFTPClient::List(const std::string& strRemoteFolder,
							std::string& strList,
								  ...
								  const bool& bCreateDir) const
{
	...
	if (stat(strLocalFile.c_str(), &file_info) == 0)
	{
	   ...
	   make sure that to pass in a type 'long' argument. */
		curl_easy_setopt(m_pCurlSession, CURLOPT_INFILESIZE_LARGE, file_info.st_size);
...
std::string CFTPClient::StringFormat(const std::string strFormat, ...)
{
	...
	while(true)
	{
		...
		int iFinaln = vsnprintf(&pFormatted[0], static_cast<size_t>(n), strFormat.c_str(), ap);
		va_end(ap);
...
size_t CFTPClient::WriteToFileCallback(void* buff, size_t size, size_t nmemb, void* data)
{
	...
	{
		pFileStream->write(reinterpret_cast<char*>(buff), static_cast<long>(size * nmemb));
	}
...
size_t CFTPClient::ReadFromFileCallback(void* ptr, size_t size, size_t nmemb, void* stream)
{
	...
	if (pFileStream->is_open())
	{
		pFileStream->read(reinterpret_cast<char*>(ptr), static_cast<long>(size * nmemb));
...
size_t CFTPClient::WriteItCallback(char* buff, size_t size, size_t nmemb, void* cb_data)
{
	...
	if (data->ofsOutput.is_open())
	{
		data->ofsOutput.write(buff, static_cast<long>(size * nmemb));
		written = nmemb;
	}
	/*else
   {

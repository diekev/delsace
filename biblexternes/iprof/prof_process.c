...
static void update_history(Prof_Zone_Stack *c)
{
   ...
   self_time = (double)(c->total_self_ticks) * timestamps_to_seconds;
   hier_time = (double)(c->total_hier_ticks) * timestamps_to_seconds;
...
static Prof_Report *allocate_buffer(int n)
{
   ...
   pob->num_record = n;
   pob->record = (Prof_Report_Record *) malloc(sizeof(*pob->record) * (unsigned)(pob->num_record));
...
static double compute_heat(double variance, double value)
{
   ...
   const double VARIANCE_TOLERANCE_FACTOR = 0.5;
...
Prof_Report *Prof_create_report(void)
{
   ...
   const char *displayed_quantity_name;
   int i,s;
   Prof_Report *pob;
   ...
   if (avg_frame_time == 0.0) avg_frame_time = 0.01;
   ...
   if (displayed_quantity == Prof_CALL_GRAPH) {
      ...
      for (i=0; i < pob->num_record; ++i) {
		 if (pob->record[i].values[0] != 0.0 || pob->record[i].values[1] != 0.0 || pob->record[i].values[2] != 0.0) {
	  ...
	  qsort(pob->record, (unsigned)(pob->num_record), sizeof(pob->record[0]), pob_expand_compare);
	  ...
	  qsort(pob->record, (unsigned)(pob->num_record), sizeof(pob->record[0]), pob_compare);
